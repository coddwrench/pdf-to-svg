...
namespace  IText.IO.Codec {
    ...
    public class TiffWriter {
        ...
        public virtual void WriteFile(Stream stream) {
            ...
            var offset = 8 + GetIfdSize();
            foreach (var field in ifd.Values) {
                var size = field.GetValueSize();
                if (size > 4) {
        ...
        public abstract class FieldBase {
            ...
            public virtual int GetValueSize() {
                return (data.Length + 1) & unchecked((int)(0xfffffffe));
            }
        ...
        public class FieldAscii : FieldBase {
            public FieldAscii(int tag, string values)
                ...
                var b = values.GetBytes(Encoding.ASCII);
        ...
        public static void CompressLZW(Stream stream, int predictor, byte[] b, int height, int samplesPerPixel, int
            ...
            if (!usePredictor) {
                ...
                for (var i = 0; i < height; i++) {
                    ...
                    for (var j = stride - 1; j >= samplesPerPixel; j--) {
