...
namespace  IText.IO.Source {
    public class GetBufferedRandomAccessSource : IRandomAccessSource {
        ...
        public GetBufferedRandomAccessSource(IRandomAccessSource source) {
            this.source = source;
            getBuffer = new byte[(int)Math.Min(Math.Max(source.Length() / 4, 1), 4096)];
            getBufferStart = -1;
            getBufferEnd = -1;
        }
        ...
        public virtual int Get(long position) {
            if (position < getBufferStart || position > getBufferEnd) {
                var count = source.Get(position, getBuffer, 0, getBuffer.Length);
                if (count == -1) {
                    return -1;
                }
            ...
            var bufPos = (int)(position - getBufferStart);
