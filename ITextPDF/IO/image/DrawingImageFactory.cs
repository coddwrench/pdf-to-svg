...
namespace  IText.IO.Image {
    internal class DrawingImageFactory {
        /// <summary>Gets an instance of an Image from <see cref="MediaTypeNames.Image"/></summary>
        /// <param name="image">the <see cref="MediaTypeNames.Image"/> to convert</param>
        ...
        /// <summary>
        /// Gets an instance of an Image from a <see cref="MediaTypeNames.Image"/>.
        /// </summary>
        /// <param name="image">the <see cref="MediaTypeNames.Image"/> to convert</param>
        /// <param name="color">
        ...
        public static ImageData GetImage(System.Drawing.Image image, Color? color, bool forceBW) {
            ...
            if (forceBW) {
                            ...
                            if (!shades) {
                                if (alpha != 0 && alpha != 255) {
                                    ...
                                    if (alpha == 0) {
                                        ...
                                        for (var prevPixelI = 0; prevPixelI <= i; prevPixelI++) {
                                            for (var prevPixelJ = 0;
                                                prevPixelJ < (prevPixelI == i ? j : h);
                                                prevPixelJ++) {
                                                var prevPxV = bm.GetPixel(prevPixelI, prevPixelJ).ToArgb();
                ...
                var img = ImageDataFactory.Create(w, h, 3, 8, pixelsByte, transparency);
                if (smask != null) {
                    var sm = ImageDataFactory.Create(w, h, 1, 8, smask, null);
                    sm.MakeMask();
                    img.imageMask = sm;
                }
                return img;
        ...
        /// <param name="?"></param>
        /// <returns></returns>
        public static ImageData GetImage(System.Drawing.Image image, ImageFormat format) {
