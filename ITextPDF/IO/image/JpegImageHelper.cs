...
namespace  IText.IO.Image {
    internal class JpegImageHelper {
        ...
        private static readonly int[] VALID_MARKERS = { 0xC0, 0xC1, 0xC2 };

        /// <summary>This is a type of marker.</summary>
        private const int UNSUPPORTED_MARKER = 1;

        /// <summary>Unsupported Jpeg markers.</summary>
        private static readonly int[] UNSUPPORTED_MARKERS = { 0xC3, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 
            0xCB, 0xCD, 0xCE, 0xCF };
        ...
        private static readonly int[] NOPARAM_MARKERS = { 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7
            , 0xD8, 0x01 };
        ...
        private static readonly byte[] JFIF_ID = { 0x4A, 0x46, 0x49, 0x46, 0x00 };

        /// <summary>sequence preceding Photoshop resolution data</summary>
        private static readonly byte[] PS_8BIM_RESO = { 0x38, 0x42, 0x49, 0x4d, 0x03, 0xed };
        ...
        internal static void AttemptToSetIccProfileToImage(byte[][] icc, ImageData image) {
            if (icc != null) {
                ...
                try {
                    ...
                    LOGGER.Error(MessageFormatUtil.Format(LogMessageConstant.DURING_CONSTRUCTION_OF_ICC_PROFILE_ERROR_OCCURRED
            ...
            while (true) {
                ...
                if (v == 0xFF) {
                    ...
                    if (firstPass && marker == M_APP0) {
                        ...
                        var bcomp = new byte[JFIF_ID.Length];
                        var r = jpegStream.Read(bcomp);
                        if (r != bcomp.Length) {
                        ...
                        var units = jpegStream.Read();
                        var dx = GetShort(jpegStream);
                        var dy = GetShort(jpegStream);
                        if (units == 1) {
                            image.SetDpi(dx, dy);
                        }
                        else {
                    ...
                    if (marker == M_APPE) {
                        ...
                        if (byteappe.Length >= 12) {
                            var appe = JavaUtil.GetStringForBytes(byteappe, 0, 5, "ISO-8859-1");
                    ...
                    if (marker == M_APP2) {
                        ...
                        if (byteapp2.Length >= 14) {
                            var app2 = JavaUtil.GetStringForBytes(byteapp2, 0, 11, "ISO-8859-1");
                    ...
                    if (marker == M_APPD) {
                        ...
                        for (k_1 = 0; k_1 < len - PS_8BIM_RESO.Length; k_1++) {
                            var found = true;
                            for (var j = 0; j < PS_8BIM_RESO.Length; j++) {
                        ...
                        if (k_1 < len - PS_8BIM_RESO.Length) {
                            ...
                            var resosize = (byteappd[k_1] << 24) + (byteappd[k_1 + 1] << 16) + (byteappd[k_1 + 2] << 8) + byteappd[k_1
                                 + 3];
                            // should be 16
                            if (resosize != 16) {
                            ...
                            k_1 += 4;
                            var dx = (byteappd[k_1] << 8) + (byteappd[k_1 + 1] & 0xff);
                            k_1 += 2;
                            // skip 2 unknown bytes
                            k_1 += 2;
                            var unitsx = (byteappd[k_1] << 8) + (byteappd[k_1 + 1] & 0xff);
                            k_1 += 2;
                            // skip 2 unknown bytes
                            k_1 += 2;
                            var dy = (byteappd[k_1] << 8) + (byteappd[k_1 + 1] & 0xff);
                            k_1 += 2;
                            // skip 2 unknown bytes
                            k_1 += 2;
                            var unitsy = (byteappd[k_1] << 8) + (byteappd[k_1 + 1] & 0xff);
                    ...
                    if (markertype == UNSUPPORTED_MARKER) {
	                    throw new IOException(IOException._1UnsupportedJpegMarker2).SetMessageParams(errorID, JavaUtil.IntegerToString
		                    (marker));
                    }
        ...
        private static int Marker(int marker) {
            ...
            for (var i = 0; i < NOPARAM_MARKERS.Length; i++) {
                if (marker == NOPARAM_MARKERS[i]) {
                    return NOPARAM_MARKER;
                }
            }
            for (var i = 0; i < UNSUPPORTED_MARKERS.Length; i++) {
