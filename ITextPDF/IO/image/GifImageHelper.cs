...
namespace  IText.IO.Image {
    public sealed class GifImageHelper {
        ...
        public static void ProcessImage(GifImageData image, int lastFrameNumber) {
            var gif = new GifParameters(image);
            Stream gifStream;
            try {
        ...
        private static void Process(Stream stream, GifParameters gif, int lastFrameNumber) {
            gif.input = stream;
            ReadHeader(gif);
        ...
        private static void ReadHeader(GifParameters gif) {
        ...
        private static int ReadShort(GifParameters gif) {
        ...
        private static int ReadBlock(GifParameters gif) {
        ...
        private static byte[] ReadColorTable(int bpc, GifParameters gif) {
            var ncolors = 1 << bpc;
        ...
        private static void ReadContents(GifParameters gif, int lastFrameNumber) {
            // read GIF file content blocks
            var done = false;
            gif.currentFrame = 0;
            while (!done) {
                var code = gif.input.Read();
                switch (code) {
                    case 0x2C: {
                        // image separator
                        ReadFrame(gif);
        ...
        private static void ReadFrame(GifParameters gif) {
            ...
            // decode pixel data
            var skipZero = DecodeImageData(gif);
            if (!skipZero) {
                Skip(gif);
            }
            try {
                ...
                var len = gif.m_curr_table.Length;
                ...
                var img = new RawImageData(gif.m_out, ImageType.GIF);
                ...
                if (gif.transparency) {
                    img.SetTransparency(new[] { gif.transIndex, gif.transIndex });
                }
            }
            catch (Exception e) {
                throw new IOException(IOException.GifImageException, e);
        ...
        private static bool DecodeImageData(GifParameters gif) {
            var NullCode = -1;
            var npix = gif.iw * gif.ih;
            int available;
            int clear;
            int code_mask;
            int code_size;
        ...
        private static void SetPixel(int x, int y, int v, GifParameters gif) {
            if (gif.m_bpc == 8) {
                ...
                var pos = gif.m_line_stride * y + x / (8 / gif.m_bpc);
                var vout = v << 8 - gif.m_bpc * (x % (8 / gif.m_bpc)) - gif.m_bpc;
                gif.m_out[pos] |= (byte)vout;
            }
        }

        /// <summary>Reads Graphics Control Extension values</summary>
        private static void ReadGraphicControlExt(GifParameters gif) {
            // block size
            gif.input.Read();
            // packed fields
            var packed = gif.input.Read();
        ...
        /// </summary>
        private static void Skip(GifParameters gif) {
            do {
                ReadBlock(gif);
            }
