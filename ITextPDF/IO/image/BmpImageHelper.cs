...
namespace  IText.IO.Image {
    internal sealed class BmpImageHelper {
        ...
        private static void Process(BmpParameters bmp, Stream stream) {
            ...
            var size = ReadDWord(bmp.inputStream);
            if (size == 12) {
            ...
            var planes = ReadWord(bmp.inputStream);
            ...
            if (size == 12) {
                ...
                // Read in the palette
                var numberOfEntries = (int)((bmp.bitmapOffset - 14 - size) / 3);
                ...
                var colorsUsed = ReadDWord(bmp.inputStream);
                var colorsImportant = ReadDWord(bmp.inputStream);
                ...
                if (size == 40 || size == 52 || size == 56) {
                    ...
                    switch ((int)bmp.compression) {
                        ...
                        case BI_RLE4: {
                            ...
                            // Read in the palette
                            var numberOfEntries = (int)((bmp.bitmapOffset - 14 - size) / 4);
                    ...
                    if (size == 108) {
                        ...
                        bmp.alphaMask = (int)ReadDWord(bmp.inputStream);
                        var csType = ReadDWord(bmp.inputStream);
                        var redX = ReadLong(bmp.inputStream);
                        var redY = ReadLong(bmp.inputStream);
                        var redZ = ReadLong(bmp.inputStream);
                        var greenX = ReadLong(bmp.inputStream);
                        var greenY = ReadLong(bmp.inputStream);
                        var greenZ = ReadLong(bmp.inputStream);
                        var blueX = ReadLong(bmp.inputStream);
                        var blueY = ReadLong(bmp.inputStream);
                        var blueZ = ReadLong(bmp.inputStream);
                        var gammaRed = ReadDWord(bmp.inputStream);
                        var gammaGreen = ReadDWord(bmp.inputStream);
                        var gammaBlue = ReadDWord(bmp.inputStream);
                        if (bmp.bitsPerPixel == 1) {
        ...
        private static byte[] GetPalette(int group, BmpParameters bmp) {
            ...
            var np = new byte[bmp.palette.Length / group * 3];
            var e = bmp.palette.Length / group;
            for (var k = 0; k < e; ++k) {
                var src = k * group;
                var dest = k * 3;
        ...
        private static bool GetImage(BmpParameters bmp) {
        ...
        private static void IndexedModel(byte[] bdata, int bpc, int paletteEntries, BmpParameters bmp
            ...
            var colorSpace = new object[4];
            colorSpace[0] = "/Indexed";
            colorSpace[1] = "/DeviceRGB";
            var np = GetPalette(paletteEntries, bmp);
            var len = np.Length;
        ...
        private static void ReadPalette(int sizeOfPalette, BmpParameters bmp) {
            if (sizeOfPalette == 0) {
                return;
            }
            ...
            while (bytesRead < sizeOfPalette) {
                var r = bmp.inputStream.JRead(bmp.palette, bytesRead, sizeOfPalette - bytesRead);
                if (r < 0) {
        ...
        private static void Read1Bit(int paletteEntries, BmpParameters bmp) {
            var bdata = new byte[(bmp.width + 7) / 8 * bmp.height];
            var padding = 0;
            var bytesPerScanline = (int)Math.Ceiling(bmp.width / 8.0d);
            var remainder = bytesPerScanline % 4;
            if (remainder != 0) {
                padding = 4 - remainder;
            }
            var imSize = (bytesPerScanline + padding) * bmp.height;
        ...
        private static void Read4Bit(int paletteEntries, BmpParameters bmp) {
            var bdata = new byte[(bmp.width + 1) / 2 * bmp.height];
            // Padding bytes at the end of each scanline
            var padding = 0;
            var bytesPerScanline = (int)Math.Ceiling(bmp.width / 2.0d);
            var remainder = bytesPerScanline % 4;
            if (remainder != 0) {
                padding = 4 - remainder;
            }
        ...
        private static void Read8Bit(int paletteEntries, BmpParameters bmp) {
            var bdata = new byte[bmp.width * bmp.height];
            // Padding bytes at the end of each scanline
            var padding = 0;
            // width * bitsPerPixel should be divisible by 32
            var bitsPerScanline = bmp.width * 8;
            if (bitsPerScanline % 32 != 0) {
                padding = (bitsPerScanline / 32 + 1) * 32 - bitsPerScanline;
                padding = (int)Math.Ceiling(padding / 8.0);
            }
            var imSize = (bmp.width + padding) * bmp.height;
            // Read till we have the whole image
            var values = new byte[imSize];
            var bytesRead = 0;
            while (bytesRead < imSize) {
        ...
        private static void Read24Bit(byte[] bdata, BmpParameters bmp) {
            // Padding bytes at the end of each scanline
            var padding = 0;
            // width * bitsPerPixel should be divisible by 32
            var bitsPerScanline = bmp.width * 24;
            if (bitsPerScanline % 32 != 0) {
                padding = (bitsPerScanline / 32 + 1) * 32 - bitsPerScanline;
                padding = (int)Math.Ceiling(padding / 8.0);
            }
            var imSize = (bmp.width * 3 + 3) / 4 * 4 * bmp.height;
            // Read till we have the whole image
            var values = new byte[imSize];
            var bytesRead = 0;
            while (bytesRead < imSize) {
                var r = bmp.inputStream.JRead(values, bytesRead, imSize - bytesRead);
                if (r < 0) {
                    break;
                }
                bytesRead += r;
            ...
            int count;
            if (bmp.isBottomUp) {
                var max = bmp.width * bmp.height * 3 - 1;
                count = -padding;
                for (var i = 0; i < bmp.height; i++) {
        ...
        private static void Read1632Bit(bool is32, BmpParameters bmp) {
            var red_mask = FindMask(bmp.redMask);
            var red_shift = FindShift(bmp.redMask);
            var red_factor = red_mask + 1;
            var green_mask = FindMask(bmp.greenMask);
            var green_shift = FindShift(bmp.greenMask);
            var green_factor = green_mask + 1;
            var blue_mask = FindMask(bmp.blueMask);
            var blue_shift = FindShift(bmp.blueMask);
            var blue_factor = blue_mask + 1;
            var bdata = new byte[bmp.width * bmp.height * 3];
            // Padding bytes at the end of each scanline
            var padding = 0;
            if (!is32) {
                // width * bitsPerPixel should be divisible by 32
                var bitsPerScanline = bmp.width * 16;
                if (bitsPerScanline % 32 != 0) {
                    padding = (bitsPerScanline / 32 + 1) * 32 - bitsPerScanline;
                    padding = (int)Math.Ceiling(padding / 8.0);
                }
            }
            var imSize = (int)bmp.imageSize;
            if (imSize == 0) {
        ...
        private static void ReadRLE8(BmpParameters bmp) {
            ...
            var val = DecodeRLE(true, values, bmp);
            ...
            if (bmp.isBottomUp) {
                ...
                var bytesPerScanline = bmp.width;
                for (var i = 0; i < bmp.height; i++) {
        ...
        private static void ReadRLE4(BmpParameters bmp) {
            ...
            var val = DecodeRLE(false, values, bmp);
            ...
            var stride = (bmp.width + 1) / 2;
            var bdata = new byte[stride * bmp.height];
            var ptr = 0;
            var sh = 0;
        ...
        private static byte[] DecodeRLE(bool is8, byte[] values, BmpParameters bmp) {
            var val = new byte[bmp.width * bmp.height];
            try {
                var ptr = 0;
                var x = 0;
                var q = 0;
                for (var y = 0; y < bmp.height && ptr < values.Length; ) {
