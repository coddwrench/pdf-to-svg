...
namespace  IText.IO.Image {
    internal sealed class Jpeg2000ImageHelper {
            ...
            try {
                ...
                if (box.length == 0x0000000c) {
                    ...
                    if (box.length == unchecked((int)(0xff4fff51))) {
                        ...
                        var x1 = Cio_read(4, jpeg2000Stream);
                        var y1 = Cio_read(4, jpeg2000Stream);
                        var x0 = Cio_read(4, jpeg2000Stream);
                        var y0 = Cio_read(4, jpeg2000Stream);
        ...
        private static Jpeg2000ImageData.ColorSpecBox Jp2_read_colr(Jpeg2000Box box, Stream jpeg2000Stream
            ) {
            var readBytes = 8;
            var colorSpecBox = new Jpeg2000ImageData.ColorSpecBox();
            for (var i = 0; i < 3; i++) {
            ...
            if (box.length - readBytes > 0) {
                var colorProfile = new byte[box.length - readBytes];
        ...
        private static void Jp2_read_boxhdr(Jpeg2000Box box, Stream jpeg2000Stream) {
            box.length = Cio_read(4, jpeg2000Stream);
            box.type = Cio_read(4, jpeg2000Stream);
            if (box.length == 1) {
