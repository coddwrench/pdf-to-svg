...
namespace  IText.IO.Font {
    ...
    public class CFFFontSubset : CFFFont {
        ...
        internal static readonly string[] SubrsFunctions = { "RESERVED_0", "hstem", "RESERVED_2", "vstem"
        ...
        internal static readonly string[] SubrsEscapeFuncs = { "RESERVED_0", "RESERVED_1", "RESERVED_2"
        ...
        internal LinkedList<Item> OutputList;
                    ...
                    switch (key) {
                        ...
                        case "callsubr": {
                            ...
                            if (NumOfArgs > 0) {
                                ...
                                var Subr = (int)((int?)TopElement) + LBias;
                        ...
                        case "callgsubr": {
                            ...
                            if (NumOfArgs > 0) {
                                // Calc the index of the Subrs
                                var Subr = (int)((int?)TopElement) + GBias;
                                // If the subr isn't in the Map -> Put in
        ...
        protected internal virtual int CalcHints(int begin, int end, int LBias, int GBias, int[] LSubrsOffsets) {
            ...
            while (GetPosition() < end) {
                ...
                switch (key) {
                    ...
                    case "callsubr": {
                        if (NumOfArgs > 0) {
                            Debug.Assert(TopElement is int?);
                            var Subr = (int)((int?)TopElement) + LBias;
                            CalcHints(LSubrsOffsets[Subr], LSubrsOffsets[Subr + 1], LBias, GBias, LSubrsOffsets);
                            Seek(pos);
                        }
        ...
        protected internal virtual byte[] AssembleIndex(int[] NewOffsets, byte[] NewObjects) {
            ...
            foreach (var newOffset in NewOffsets) {
                // The value to be written
                var Num = newOffset - NewOffsets[0] + 1;
        ...
        protected internal virtual byte[] BuildNewFile(int Font) {
            ...
            OutputList.AddLast(new RangeItem(new RandomAccessFileOrArray(rasFactory.CreateSource(NewGSubrsIndex
            ...
            OutputList.AddLast(new RangeItem(new RandomAccessFileOrArray(rasFactory.CreateSource(NewCharStringsIndex
        ...
        protected internal virtual void CreateKeys(OffsetItem fdarrayRef, OffsetItem fdselectRef, 
            OffsetItem charsetRef, OffsetItem charstringsRef) {
        ...
        protected internal virtual void CreateNewStringIndex(int Font) {
            ...
            foreach (var stringOffset in stringOffsets) {
                OutputList.AddLast(new IndexOffsetItem(stringsIndexOffSize, stringOffset - stringsBaseOffset));
            }
        ...
        protected internal virtual void CreateFDSelect(OffsetItem fdselectRef, int nglyphs) {
            OutputList.AddLast(new MarkerItem(fdselectRef));
            // format identifier
        ...
        /// <param name="charsetRef">OffsetItem for the CharSet</param>
        /// <param name="nglyphs">the number of glyphs in the font</param>
        protected internal virtual void CreateCharset(OffsetItem charsetRef, int nglyphs) {
            OutputList.AddLast(new MarkerItem(charsetRef));
            // format identifier
        ...
        /// <param name="Font">the font</param>
        protected internal virtual void CreateFDArray(OffsetItem fdarrayRef, OffsetItem privateRef
            , int Font) {
            OutputList.AddLast(new MarkerItem(fdarrayRef));
        ...
        internal virtual void ReconstructFDArray(int Font, OffsetItem[] fdPrivate) {
            ...
            for (var k = 0; k < fonts[Font].FDArrayOffsets.Length - 1; k++) {
                ...
                while (GetPosition() < fonts[Font].FDArrayOffsets[k + 1]) {
                    ...
                    if ("Private".Equals(key)) {
                        ...
                        var NewSize = (int)((int?)args[0]);
                        // Save the size of the offset to the subrs in that private
                        var OrgSubrsOffsetSize = CalcSubrOffsetSize(fonts[Font].fdprivateOffsets[k], fonts[Font].fdprivateLengths[
                            k]);
        ...
        internal virtual void ReconstructPrivateDict(int Font, OffsetItem[] fdPrivate, IndexBaseItem
            [] fdPrivateBase, OffsetItem[] fdSubrs) {
        ...
        internal virtual void ReconstructPrivateSubrs(int Font, IndexBaseItem[] fdPrivateBase, OffsetItem
            [] fdSubrs) {
            ...
            for (var i = 0; i < fonts[Font].fdprivateLengths.Length; i++) {
                ...
                if (fdSubrs[i] != null && fonts[Font].PrivateSubrsOffset[i] >= 0) {
                    ...
                    if (NewLSubrsIndex[i] != null) {
                        OutputList.AddLast(new RangeItem(new RandomAccessFileOrArray(rasFactory.CreateSource(NewLSubrsIndex
        ...
        internal virtual void CreateNonCIDPrivate(int Font, OffsetItem Subr) {
        ...
        internal virtual void CreateNonCIDSubrs(int Font, IndexBaseItem PrivateBase, OffsetItem Subrs
            ) {
            ...
            if (NewSubrsIndexNonCID != null) {
                OutputList.AddLast(new RangeItem(new RandomAccessFileOrArray(rasFactory.CreateSource(NewSubrsIndexNonCID
