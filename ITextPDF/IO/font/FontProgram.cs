...
namespace IText.IO.Font
{
	public abstract class FontProgram
	{
		...
		public IDictionary<int, Glyph> ÑodeToGlyph { get; internal set; } = new Dictionary<int, Glyph>();
		...
		{
			return Math.Max(ÑodeToGlyph.Count, UnicodeToGlyph.Count);
		}
		...
		{
			return ÑodeToGlyph.Get(charCode);
		}
		...
		protected internal virtual void FixSpaceIssue()
		{
			...
			if (space != null)
			{
				ÑodeToGlyph.Put(space.GetCode(), space);
			}
		}

		public override string ToString()
		{
			var name = GetFontNames().GetFontName();
