...
namespace  IText.IO.Font {
    ...
    public sealed class FontProgramFactory {
        ...
        private static FontProgram CreateFont(string name, byte[] fontProgram, bool cached) {
            var baseName = FontProgram.TrimFontStyle(name);
            //yes, we trying to find built-in standard font with original name, not baseName.
            var isBuiltinFonts14 = StandardFonts.IsStandardFont(name);
            var isCidFont = !isBuiltinFonts14 && FontCache.IsPredefinedCidFont(baseName);
            ...
            if (name == null) {
                ...
                var extensionBeginIndex = baseName.LastIndexOf('.');
                    ...
                    if (isCidFont) {
                        ...
                        if (".ttf".Equals(fontFileExtension) || ".otf".Equals(fontFileExtension)) {
                            ...
                            if (".woff".Equals(fontFileExtension) || ".woff2".Equals(fontFileExtension)) {
                                ...
                                var ttcSplit = baseName.ToLowerInvariant().IndexOf(".ttc,", StringComparison.Ordinal);
                                if (ttcSplit > 0) {
                                    try {
                                        // count(.ttc) = 4
                                        var ttcName = baseName.JSubstring(0, ttcSplit + 4);
                                        // count(.ttc,) = 5)
                                        var ttcIndex = Convert.ToInt32(baseName.Substring(ttcSplit + 5), CultureInfo.InvariantCulture
                                            );
                                        fontBuilt = new TrueTypeFont(ttcName, ttcIndex);
                                    }
                                    catch (FormatException nfe) {
                                        throw new IOException(nfe.Message, nfe);
                                    }
                                }
                            }
                        }
        ...
        public static FontProgram CreateFont(string ttc, int ttcIndex, bool cached) {
            var fontCacheKey = FontCacheKey.Create(ttc, ttcIndex);
            if (cached) {
                var fontFound = FontCache.GetFont(fontCacheKey);
                if (fontFound != null) {
                    return fontFound;
                }
            }
        ...
        public static FontProgram CreateFont(byte[] ttc, int ttcIndex, bool cached) {
            var fontKey = FontCacheKey.Create(ttc, ttcIndex);
            if (cached) {
                var fontFound = FontCache.GetFont(fontKey);
                if (fontFound != null) {
                    return fontFound;
                }
            }
        ...
        /// <see cref="FontStyles"/>.
        /// See
        /// <see cref="FontStyles.BOLD"/>
        /// ,
        /// <see cref="FontStyles.ITALIC"/>
        /// ,
        /// <see cref="FontStyles.NORMAL"/>
        /// ,
        /// <see cref="FontStyles.BOLDITALIC"/>
        /// ,
        /// <see cref="FontStyles.UNDEFINED"/>
        /// </param>
        ...
        internal static byte[] ReadFontBytesFromPath(string path) {
            var raf = new RandomAccessFileOrArray(new RandomAccessSourceFactory().CreateBestSource
                (path));
            var bufLen = (int)raf.Length();
            if (bufLen < raf.Length()) {
                throw new IOException(MessageFormatUtil.Format("Source data from \"{0}\" is bigger than byte array can hold."
                    , path));
            }
