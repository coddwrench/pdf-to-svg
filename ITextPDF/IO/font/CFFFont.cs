...
namespace IText.IO.Font {
    public class CFFFont {
        internal static readonly string[] operatorNames = { "version", "Notice", "FullName", "FamilyName"
        ...
        protected internal int arg_count;
        ...
        protected internal sealed class RangeItem : Item {
            public int offset;
        ...
        /// </remarks>
        protected internal sealed class IndexOffsetItem : OffsetItem {
        ...
        protected internal sealed class IndexBaseItem : Item {
        }

        protected internal sealed class IndexMarkerItem : Item {
        ...
        /// </summary>
        protected internal sealed class SubrMarkerItem : Item {
            private OffsetItem offItem;
        ...
        /// </remarks>
        protected internal sealed class DictOffsetItem : OffsetItem {
            public readonly int size;
        ...
        protected internal sealed class UInt24Item : Item {
            public int value;
        ...
        protected internal sealed class UInt32Item : Item {
            public int value;
        ...
        protected internal sealed class UInt16Item : Item {
            public char value;
        ...
        protected internal sealed class UInt8Item : Item {
            public char value;
        ...
        protected internal sealed class StringItem : Item {
            public string s;
        ...
        /// representation.
        /// </remarks>
        protected internal sealed class DictNumberItem : Item {
        ...
        /// </remarks>
        protected internal sealed class MarkerItem : Item {
            internal OffsetItem p;
        ...
        protected internal virtual RangeItem GetEntireIndexRange(int indexOffset) {
            Seek(indexOffset);
            int count = GetCard16();
            if (count == 0) {
                ...
                while (GetPosition() < topdictOffsets[j + 1]) {
                    ...
                    if (key == "FullName") {
                        ...
                        if (key == "ROS") {
                            ...
                            if (key == "Private") {
                                fonts[j].privateLength = ((int?)args[0]).Value;
                                fonts[j].privateOffset = ((int?)args[1]).Value;
                            }
                            else {
                                if (key == "charset") {
                                    fonts[j].charsetOffset = ((int?)args[0]).Value;
                                }
                                else {
                                    ...
                                    //                }
                                    if (key == "CharStrings") {
                                        fonts[j].charstringsOffset = ((int?)args[0]).Value;
                                        ...
                                        if (key == "FDArray") {
                                            fonts[j].fdarrayOffset = ((int?)args[0]).Value;
                                        }
                                        else {
                                            if (key == "FDSelect") {
                                                fonts[j].fdselectOffset = ((int?)args[0]).Value;
                                            }
                                            else {
                                                if (key == "CharstringType") {
                                                    fonts[j].CharstringType = ((int?)args[0]).Value;
                                                }
                                            }
                                        }
                                    }
                ...
                if (fonts[j].privateOffset >= 0) {
                    ...
                    while (GetPosition() < fonts[j].privateOffset + fonts[j].privateLength) {
                        ...
                        if (key == "Subrs") {
                            ...
                            fonts[j].privateSubrs = ((int?)args[0]).Value + fonts[j].privateOffset;
                        }
                    }
                }
                // fdarray index
                if (fonts[j].fdarrayOffset >= 0) {
                    ...
                    for (var k = 0; k < fdarrayOffsets.Length - 1; k++) {
                        ...
                        while (GetPosition() < fdarrayOffsets[k + 1]) {
                            GetDictItem();
                            if (key == "Private") {
                                fonts[j].fdprivateLengths[k] = ((int?)args[0]).Value;
                                fonts[j].fdprivateOffsets[k] = ((int?)args[1]).Value;
                            }
                        }
                    }
                }
