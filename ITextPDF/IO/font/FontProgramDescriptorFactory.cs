...
namespace  IText.IO.Font {
    public sealed class FontProgramDescriptorFactory {
        ...
        public static FontProgramDescriptor FetchDescriptor(string fontName) {
            ...
            var baseName = FontProgram.TrimFontStyle(fontName);
            //yes, we trying to find built-in standard font with original name, not baseName.
            var isBuiltinFonts14 = StandardFonts.IsStandardFont(fontName);
            ...
            try {
                var fontNameLowerCase = baseName.ToLowerInvariant();
        ...
        private static FontProgramDescriptor FetchTTCDescriptor(string baseName) {
            ...
            if (ttcSplit > 0) {
                ...
                try {
                    ...
                    // count(.ttc,) = 5)
                    ttcIndex = Convert.ToInt32(baseName.Substring(ttcSplit + 5), CultureInfo.InvariantCulture
                        );
                }
                catch (FormatException nfe) {
                    throw new IOException(nfe.Message, nfe);
                }
                var parser = new OpenTypeParser(ttcName, ttcIndex);
                var descriptor = FetchOpenTypeFontDescriptor(parser);
                parser.Close();
                return descriptor;
            }
        ...
        private static FontProgramDescriptor FetchTrueTypeFontDescriptor(string fontName) {
            using (var parser = new OpenTypeParser(fontName)) {
        ...
        private static FontProgramDescriptor FetchTrueTypeFontDescriptor(byte[] fontProgram) {
            using (var parser = new OpenTypeParser(fontProgram)) {
                return FetchOpenTypeFontDescriptor(parser);
            }
        }
        ...
        private static FontProgramDescriptor FetchType1FontDescriptor(string fontName, byte[] afm) {
            ...
            var fp = new Type1Font(fontName, null, afm, null);
        ...
        private static FontProgramDescriptor FetchCidFontDescriptor(string fontName) {
            var font = new CidFont(fontName, null);
