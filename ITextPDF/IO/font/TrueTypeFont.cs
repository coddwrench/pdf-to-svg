...
namespace  IText.IO.Font {
    public class TrueTypeFont : FontProgram {
        ...
        protected internal virtual void ReadGsubTable() {
            ...
            if (gsub != null) {
                gsubTable = new GlyphSubstitutionTableReader(fontParser.raf, gsub[0], gdefTable, СodeToGlyph, fontMetrics.
                    GetUnitsPerEm());
            }
        }
        ...
        protected internal virtual void ReadGposTable() {
            ...
            if (gpos != null) {
                gposTable = new GlyphPositioningTableReader(fontParser.raf, gpos[0], gdefTable, СodeToGlyph, fontMetrics.GetUnitsPerEm
                    ());
            }
        }
        ...
        private void InitializeFontProperties() {
            ...
            СodeToGlyph = new LinkedDictionary<int, Glyph>(numOfGlyphs);
            avgWidth = 0;
            foreach (var charCode in cmap.Keys) {
                ...
                if (!СodeToGlyph.ContainsKey(index)) {
                    СodeToGlyph.Put(index, glyph);
                }
            ...
            if (СodeToGlyph.Count != 0) {
                avgWidth /= СodeToGlyph.Count;
            }
            ReadGdefTable();
            ReadGsubTable();
            ReadGposTable();
