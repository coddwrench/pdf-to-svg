...
namespace  IText.IO.Font.Otf {
    ...
    public abstract class OpenTypeFontTableReader {
        ...
        public virtual IList<FeatureRecord> GetFeatures(string[] scripts, string language) {
            var rec = scriptsType.GetLanguageRecord(scripts, language);
            if (rec == null) {
                return null;
            }
        ...
        public virtual IList<OpenTableLookup> GetLookups(FeatureRecord[] features) {
            var hash = new IntHashtable();
            foreach (var rec in features) {
                foreach (var idx in rec.lookups) {
                    hash.Put(idx, 1);
                }
            }
            IList<OpenTableLookup> ret = new List<OpenTableLookup>();
            foreach (var idx in hash.ToOrderedKeys()) {
        ...
        public virtual IList<OpenTableLookup> GetLookups(FeatureRecord feature) {
            ...
            foreach (var idx in feature.lookups) {
                ret.Add(lookupList[idx]);
            }
            return ret;
        }
        ...
        public virtual LanguageRecord GetLanguageRecord(string otfScriptTag, string langTag) {
            ...
            foreach (var record in GetScriptRecords()) {
                ...
                foreach (var lang in record.languages) {
        ...
        protected internal virtual TagAndLocation[] ReadTagAndLocations(int baseLocation) {
            ...
            var tagslLocs = new TagAndLocation[count];
            for (var k = 0; k < count; ++k) {
                var tl = new TagAndLocation();
        ...
        internal void StartReadingTable() {
            try {
                ...
                // version not used
                rf.ReadInt();
                var scriptListOffset = rf.ReadUnsignedShort();
                var featureListOffset = rf.ReadUnsignedShort();
                var lookupListOffset = rf.ReadUnsignedShort();
        ...
        private void ReadLookupListTable(int lookupListTableLocation) {
            ...
            var lookupCount = rf.ReadUnsignedShort();
            var lookupTableLocations = ReadUShortArray(lookupCount, lookupListTableLocation);
            // read LookUp tables
            foreach (var lookupLocation in lookupTableLocations) {
        ...
        private void ReadLookupTable(int lookupTableLocation) {
            ...
            var lookupType = rf.ReadUnsignedShort();
            var lookupFlag = rf.ReadUnsignedShort();
            var subTableCount = rf.ReadUnsignedShort();
            var subTableLocations = ReadUShortArray(subTableCount, lookupTableLocation);
