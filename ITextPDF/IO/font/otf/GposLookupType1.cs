...
namespace  IText.IO.Font.Otf {
    ...
    public class GposLookupType1 : OpenTableLookup {
        ...
        public override bool TransformOne(GlyphLine line) {
            ...
            var glyphCode = line.Get(line.idx).GetCode();
            var positionApplied = false;
            var valueRecord = valueRecordMap.Get(glyphCode);
            if (valueRecord != null) {
                var newGlyph = new Glyph(line.Get(line.idx));
        ...
        protected internal override void ReadSubTable(int subTableLocation) {
            ...
            var valueFormat = openReader.rf.ReadUnsignedShort();
            if (subTableFormat == 1) {
                var valueRecord = OtfReadCommon.ReadGposValueRecord(openReader, valueFormat);
                ...
                if (subTableFormat == 2) {
                    var valueCount = openReader.rf.ReadUnsignedShort();
                    ...
                    var coverageGlyphIds = openReader.ReadCoverageFormat(subTableLocation + coverageOffset);
                    for (var i = 0; i < coverageGlyphIds.Count; i++) {
                        valueRecordMap.Put(coverageGlyphIds[i], valueRecords[i]);
                    }
                }
                else {
