...
namespace  IText.IO.Font.Otf.Lookuptype5 {
    ...
    public class SubTableLookup5Format3 : ContextualSubTable {
        ...
        public SubTableLookup5Format3(OpenTypeFontTableReader openReader, int lookupFlag, SubstRuleFormat3
             rule)
        ...
        protected internal override IList<ContextualSubstRule> GetSetOfRulesForStartGlyph(int startId) {
            var ruleFormat3 = (SubstRuleFormat3)substitutionRule;
            if (ruleFormat3.coverages[0].Contains(startId) && !openReader.IsSkip(startId, lookupFlag)) {
                return JavaCollectionsUtil.SingletonList(substitutionRule);
            }
