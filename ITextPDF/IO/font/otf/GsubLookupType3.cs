    ...
    public class GsubLookupType3 : OpenTableLookup {
        ...
        public override bool TransformOne(GlyphLine line) {
            ...
            if (!openReader.IsSkip(g.GetCode(), lookupFlag)) {
                var substCode = substMap.Get(g.GetCode());
        ...
        protected internal override void ReadSubTable(int subTableLocation) {
            ...
            Debug.Assert(substFormat == 1);
            var coverage = openReader.rf.ReadUnsignedShort();
            var alternateSetCount = openReader.rf.ReadUnsignedShort();
            var substitute = new int[alternateSetCount][];
            var alternateLocations = openReader.ReadUShortArray(alternateSetCount, subTableLocation);
            for (var k = 0; k < alternateSetCount; k++) {
            ...
            var coverageGlyphIds = openReader.ReadCoverageFormat(subTableLocation + coverage);
            for (var k = 0; k < alternateSetCount; ++k) {
