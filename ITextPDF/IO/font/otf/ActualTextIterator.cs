	...
	public class ActualTextIterator : IEnumerator<GlyphLine.GlyphLinePart>
	{
	    ...
	    public GlyphLine.GlyphLinePart Next()
	    {
		    if (glyphLine.actualText == null) {
                var result = new GlyphLine.GlyphLinePart(pos, glyphLine.end,
                    null);
                pos = glyphLine.end;
                return result;
            }

		    var currentResult = NextGlyphLinePart(pos);
		    ...
		    if (!GlyphLinePartNeedsActualText(currentResult)) {
			    ...
			    while (pos < glyphLine.end) {
				    var nextResult = NextGlyphLinePart(pos);
		...
		private GlyphLine.GlyphLinePart NextGlyphLinePart(int pos)
		{
			...
			var startPos = pos;
			var startActualText = glyphLine.actualText[pos];
		...
		private bool GlyphLinePartNeedsActualText(GlyphLine.GlyphLinePart glyphLinePart)
		{
			...
			var toUnicodeMapResult = new StringBuilder();
			for (var i = glyphLinePart.start; i < glyphLinePart.end; i++)
			{
				var currentGlyph = glyphLine.glyphs[i];
