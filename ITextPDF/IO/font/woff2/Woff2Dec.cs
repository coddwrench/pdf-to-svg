...
namespace  IText.IO.Font.Woff2 {
    ...
    internal class Woff2Dec {
        ...
        private class RebuildMetadata {
            ...
            // set by writeHeaders
            internal Woff2FontInfo[] font_infos;

            // checksums for tables that have been written.
            // (tag, src_offset) => checksum. Need both because 0-length loca.
            internal IDictionary<TableChecksumInfo, int?> checksums = new Dictionary<TableChecksumInfo
                , int?>();
        }
        ...
        private class TableChecksumInfo {
            ...
            public override bool Equals(object o) {
                ...
                if (o is TableChecksumInfo) {
                    var info = (TableChecksumInfo)o;
        ...
        private static int TripletDecode(byte[] data, int flags_in_offset, int in_offset, int in_size, int n_points
            ...
            for (var i = 0; i < n_points; ++i) {
                var flag = JavaUnsignedUtil.AsU8(data[i + flags_in_offset]);
                var on_curve = (flag >> 7) == 0;
                flag &= 0x7f;
                int n_data_bytes;
                if (flag < 84) {
                    n_data_bytes = 1;
                ...
                if (flag < 10) {
                    ...
                    if (flag < 20) {
                        ...
                        if (flag < 84) {
                            var b0 = flag - 20;
                            var b1 = JavaUnsignedUtil.AsU8(data[in_offset + triplet_index]);
                            ...
                            if (flag < 120) {
                                ...
                                if (flag < 124) {
                                    var b2 = JavaUnsignedUtil.AsU8(data[in_offset + triplet_index + 1]);
        ...
        private static int StorePoints(int n_points, Woff2Common.Point[] points, int n_contours, int instruction_length
            ...
            var flag_offset = kEndPtsOfContoursOffset + 2 * n_contours + 2 + instruction_length;
            var last_flag = -1;
            var repeat_count = 0;
            var last_x = 0;
            var last_y = 0;
            ...
            for (var i = 0; i < n_points; ++i) {
                var point = points[i];
                var flag = point.on_curve ? kGlyfOnCurve : 0;
                var dx = point.x - last_x;
                var dy = point.y - last_y;
                if (dx == 0) {
            ...
            var xy_bytes = x_bytes + y_bytes;
            if (xy_bytes < x_bytes || flag_offset + xy_bytes < flag_offset || flag_offset + xy_bytes > dst_size) {
                throw new FontCompressionException(FontCompressionException.RECONSTRUCT_POINT_FAILED);
            }
            var x_offset = flag_offset;
            var y_offset = flag_offset + x_bytes;
            last_x = 0;
            last_y = 0;
            for (var i = 0; i < n_points; ++i) {
                var dx = points[i].x - last_x;
                if (dx == 0) {
                }
                else {
                    // pass
        ...
        private static CompositeGlyphInfo SizeOfComposite(Buffer composite_stream) {
            ...
            var start_offset = composite_stream.GetOffset();
            var we_have_instructions = false;
            var flags = FLAG_MORE_COMPONENTS;
            ...
            var size = composite_stream.GetOffset() - start_offset;
            var have_instructions = we_have_instructions;
        ...
        private static int StoreLoca(int[] loca_values, int index_format, Woff2Out @out) {
            ...
            var loca_content = new byte[(int)(loca_size * offset_size)];
            var offset = 0;
            for (var i = 0; i < loca_values.Length; ++i) {
                var value = loca_values[i];
                if (index_format != 0) {
            ...
            var checksum = Woff2Common.ComputeULongSum(loca_content, 0, loca_content.Length);
            @out.Write(loca_content, 0, loca_content.Length);
            return checksum;
        }

        // Reconstruct entire glyf table based on transformed original
        private static Checksums ReconstructGlyf(byte[] data, int data_offset, Woff2Common.Table glyf_table
            , int glyph_checksum, Woff2Common.Table loca_table, int loca_checksum, Woff2FontInfo info, Woff2Out
             @out) {
            var kNumSubStreams = 7;
            var file = new Buffer(data, data_offset, glyf_table.transform_length);
            int version;
            var substreams = new List<StreamInfo>(kNumSubStreams);
            var glyf_start = @out.Size();
            // TODO: check version on 0?
            version = file.ReadInt();
            info.num_glyphs = file.ReadShort();
            info.index_format = file.ReadShort();
            var offset = (2 + kNumSubStreams) * 4;
            ...
            for (var i = 0; i < kNumSubStreams; ++i) {
                int substream_size;
            ...
            var n_contour_stream = new Buffer(data, substreams[0].offset, substreams[0].length);
            var n_points_stream = new Buffer(data, substreams[1].offset, substreams[1].length);
            var flag_stream = new Buffer(data, substreams[2].offset, substreams[2].length);
            var glyph_stream = new Buffer(data, substreams[3].offset, substreams[3].length);
            var composite_stream = new Buffer(data, substreams[4].offset, substreams[4].length);
            var bbox_stream = new Buffer(data, substreams[5].offset, substreams[5].length);
            var instruction_stream = new Buffer(data, substreams[6].offset, substreams[6].length);
            var loca_values = new int[JavaUnsignedUtil.AsU16(info.num_glyphs) + 1];
            var n_points_vec = new List<int>();
            var points = new Woff2Common.Point[0];
            var points_size = 0;
            var bbox_bitmap_offset = bbox_stream.GetInitialOffset();
            // Safe because num_glyphs is bounded
            var bitmap_length = ((JavaUnsignedUtil.AsU16(info.num_glyphs) + 31) >> 5) << 2;
            bbox_stream.Skip(bitmap_length);
            // Temp buffer for glyph's.
            var glyph_buf_size = kDefaultGlyphBuf;
            var glyph_buf = new byte[glyph_buf_size];
            info.x_mins = new short[JavaUnsignedUtil.AsU16(info.num_glyphs)];
            for (var i = 0; i < JavaUnsignedUtil.AsU16(info.num_glyphs); ++i) {
                var glyph_size = 0;
                var n_contours = 0;
                var have_bbox = false;
                var bitmap = new byte[bitmap_length];
                ...
                if (n_contours == 0xffff) {
                    ...
                    var instruction_size = 0;
                    if (!have_bbox) {
                        // composite glyphs must have an explicit bbox
                        throw new FontCompressionException(FontCompressionException.RECONSTRUCT_GLYF_TABLE_FAILED);
                    }
                    int composite_size;
                    var compositeGlyphInfo = SizeOfComposite(composite_stream);
                    ...
                    var size_needed = 12 + composite_size + instruction_size;
                    ...
                    if (n_contours > 0) {
                        ...
                        var flag_size = total_n_points;
                        if (flag_size > flag_stream.GetLength() - flag_stream.GetOffset()) {
                            throw new FontCompressionException(FontCompressionException.RECONSTRUCT_GLYF_TABLE_FAILED);
                        }
                        var flags_buf_offset = flag_stream.GetInitialOffset() + flag_stream.GetOffset();
                        var triplet_buf_offset = glyph_stream.GetInitialOffset() + glyph_stream.GetOffset();
                        var triplet_size = glyph_stream.GetLength() - glyph_stream.GetOffset();
                        var triplet_bytes_consumed = 0;
                        ...
                        var size_needed = 12 + 2 * n_contours + 5 * total_n_points + instruction_size;
                        if (glyph_buf_size < size_needed) {
                            glyph_buf = new byte[size_needed];
                            glyph_buf_size = size_needed;
                        }
                        ...
                        var end_point = -1;
                        for (var contour_ix = 0; contour_ix < n_contours; ++contour_ix) {
                ...
                if (n_contours > 0) {
                    var x_min_buf = new Buffer(glyph_buf, 2, 2);
        ...
        private static short ReadNumHMetrics(byte[] data, int offset, int data_length) {
            ...
            var buffer = new Buffer(data, offset, data_length);
            buffer.Skip(34);
            var result = buffer.ReadShort();
            return result;
        }
        ...
        private static int ReconstructTransformedHmtx(byte[] transformed_buf, int transformed_offset, int transformed_size
            ...
            //uint16
            //uint16
            var hmtx_buff_in = new Buffer(transformed_buf, transformed_offset, transformed_size);
            var hmtx_flags = JavaUnsignedUtil.AsU8(hmtx_buff_in.ReadByte());
            short[] advance_widths;
            short[] lsbs;
            var has_proportional_lsbs = (hmtx_flags & 1) == 0;
            var has_monospace_lsbs = (hmtx_flags & 2) == 0;
            ...
            var hmtx_output_size = 2 * num_glyphs + 2 * num_hmetrics;
            var hmtx_table = new byte[hmtx_output_size];
            var dst_offset = 0;
            ...
            var checksum = Woff2Common.ComputeULongSum(hmtx_table, 0, hmtx_output_size);
        ...
        private static void Woff2Uncompress(byte[] dst_buf, int dst_offset, int dst_length, byte[] src_buf, int src_offset
            , int src_length) {
            var remain = dst_length;
            try {
                var stream = new BrotliInputStream(new MemoryStream(src_buf, src_offset, src_length));
                while (remain > 0) {
                    var read = stream.JRead(dst_buf, dst_offset, dst_length);
                    if (read < 0) {
        ...
        private static void ReadTableDirectory(Buffer file, Woff2Common.Table[] tables, int num_tables) {
            ...
            for (var i = 0; i < num_tables; ++i) {
                var table = new Woff2Common.Table();
                tables[i] = table;
                var flag_byte = JavaUnsignedUtil.AsU8(file.ReadByte());
                int tag;
                ...
                var flags = 0;
                var xform_version = ((flag_byte >> 6) & 0x03);
                ...
                flags |= xform_version;
                var dst_length = VariableLength.ReadBase128(file);
        ...
        private static int StoreOffsetTable(byte[] result, int offset, int flavor, int num_tables) {
            ...
            var output_search_range = (1 << max_pow2) << 4;
        ...
        private static long ComputeOffsetToFirstTable(Woff2Header hdr) {
            ...
            if (hdr.header_version != 0) {
                ...
                foreach (var ttc_font in hdr.ttc_fonts) {
        ...
        private static List<Woff2Common.Table> Tables(Woff2Header hdr, int font_index) {
            var tables = new List<Woff2Common.Table>();
            if (hdr.header_version != 0) {
                foreach (var index in hdr.ttc_fonts[font_index].table_indices) {
                    tables.Add(hdr.tables[JavaUnsignedUtil.AsU16(index)]);
                }
            }
            else {
                foreach (var table in hdr.tables) {
                    tables.Add(table);
                }
            }
            return tables;
        ...
        private static void ReconstructFont(byte[] transformed_buf, int transformed_buf_offset, int transformed_buf_size
            , RebuildMetadata metadata, Woff2Header hdr, int font_index, Woff2Out @out) {
            var dest_offset = @out.Size();
            var table_entry = new byte[12];
            var info = metadata.font_infos[font_index];
            var tables = Tables(hdr, font_index);
            ...
            var font_checksum = metadata.header_checksum;
            if (hdr.header_version != 0) {
            ...
            for (var i = 0; i < tables.Count; i++) {
                var table = tables[i];
                var checksum_key = new TableChecksumInfo(table.tag, table.src_offset);
                var reused = metadata.checksums.ContainsKey(checksum_key);
                ...
                if (!reused) {
                    if ((table.flags & Woff2Common.kWoff2FlagsTransform) != Woff2Common.kWoff2FlagsTransform) {
                        ...
                        if (table.tag == TableTags.kGlyfTableTag) {
                            table.dst_offset = dest_offset;
                            var loca_table = FindTable(tables, TableTags.kLocaTableTag);
                            var resultChecksum = ReconstructGlyf(transformed_buf, transformed_buf_offset + table.src_offset
            ...
            var head_table = FindTable(tables, TableTags.kHeadTableTag);
            if (head_table != null) {
                if (head_table.dst_length < 12) {
                    throw new FontCompressionException(FontCompressionException.RECONSTRUCT_TABLE_DIRECTORY_FAILED);
                }
                var checksum_adjustment = new byte[4];
        ...
        private static void ReadWoff2Header(byte[] data, int length, Woff2Header hdr) {
            var file = new Buffer(data, 0, length);
            int signature;
            ...
            var reported_length = file.ReadInt();
            Debug.Assert(reported_length > 0);
            ...
            Debug.Assert(hdr.compressed_length >= 0);
            // We don't care about these fields of the header:
            ...
            Debug.Assert(meta_offset >= 0);
            meta_length = file.ReadInt();
            Debug.Assert(meta_length >= 0);
            meta_length_orig = file.ReadInt();
            Debug.Assert(meta_length_orig >= 0);
            if (meta_offset != 0) {
            ...
            Debug.Assert(priv_offset >= 0);
            priv_length = file.ReadInt();
            Debug.Assert(priv_length >= 0);
            if (priv_offset != 0) {
            ...
            var last_table = hdr.tables[hdr.tables.Length - 1];
            hdr.uncompressed_size = last_table.src_offset + last_table.src_length;
            Debug.Assert(hdr.uncompressed_size > 0);
            ...
            if (hdr.flavor == Woff2Common.kTtcFontFlavor) {
                ...
                for (var i = 0; i < num_fonts; i++) {
                    ...
                    for (var j = 0; j < num_tables; j++) {
                        ...
                        var table = hdr.tables[table_idx];
                        if (table.tag == TableTags.kLocaTableTag) {
                            loca_table = table;
                        }
                        if (table.tag == TableTags.kGlyfTableTag) {
            ...
            var first_table_offset = ComputeOffsetToFirstTable(hdr);
            hdr.compressed_offset = file.GetOffset();
            //TODO literary can't happen
            if (hdr.compressed_offset > int.MaxValue) {
                throw new FontCompressionException(FontCompressionException.READ_HEADER_FAILED);
            }
            var src_offset = Round.Round4(hdr.compressed_offset + hdr.compressed_length);
            var dst_offset = first_table_offset;
        ...
        private static void WriteHeaders(byte[] data, int length, RebuildMetadata metadata, Woff2Header
             hdr, Woff2Out @out) {
            var firstTableOffset = ComputeOffsetToFirstTable(hdr);
            Debug.Assert(firstTableOffset <= int.MaxValue);
            var output = new byte[(int)firstTableOffset];
            ...
            if (hdr.header_version != 0) {
                ...
                foreach (var ttc_font in hdr.ttc_fonts) {
                    ...
                    foreach (var table_index in ttc_font.table_indices) {
                        sorted_index_by_tag.Put(hdr.tables[table_index].tag, table_index);
                    }
                    short index = 0;
                    foreach (var i in sorted_index_by_tag) {
            ...
            if (hdr.header_version != 0) {
                ...
                var offset_table = offset;
                // keep start of offset table for later
                for (var i = 0; i < hdr.ttc_fonts.Length; ++i) {
                    offset = StoreBytes.StoreU32(result, offset, 0);
                }
                ...
                for (var i = 0; i < hdr.ttc_fonts.Length; ++i) {
                    ...
                    foreach (var table_index in ttc_font.table_indices) {
                        var tag = hdr.tables[table_index].tag;
                ...
                for (var i = 0; i < hdr.num_tables; ++i) {
        ...
        public static void ConvertWoff2ToTtf(byte[] data, int length, Woff2Out @out) {
            ...
            var hdr = new Woff2Header();
            ReadWoff2Header(data, length, hdr);
            WriteHeaders(data, length, metadata, hdr, @out);
            var compression_ratio = (float)hdr.uncompressed_size / length;
