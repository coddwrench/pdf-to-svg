...
namespace  IText.IO.Font {
    internal class WoffConverter {
        ...
        public static byte[] Convert(byte[] woffBytes) {
            ...
            srcPos += 2;
            var totalSfntSize = BytesToUInt(woffBytes, srcPos);
            srcPos += 4;
            // majorVersion
            srcPos += 2;
            // minorVersion
            ...
            var otfBytes = new byte[(int)totalSfntSize];
            ...
            var entrySelector = -1;
            var searchRange = -1;
            var numTablesVal = BytesToUShort(numTables, 0);
            for (var i = 0; i < 17; ++i) {
                var powOfTwo = (int)Math.Pow(2, i);
            ...
            destPos += 2;
            var rangeShift = numTablesVal * 16 - searchRange;
            otfBytes[destPos] = (byte)(rangeShift >> 8);
            otfBytes[destPos + 1] = (byte)(rangeShift);
            destPos += 2;
            var outTableOffset = destPos;
            IList<TableDirectory> tdList = new List<TableDirectory>(numTablesVal);
            for (var i = 0; i < numTablesVal; ++i) {
            ...
            foreach (var td in tdList) {
                var compressedData = new byte[(int)td.compLength];
                byte[] uncompressedData;
                Array.Copy(woffBytes, (int)td.offset, compressedData, 0, (int)td.compLength);
                var expectedUncompressedLen = (int)td.origLengthVal;
                ...
                if (td.compLength != td.origLengthVal) {
                    ...
                    while (expectedUncompressedLen - bytesRead > 0) {
                        var readRes = zip.JRead(uncompressedData, bytesRead, expectedUncompressedLen - bytesRead);
                        if (readRes < 0) {
                            throw new ArgumentException();
                        }
                        bytesRead += readRes;
