...
namespace  IText.IO.Font.Cmap {
    ...
    public class CMapByteCid : AbstractCMap {
        ...
        protected internal virtual int DecodeSingle(byte[] cidBytes, Cursor cursor) {
            var end = cursor.offset + cursor.length;
            var currentPlane = 0;
            while (cursor.offset < end) {
                var one = cidBytes[cursor.offset++] & 0xff;
                cursor.length--;
        ...
        private void EncodeSequence(byte[] seq, int cid) {
            ...
            for (var idx = 0; idx < size; ++idx) {
                var plane = planes[nextPlane];
                var one = seq[idx] & 0xff;
                var c = plane[one];
                if (c != 0 && (c & 0x8000) == 0) {
            ...
            var plane_1 = planes[nextPlane];
            var one_1 = seq[size] & 0xff;
            var c_1 = plane_1[one_1];
            if ((c_1 & 0x8000) != 0) {
                throw new IOException("Inconsistent mapping.");
            }
            plane_1[one_1] = cid;
