...
namespace  IText.IO.Font.Cmap {
    public class CMapParser {
        ...
        private static void ParseCid(string cmapName, AbstractCMap cmap, ICMapLocation location, int level) {
            ...
            var inp = location.GetLocation(cmapName);
            try {
                IList<CMapObject> list = new List<CMapObject>();
                var cp = new CMapContentParser(inp);
                var maxExc = 50;
                while (true) {
                    try {
                        cp.Parse(list);
                    ...
                    var last = list[list.Count - 1].ToString();
                    if (level == 0 && list.Count == 3 && last.Equals(def)) {
                        var cmapObject = list[0];
                        ...
                        if ((last.Equals(endcidchar) || last.Equals(endbfchar)) && list.Count >= 3) {
                            var lMax = list.Count - 2;
                            for (var k = 0; k < lMax; k += 2) {
                                if (list[k].IsString()) {
                            ...
                            if ((last.Equals(endcidrange) || last.Equals(endbfrange)) && list.Count >= 4) {
                                var lMax = list.Count - 3;
                                for (var k = 0; k < lMax; k += 3) {
                                    ...
                                    if (last.Equals(endcodespacerange)) {
                                        for (var i = 0; i < list.Count + 1; i += 2) {
                                            if (list[i].IsHexString() && list[i + 1].IsHexString()) {
                                                var low = list[i].ToHexByteArray();
                                                var high = list[i + 1].ToHexByteArray();
                ...
                var logger = LogManager.GetLogger(typeof(CMapParser));
