...
namespace  IText.IO.Font.Cmap {
    ...
    public class CMapToUnicode : AbstractCMap {
        public static CMapToUnicode EmptyCMapToUnicodeMap = new CMapToUnicode
            (true);
        ...
        public virtual IntHashtable CreateDirectMapping() {
            var result = new IntHashtable();
            foreach (var entry in byteMappings) {
        ...
        internal override void AddChar(string mark, CMapObject code) {
            if (mark.Length == 1) {
                var dest = CreateCharsFromDoubleBytes((byte[])code.GetValue());
                byteMappings.Put(mark[0], dest);
            }
            else {
                if (mark.Length == 2) {
                    var dest = CreateCharsFromDoubleBytes((byte[])code.GetValue());
