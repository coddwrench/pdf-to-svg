...
namespace IText.Kernel.Pdf.Tagutils {
    ...
    public class TagTreePointer {
        ...
        /// <see cref="DefaultAccessibilityProperties.SetNamespace(PdfNamespace)"/>
        ...
        /// creation.
        /// See also
        /// <see cref="SetNamespaceForNewTags(PdfNamespace)"/>.
        /// </remarks>
        ...
        public TagTreePointer(TagTreePointer tagPointer) {
            tagStructureContext = tagPointer.tagStructureContext;
            SetCurrentStructElem(tagPointer.GetCurrentStructElem());
            currentPage = tagPointer.currentPage;
            contentStream = tagPointer.contentStream;
            currentNamespace = tagPointer.currentNamespace;
        }
        ...
        /// to the
        /// <see cref="PdfCanvas.OpenTag(TagReference)"/>
        ...
        /// </returns>
        /// <seealso cref="SetNamespaceForNewTags(PdfNamespace)"/>
        ...
        /// <see cref="PdfPage.AddAPdfAnnotationt.PdfAnnotation, bool)"/>
        ...
        public virtual TagTreePointer AddAnnotationTag(PdfAnnotation annotation) {
            ...
            var kid = new PdfObjRef(annotation, GetCurrentStructElem(), GetDocument().GetNextStructParentIndex()
                );
        ...
        /// and
        /// <see cref="AddAnnotationTag(PdfAnnotation)"/>.
        /// <br />
        ...
        public virtual TagTreePointer RemoveTag() {
            var currentStructElem = GetCurrentStructElem();
            var parentElem = currentStructElem.GetParent();
            if (parentElem is PdfStructTreeRoot) {
                throw new PdfException(PdfException.CannotRemoveDocumentRootTag);
            }
            var kids = currentStructElem.GetKids();
            var parent = (PdfStructElem)parentElem;
            if (parent.IsFlushed()) {
                throw new PdfException(PdfException.CannotRemoveTagBecauseItsParentIsFlushed);
            }
            // remove waiting tag state if tag is removed
            var objForStructDict = tagStructureContext.GetWaitingTagsManager().GetObjForStructDict(currentStructElem
                .GetPdfObject());
            tagStructureContext.GetWaitingTagsManager().RemoveWaitingState(objForStructDict);
            var removedKidIndex = parent.RemoveKid(currentStructElem);
            var indRef = currentStructElem.GetPdfObject().GetIndirectReference();
            if (indRef != null) {
            ...
            foreach (var kid in kids) {
                if (kid is PdfStructElem) {
                    ...
                    var mcr = PrepareMcrForMovingToNewParent((PdfMcr)kid, parent);
        ...
        /// instance.
        /// </returns>
        public virtual TagTreePointer RelocateKid(int kidIndex, TagTreePointer
             pointerToNewParent) {
            ...
            var removedKid = GetCurrentStructElem().RemoveKid(kidIndex, true);
            if (removedKid is PdfStructElem) {
                pointerToNewParent.AddNewKid((PdfStructElem)removedKid);
            }
            else {
                if (removedKid is PdfMcr) {
                    var mcrKid = PrepareMcrForMovingToNewParent((PdfMcr)removedKid, pointerToNewParent.GetCurrentStructElem
                        ());
        ...
        /// instance.
        /// </returns>
        public virtual TagTreePointer Relocate(TagTreePointer 
            pointerToNewParent) {
            ...
            var i = GetIndexInParentKidsList();
            if (i < 0) {
                throw new PdfException(PdfException.CannotRelocateTagWhichParentIsAlreadyFlushed);
            }
        ...
        /// <see cref="PdfCanvas.OpenTag(TagReference)"/>
        /// and
        /// <see cref="SetPageForTagging(PdfPage)"/>.
        /// </remarks>
        ...
        public virtual TagTreePointer MoveToParent() {
            ...
            var parent = (PdfStructElem)GetCurrentStructElem().GetParent();
            if (parent.IsFlushed()) {
                var logger = LogManager.GetLogger(typeof(TagTreePointer));
        ...
        public virtual TagTreePointer MoveToKid(int kidIndex) {
            var kid = GetCurrentStructElem().GetKids()[kidIndex];
            if (kid is PdfStructElem) {
        ...
        public virtual TagTreePointer MoveToKid(int n, string role) {
            ...
            for (var i = 0; i < descendants.Count; ++i) {
                ...
                var descendantRole = descendants[i].GetRole().GetValue();
        ...
        public virtual IList<string> GetKidsRoles() {
            ...
            var kids = GetCurrentStructElem().GetKids();
            foreach (var kid in kids) {
                if (kid == null) {
                    roles.Add(null);
                }
        ...
        public virtual TagTreePointer FlushTag() {
            ...
            var parent = tagStructureContext.GetWaitingTagsManager().FlushTag(GetCurrentStructElem());
            if (parent != null) {
        ...
        /// <see cref="TagStructureContext.FlushPageTags(PdfPage)"/>
        ...
        /// instance.
        /// </returns>
        public virtual TagTreePointer MoveToPointer(TagTreePointer
             tagTreePointer) {
            currentStructElem = tagTreePointer.currentStructElem;
            return this;
        }
        ...
        internal virtual TagTreePointer SetCurrentStructElem(PdfStructElem structElem) {
        ...
        private int GetNextNewKidPosition() {
            var nextPos = nextNewKidIndex;
            nextNewKidIndex = -1;
            return nextPos;
        }

        private PdfStructElem AddNewKid(string role) {
            var kid = new PdfStructElem(GetDocument(), PdfStructTreeRoot.ConvertRoleToPdfName(role));
            ProcessKidNamespace(kid);
            return AddNewKid(kid);
        }

        private PdfStructElem AddNewKid(AccessibilityProperties properties) {
            var kid = new PdfStructElem(GetDocument(), PdfStructTreeRoot.ConvertRoleToPdfName(properties.GetRole
                ()));
        ...
        private void ProcessKidNamespace(PdfStructElem kid) {
            var kidNamespace = kid.GetNamespace();
        ...
        private PdfMcr PrepareMcrForMovingToNewParent(PdfMcr mcrKid, PdfStructElem newParent) {
            var mcrObject = mcrKid.GetPdfObject();
            var mcrPage = mcrKid.GetPageObject();
        ...
        private bool EnsureElementPageEqualsKidPage(PdfStructElem elem, PdfDictionary kidPage) {
            var pageObject = elem.GetPdfObject().Get(PdfName.Pg);
            if (pageObject == null) {
                pageObject = kidPage;
                // Explicitly using object indirect reference here in order to correctly process released objects.
