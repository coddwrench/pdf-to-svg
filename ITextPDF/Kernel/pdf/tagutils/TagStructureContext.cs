...
namespace IText.Kernel.Pdf.Tagutils {
    ...
    /// <see cref="PdfDocument.GetTagStructureContext()"/>.
    /// </remarks>
    public class TagStructureContext {
        ...
        /// <see cref="StandardNamespaces.PDF_2_0"/>.
        /// <para />
        ...
        public virtual TagStructureContext SetDocumentDefaultNamespace(PdfNamespace @namespace
            ...
            documentDefaultNamespace = @namespace;
            return this;
        }
        ...
        /// <see cref="PdfName.Namespaces">/Namespaces</see>
        ...
        public virtual PdfNamespace FetchNamespace(string namespaceName) {
            var ns = nameToNamespace.Get(namespaceName);
            if (ns == null) {
        ...
        public virtual IRoleMappingResolver ResolveMappingToStandardOrDomainSpecificRole(string role, PdfNamespace
             @namespace) {
            var mappingResolver = GetRoleMappingResolver(role, @namespace);
            ...
            while (mappingResolver.CurrentRoleShallBeMappedToStandard()) {
                if (++i > maxIters) {
                    var logger = LogManager.GetLogger(typeof(TagStructureContext));
        ...
        public virtual TagTreePointer RemoveAnnotationTag(PdfAnnotation annotation) {
            ...
            var annotDic = annotation.GetPdfObject();
            var structParentIndex = (PdfNumber)annotDic.Get(PdfName.StructParent);
            if (structParentIndex != null) {
                var objRef = document.GetStructTreeRoot().FindObjRefByStructParentIndex(annotDic.GetAsDictionary(PdfName
                    .P), structParentIndex.IntValue());
                if (objRef != null) {
                    var parent = (PdfStructElem)objRef.GetParent();
        ...
        public virtual TagTreePointer RemoveContentItem(PdfPage page, int mcid) {
            var mcr = document.GetStructTreeRoot().FindMcrByMcid(page.GetPdfObject(), mcid);
            if (mcr == null) {
                return null;
            }
            var parent = (PdfStructElem)mcr.GetParent();
        ...
        /// at
        /// <see cref="FlushPageTags(PdfPage)"/>.
        /// </remarks>
        ...
        public virtual TagStructureContext RemovePageTags(PdfPage page) {
            ...
            var pageMcrs = structTreeRoot.GetPageMarkedContentReferences(page);
            if (pageMcrs != null) {
        ...
        public virtual void NormalizeDocumentRootTag() {
            ...
            var forbid = forbidUnknownRoles;
            forbidUnknownRoles = false;
            var rootKids = document.GetStructTreeRoot().GetKids();
        ...
        internal virtual void EnsureNamespaceRegistered(PdfNamespace @namespace) {
            if (@namespace != null) {
                var namespaceObj = @namespace.GetPdfObject();
        ...
        internal virtual void ThrowExceptionIfRoleIsInvalid(string role, PdfNamespace @namespace) {
            if (!CheckIfRoleShallBeMappedToStandardRole(role, @namespace)) {
                var exMessage = ComposeInvalidRoleException(role, @namespace);
        ...
        internal virtual void FlushParentIfBelongsToPage(PdfStructElem parent, PdfPage currentPage) {
            ...
            var readyToBeFlushed = true;
            foreach (var kid in kids) {
                if (kid is PdfMcr) {
                    var kidPage = ((PdfMcr)kid).GetPageObject();
        ...
        private void SetNamespaceForNewTagsBasedOnExistingRoot() {
            ...
            if (rootKids.Count > 0) {
                var firstKid = (PdfStructElem)rootKids[0];
                var resolvedMapping = ResolveMappingToStandardOrDomainSpecificRole(firstKid.GetRole().GetValue
                ...
                if (resolvedMapping == null || !resolvedMapping.CurrentRoleIsStandard()) {
                    ...
                    logger.Warn(string.Format(LogMessageConstant.EXISTING_TAG_STRUCTURE_ROOT_IS_NOT_STANDARD, firstKid
        ...
        private string ComposeTooMuchTransitiveMappingsException(string role, PdfNamespace @namespace) {
            return ComposeExceptionBasedOnNamespacePresence(role, @namespace, LogMessageConstant.CANNOT_RESOLVE_ROLE_TOO_MUCH_TRANSITIVE_MAPPINGS
                , LogMessageConstant.CANNOT_RESOLVE_ROLE_IN_NAMESPACE_TOO_MUCH_TRANSITIVE_MAPPINGS);
        }

        private void InitRegisteredNamespaces() {
            var structTreeRoot = document.GetStructTreeRoot();
            foreach (var @namespace in structTreeRoot.GetNamespaces()) {
        ...
        private void RemovePageTagFromParent(IStructureNode pageTag, IStructureNode parent) {
            if (parent is PdfStructElem) {
                var structParent = (PdfStructElem)parent;
                if (!structParent.IsFlushed()) {
                    structParent.RemoveKid(pageTag);
                    var parentStructDict = structParent.GetPdfObject();
                    if (waitingTagsManager.GetObjForStructDict(parentStructDict) == null && parent.GetKids().Count == 0 && !(structParent
                        ...
                        var indRef = parentStructDict.GetIndirectReference();
                        if (indRef != null) {
            ...
            , string withNsEx)
        {
	        ...
	        var nsName = @namespace.GetNamespaceName();
	        var @ref = @namespace.GetPdfObject().GetIndirectReference();
	        if (@ref != null) {
