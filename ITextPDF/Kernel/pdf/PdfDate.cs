    ...
    public class PdfDate : PdfObjectWrapper<PdfString> {
        ...
        public static string GetW3CDate(string d) {
            ...
            if (d.StartsWith("-") || d.StartsWith("+")) {
                var sign = d.JSubstring(0, 1);
                d = d.Substring(1);
                if (d.Length >= 2) {
                    var h = d.JSubstring(0, 2);
                    var m = "00";
                    if (d.Length > 2) {
                        d = d.Substring(3);
                        if (d.Length >= 2) {
        ...
        private static string GenerateStringByDateTime(DateTime d) {
            var value = d.ToString("\\D\\:yyyyMMddHHmmss", DateTimeFormatInfo.InvariantInfo);
            var timezone = d.ToString("zzz", DateTimeFormatInfo.InvariantInfo).Replace(":", "'") + "'";
