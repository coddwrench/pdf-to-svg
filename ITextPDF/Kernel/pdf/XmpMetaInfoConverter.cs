...
namespace IText.Kernel.Pdf {
    internal class XmpMetaInfoConverter {
        ...
        internal static void AppendMetadataToInfo(byte[] xmpMetadata, PdfDocumentInfo info) {
            if (xmpMetadata != null) {
                try {
                    var meta = XMPMetaFactory.ParseFromBuffer(xmpMetadata);
                    var title = meta.GetLocalizedText(XMPConst.NS_DC, PdfConst.Title, XMPConst.X_DEFAULT, XMPConst.X_DEFAULT
                        );
                    if (title != null) {
                        info.SetTitle(title.GetValue());
                    }
                    var author = FetchArrayIntoString(meta, XMPConst.NS_DC, PdfConst.Creator);
                    if (author != null) {
                        info.SetAuthor(author);
                    }
                    // We assume that pdf:keywords has precedence over dc:subject
                    var keywords = meta.GetProperty(XMPConst.NS_PDF, PdfConst.Keywords);
                    if (keywords != null) {
                        info.SetKeywords(keywords.GetValue());
                    }
                    else {
                        var keywordsStr = FetchArrayIntoString(meta, XMPConst.NS_DC, PdfConst.Subject);
                        if (keywordsStr != null) {
                            info.SetKeywords(keywordsStr);
                        }
                    }
                    var subject = meta.GetLocalizedText(XMPConst.NS_DC, PdfConst.Description, XMPConst.X_DEFAULT, XMPConst
                        .X_DEFAULT);
                    if (subject != null) {
                        info.SetSubject(subject.GetValue());
                    }
                    var creator = meta.GetProperty(XMPConst.NS_XMP, PdfConst.CreatorTool);
                    if (creator != null) {
                        info.SetCreator(creator.GetValue());
                    }
                    var producer = meta.GetProperty(XMPConst.NS_PDF, PdfConst.Producer);
                    if (producer != null) {
                        info.Put(PdfName.Producer, new PdfString(producer.GetValue(), PdfEncodings.UNICODE_BIG));
                    }
                    var trapped = meta.GetProperty(XMPConst.NS_PDF, PdfConst.Trapped);
                    if (trapped != null) {
        ...
        internal static void AppendDocumentInfoToMetadata(PdfDocumentInfo info, XMPMeta xmpMeta) {
            var docInfo = info.GetPdfObject();
            if (docInfo != null) {
                PdfName key;
                PdfObject obj;
                string value;
                foreach (var pdfName in docInfo.KeySet()) {
                    key = pdfName;
                    obj = docInfo.Get(key);
                    if (obj == null) {
                        continue;
                    ...
                    if (PdfName.Title.Equals(key)) {
                        ...
                        if (PdfName.Author.Equals(key)) {
                            foreach (var v in StringUtil.Split(value, ",|;")) {
                                if (v.Trim().Length > 0) {
        ...
        private static void AppendArrayItemIfDoesNotExist(XMPMeta meta, string ns, string arrayName, string value, 
            int arrayOption) {
            var currentCnt = meta.CountArrayItems(ns, arrayName);
            for (var i = 0; i < currentCnt; i++) {
                var item = meta.GetArrayItem(ns, arrayName, i + 1);
        ...
        private static string FetchArrayIntoString(XMPMeta meta, string ns, string arrayName) {
            var keywordsCnt = meta.CountArrayItems(ns, arrayName);
            StringBuilder sb = null;
            for (var i = 0; i < keywordsCnt; i++) {
                var curKeyword = meta.GetArrayItem(ns, arrayName, i + 1);
                if (sb == null) {
