...
namespace IText.Kernel.Pdf.Xobject
{
	...
	public class PdfImageXObject : PdfXObject
	{
		...
		/// with Image XObject.
		/// </param>
		/// <seealso cref="PdfXObject.MakeXObject(PdfStream)"/>
		...
		/// <see cref="PdfObjectWrapper{TPdfDocumentPdf.PdfDocument)"/>.
		...
		{
			var image = new PdfImageXObject((PdfStream)GetPdfObject
				().CopyTo(document));
			image.mask = mask;
		...
		public virtual byte[] GetImageBytes(bool decoded)
		{
			...
			var bytes = GetPdfObject().GetBytes(false);
			if (decoded)
			{
				var filters = new Dictionary<PdfName, IFilterHandler>(FilterHandlers.GetDefaultFilterHandlers());
				filters.Put(PdfName.JBIG2Decode, new DoNothingFilter());
				bytes = PdfReader.DecodeBytes(bytes, GetPdfObject(), filters);
				var imageType = IdentifyImageType();
		...
		/// <see cref="ImageType.PNG"/>
		/// as we lose this information when converting a
		/// PNG image into something that can be put into a PDF file.
		/// The possible values are:
		/// <see cref="ImageType.JPEG"/>
		/// ,
		/// <see cref="ImageType.JPEG2000"/>
		/// ,
		/// <see cref="ImageType.JBIG2"/>
		/// ,
		/// <see cref="ImageType.TIFF"/>
		/// ,
		/// <see cref="ImageType.PNG"/>
		/// </remarks>
		...
		{
			var filter = GetPdfObject().Get(PdfName.Filter);
			...
			{
				var filterName = (PdfName)filters.Get(i);
			...
			var imageInfo = new ImagePdfBytesInfo(this);
		...
		{
			var bytesType = IdentifyImageType();
			switch (bytesType)
			{
				case ImageType.PNG:
					{
		...
		{
			var wNum = GetPdfObject().GetAsNumber(PdfName.Width);
			if (wNum != null)
			{
		...
		{
			var hNum = GetPdfObject().GetAsNumber(PdfName.Height);
			if (hNum != null)
			{
			...
			)
		{
			...
			var decodeParms = CreateDictionaryFromMap(stream, image.GetDecodeParms());
			if (decodeParms != null)
			{
			...
			if (image is PngImageData)
			{
				var pngImage = (PngImageData)image;
			...
			var iccProfile = image.GetProfile();
			if (iccProfile != null)
			{
				var iccProfileStream = PdfCieBasedCs.IccBased.GetIccProfileStream(iccProfile);
				var iccBasedColorSpace = new PdfArray();
				iccBasedColorSpace.Add(PdfName.ICCBased);
				iccBasedColorSpace.Add(iccProfileStream);
				var colorSpaceObject = stream.Get(PdfName.ColorSpace);
				var iccProfileShouldBeApplied = true;
				if (colorSpaceObject != null)
				{
					var cs = PdfColorSpace.MakeColorSpace(colorSpaceObject);
					if (cs == null)
					{
						LogManager.GetLogger(typeof(PdfImageXObject)).Error(LogMessageConstant.IMAGE_HAS_INCORRECT_OR_UNSUPPORTED_COLOR_SPACE_OVERRIDDEN_BY_ICC_PROFILE
							);
					}
					else
					{
						...
						{
							var baseCs = ((PdfSpecialCs.Indexed)cs).GetBaseCs();
							if (baseCs == null)
							{
								LogManager.GetLogger(typeof(PdfImageXObject)).Error(LogMessageConstant.IMAGE_HAS_INCORRECT_OR_UNSUPPORTED_BASE_COLOR_SPACE_IN_INDEXED_COLOR_SPACE_OVERRIDDEN_BY_ICC_PROFILE
									);
							}
							else
							{
								...
								{
									LogManager.GetLogger(typeof(PdfImageXObject)).Error(LogMessageConstant.IMAGE_HAS_ICC_PROFILE_WITH_INCOMPATIBLE_NUMBER_OF_COLOR_COMPONENTS_COMPARED_TO_BASE_COLOR_SPACE_IN_INDEXED_COLOR_SPACE
										);
						...
						else
						{
							...
							{
								LogManager.GetLogger(typeof(PdfImageXObject)).Error(LogMessageConstant.IMAGE_HAS_ICC_PROFILE_WITH_INCOMPATIBLE_NUMBER_OF_COLOR_COMPONENTS_COMPARED_TO_COLOR_SPACE
									);
								iccProfileShouldBeApplied = false;
							}
							else
			...
			{
				var t = new PdfArray();
				foreach (var transparencyItem in transparency)
				{
					...
					else
					{
						...
						else
						{
							...
							else
							{
								if (value is byte[])
								{
									var globalsStream = new PdfStream();
		...
		private static PdfObject GetColorSpaceInfo(PngImageData pngImageData)
		{
			...
			if (pngImageData.IsGrayscaleImage())
			{
				...
				map.Put(PdfName.WhitePoint, new PdfArray(new[] { 1, 1, 1 }));
			}
			else
			{
				float[] wp = { 1, 1, 1 };
				array.Add(PdfName.CalRGB);
				var gamma = pngImageData.GetGamma();
				if (gamma != 1f)
				{
					var gm = new float[3];
					gm[0] = gamma;
				...
				{
					var helper = new PngChromaticitiesHelper();
		...
		private class PngChromaticitiesHelper
		{
			...
			{
				var pngChromaticities = pngImageData.GetPngChromaticities();
				var z = pngChromaticities.GetYW() * ((pngChromaticities.GetXG() - pngChromaticities.GetXB()) * pngChromaticities
				...
				var YA = pngChromaticities.GetYR() * ((pngChromaticities.GetXG() - pngChromaticities.GetXB()) * pngChromaticities
				...
				var XA = YA * pngChromaticities.GetXR() / pngChromaticities.GetYR();
				var ZA = YA * ((1 - pngChromaticities.GetXR()) / pngChromaticities.GetYR() - 1);
				var YB = -pngChromaticities.GetYG() * ((pngChromaticities.GetXR() - pngChromaticities.GetXB()) * pngChromaticities
				...
				var XB = YB * pngChromaticities.GetXG() / pngChromaticities.GetYG();
				var ZB = YB * ((1 - pngChromaticities.GetXG()) / pngChromaticities.GetYG() - 1);
				var YC = pngChromaticities.GetYB() * ((pngChromaticities.GetXR() - pngChromaticities.GetXG()) * pngChromaticities
				...
				var XC = YC * pngChromaticities.GetXB() / pngChromaticities.GetYB();
				var ZC = YC * ((1 - pngChromaticities.GetXB()) / pngChromaticities.GetYB() - 1);
				var XW = XA + XB + XC;
				float YW = 1;
				var ZW = ZA + ZB + ZC;
				var wpa = new float[3];
				...
				wpa[1] = YW;
				wpa[2] = ZW;
				wp = JavaUtil.ArraysCopyOf(wpa, 3);
