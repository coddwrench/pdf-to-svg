        ...
        public virtual byte[] DecodeTiffAndPngBytes(byte[] imageBytes) {
            ...
            if (pngColorType < 0) {
                ...
                if (colorspace is PdfArray) {
                    var ca = (PdfArray)colorspace;
                    var tyca = ca.Get(0);
                    if (!PdfName.ICCBased.Equals(tyca)) {
                        throw new IOException(IOException.ColorSpaceIsNotSupported).SetMessageParams(tyca.ToString
                            ());
                    }
                    var pr = (PdfStream)ca.Get(1);
                    var n = pr.GetAsNumber(PdfName.N).IntValue();
                    if (n != 4) {
                ...
                stride = 4 * width;
                var wr = new TiffWriter();
                wr.AddField(new TiffWriter.FieldShort(TIFFConstants.TIFFTAG_SAMPLESPERPIXEL, 4));
                wr.AddField(new TiffWriter.FieldShort(TIFFConstants.TIFFTAG_BITSPERSAMPLE, new[] { 8, 8, 8, 8 }));
                ...
                wr.AddField(new TiffWriter.FieldRational(TIFFConstants.TIFFTAG_XRESOLUTION, new[] { 300, 1 }));
                wr.AddField(new TiffWriter.FieldRational(TIFFConstants.TIFFTAG_YRESOLUTION, new[] { 300, 1 }));
