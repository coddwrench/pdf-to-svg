...
namespace IText.Kernel.Pdf {
    ...
    public class PdfCatalog : PdfObjectWrapper<PdfDictionary> {
        private static readonly ILog LOGGER = LogManager.GetLogger(typeof(PdfCatalog));
        ...
        public virtual PdfOCProperties GetOCProperties(bool createIfNotExists) {
            ...
            var ocPropertiesDict = GetPdfObject().GetAsDictionary(PdfName.OCProperties);
        ...
        public virtual PdfViewerPreferences GetViewerPreferences() {
            var viewerPreferences = GetPdfObject().GetAsDictionary(PdfName.ViewerPreferences);
        ...
        public virtual PdfNameTree GetNameTree(PdfName treeType) {
            var tree = nameTrees.Get(treeType);
            if (tree == null) {
        ...
        public virtual void AddDeveloperExtension(PdfDeveloperExtension extension) {
            var extensions = GetPdfObject().GetAsDictionary(PdfName.Extensions);
            if (extensions == null) {
                extensions = new PdfDictionary();
                Put(PdfName.Extensions, extensions);
            }
            else {
                var existingExtensionDict = extensions.GetAsDictionary(extension.GetPrefix());
                if (existingExtensionDict != null) {
                    var diff = extension.GetBaseVersion().CompareTo(existingExtensionDict.GetAsName(PdfName.BaseVersion));
                    if (diff < 0) {
                        return;
                    }
        ...
        /// <returns>
        /// 
        /// <see cref="PdfCollection"/>
        /// wrapper of collection dictionary.
        /// </returns>
        public virtual PdfCollection GetCollection() {
            var collectionDictionary = GetPdfObject().GetAsDictionary(PdfName.Collection);
        ...
        /// 
        /// <see cref="PdfCollection">dictionary</see>
        /// </param>
        /// <returns>
        /// 
        /// <see cref="PdfCatalog"/>
        ...
        internal virtual PdfOutline GetOutlines(bool updateOutlines) {
            ...
            outlineMode = true;
            var destsTree = GetNameTree(PdfName.Dests);
            var outlineRoot = GetPdfObject().GetAsDictionary(PdfName.Outlines);
            if (outlineRoot == null) {
            ...
            if (HasOutlines()) {
                ...
                if (pagesWithOutlines.Count > 0) {
                    if (pagesWithOutlines.Get(page.GetPdfObject()) != null) {
                        foreach (var outline in pagesWithOutlines.Get(page.GetPdfObject())) {
        ...
        internal virtual PdfDestination CopyDestination(PdfObject dest, IDictionary<PdfPage, PdfPage> page2page, PdfDocument
            ...
            if (dest.IsArray()) {
                var pageObject = ((PdfArray)dest).Get(0);
                foreach (var oldPage in page2page.Keys) {
                    if (oldPage.GetPdfObject() == pageObject) {
                        ...
                        var copiedArray = (PdfArray)dest.CopyTo(toDocument, false);
                ...
                if (dest.IsString() || dest.IsName()) {
                    var destsTree = GetNameTree(PdfName.Dests);
                    var dests = destsTree.GetNames();
                    var srcDestName = dest.IsString() ? ((PdfString)dest).ToUnicodeString() : ((PdfName)dest).GetValue();
                    var srcDestArray = (PdfArray)dests.Get(srcDestName);
                    if (srcDestArray != null) {
                        var pageObject = srcDestArray.Get(0);
                        ...
                        foreach (var oldPage in page2page.Keys) {
                            if (oldPage.GetPdfObject() == pageObject) {
                                ...
                                if (!IsEqualSameNameDestExist(page2page, toDocument, srcDestName, srcDestArray, oldPage)) {
                                    // in the copiedArray old page ref will be correctly replaced by the new page ref as this page is already copied
                                    var copiedArray = (PdfArray)srcDestArray.CopyTo(toDocument, false);
        ...
        private bool IsEqualSameNameDestExist(IDictionary<PdfPage, PdfPage> page2page, PdfDocument toDocument, string
            ...
            var sameNameDest = (PdfArray)toDocument.GetCatalog().GetNameTree(PdfName.Dests).GetNames().Get(srcDestName
                );
            var equalSameNameDestExists = false;
            if (sameNameDest != null && sameNameDest.GetAsDictionary(0) != null) {
                var existingDestPageRef = sameNameDest.GetAsDictionary(0).GetIndirectReference();
                var newDestPageRef = page2page.Get(oldPage).GetPdfObject().GetIndirectReference();
                if (equalSameNameDestExists = existingDestPageRef.Equals(newDestPageRef) && sameNameDest.Size() == srcDestArray
                    .Size()) {
                    for (var i = 1; i < sameNameDest.Size(); ++i) {
        ...
        private void AddOutlineToPage(PdfOutline outline, IDictionary<string, PdfObject> names) {
            var pageObj = outline.GetDestination().GetDestinationPage(names);
            if (pageObj is PdfNumber) {
                var pageNumber = ((PdfNumber)pageObj).IntValue() + 1;
                try {
                    pageObj = GetDocument().GetPage(pageNumber).GetPdfObject();
                }
                catch (IndexOutOfRangeException) {
                    pageObj = null;
                    LOGGER.Warn(MessageFormatUtil.Format(LogMessageConstant.OUTLINE_DESTINATION_PAGE_NUMBER_IS_OUT_OF_BOUNDS
                        , pageNumber));
                }
            }
            if (pageObj != null) {
                var outs = pagesWithOutlines.Get(pageObj);
                if (outs == null) {
        ...
        private void AddOutlineToPage(PdfOutline outline, PdfDictionary item, IDictionary<string, PdfObject> names
            ...
            var dest = item.Get(PdfName.Dest);
            if (dest != null) {
                var destination = PdfDestination.MakeDestination(dest);
        ...
        private void ConstructOutlines(PdfDictionary outlineRoot, IDictionary<string, PdfObject> names) {
            ...
            var first = outlineRoot.GetAsDictionary(PdfName.First);
            var current = first;
            PdfDictionary next;
            PdfDictionary parent;
            var parentOutlineMap = new Dictionary<PdfDictionary, PdfOutline>();
            outlines = new PdfOutline(OutlineRoot, outlineRoot, GetDocument());
            var parentOutline = outlines;
            ...
            while (current != null) {
                ...
                var currentOutline = new PdfOutline(current.GetAsString(PdfName.Title).ToUnicodeString(), current, 
                    parentOutline);
