...
namespace IText.Kernel.Pdf.Collection {
    public class PdfCollection : PdfObjectWrapper<PdfDictionary> {
        ...
        [Obsolete(@"Will always return null. The return will be changed to PdfName in 7.2. Use getPdfObject().getAsName(PdfName.View) , or one of IsViewDetails() , IsViewTile() , IsViewHidden() ."
            )]
        ...
        public virtual bool IsViewDetails() {
            var view = GetPdfObject().GetAsName(PdfName.View);
