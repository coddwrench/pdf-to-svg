...
namespace IText.Kernel.Pdf {
    ...
    public class PdfEncryption : PdfObjectWrapper<PdfDictionary> {
        ...
        public PdfEncryption(X509Certificate[] certs, int[] permissions, int encryptionType, PdfVersion version)
            ...
            switch (revision) {
                case STANDARD_ENCRYPTION_40: {
                    securityHandler = new PubSecHandlerUsingStandard40(GetPdfObject(), certs, permissions, encryptMetadata
                        , embeddedFilesOnly);
                    break;
                }

                case STANDARD_ENCRYPTION_128: {
                    securityHandler = new PubSecHandlerUsingStandard128(GetPdfObject(), certs, permissions, encryptMetadata
                        , embeddedFilesOnly);
                    break;
                }

                case AES_128: {
                    securityHandler = new PubSecHandlerUsingAes128(GetPdfObject(), certs, permissions, encryptMetadata, embeddedFilesOnly
                        );
                    break;
                }

                case AES_256: {
                    securityHandler = new PubSecHandlerUsingAes256(GetPdfObject(), certs, permissions, encryptMetadata, embeddedFilesOnly
                        );
                    break;
                }
            }
        ...
        public PdfEncryption(PdfDictionary pdfDict, byte[] password, byte[] documentId)
            ...
            SetForbidRelease();
            this.documentId = documentId;
            var revision = ReadAndSetCryptoModeForStdHandler(pdfDict);
            switch (revision) {
                case STANDARD_ENCRYPTION_40: {
                    var handlerStd40 = new StandardHandlerUsingStandard40(GetPdfObject(), password
                ...
                case STANDARD_ENCRYPTION_128: {
                    var handlerStd128 = new StandardHandlerUsingStandard128(GetPdfObject(), password
                        , documentId, encryptMetadata);
                ...
                case AES_128: {
                    var handlerAes128 = new StandardHandlerUsingAes128(GetPdfObject(), password, documentId
                        , encryptMetadata);
                ...
                case AES_256: {
                    var aes256Handler = new StandardHandlerUsingAes256(GetPdfObject(), password);
        ...
        public PdfEncryption(PdfDictionary pdfDict, object certificateKey, X509Certificate certificate)
            : base(pdfDict) {
            SetForbidRelease();
            var revision = ReadAndSetCryptoModeForPubSecHandler(pdfDict);
            switch (revision) {
                case STANDARD_ENCRYPTION_40: {
                    securityHandler = new PubSecHandlerUsingStandard40(GetPdfObject(), certificateKey, certificate, encryptMetadata
                        );
                    break;
                }

                case STANDARD_ENCRYPTION_128: {
                    securityHandler = new PubSecHandlerUsingStandard128(GetPdfObject(), certificateKey, certificate, encryptMetadata
                        );
                    break;
                }

                case AES_128: {
                    securityHandler = new PubSecHandlerUsingAes128(GetPdfObject(), certificateKey, certificate, encryptMetadata
                        );
                    break;
                }

                case AES_256: {
                    securityHandler = new PubSecHandlerUsingAes256(GetPdfObject(), certificateKey, certificate, encryptMetadata
                        );
                    break;
                }
            }
        ...
        public static byte[] GenerateNewDocumentId() {
            ...
            //}
            //var time = SystemUtil.GetTimeBasedSeed();
            //var mem = SystemUtil.GetFreeMemory();
            //var s = time + "+" + mem + "+" + (seq++);
            //return md5.Digest(s.GetBytes(EncodingUtil.ISO_8859_1));
        }
        ...
        public static PdfObject CreateInfoId(byte[] firstId, byte[] secondId) {
            ...
            buf.Append('[').Append('<');
            for (var k = 0; k < firstId.Length; ++k) {
                buf.AppendHex(firstId[k]);
            }
            buf.Append('>').Append('<');
            for (var k = 0; k < secondId.Length; ++k) {
        ...
        public virtual byte[] EncryptByteArray(byte[] b) {
            ...
            var ose = GetEncryptionStream(ba);
            try {
                ose.Write(b);
            }
            catch (IOException e) {
        ...
        private int ReadAndSetCryptoModeForStdHandler(PdfDictionary encDict) {
            int cryptoMode;
            var length = 0;
            var rValue = encDict.GetAsNumber(PdfName.R);
            if (rValue == null) {
                throw new PdfException(PdfException.IllegalRValue);
            }
            var revision = rValue.IntValue();
            var embeddedFilesOnlyMode = ReadEmbeddedFilesOnlyFromEncryptDictionary(encDict);
            switch (revision) {
                case 2: {
                ...
                case 3: {
                    var lengthValue = encDict.GetAsNumber(PdfName.Length);
                    if (lengthValue == null) {
                ...
                case 4: {
                    var dic = (PdfDictionary)encDict.Get(PdfName.CF);
                    if (dic == null) {
                    ...
                    var em = encDict.GetAsBoolean(PdfName.EncryptMetadata);
                ...
                case 6: {
                    ...
                    var em5 = encDict.GetAsBoolean(PdfName.EncryptMetadata);
        ...
        private int ReadAndSetCryptoModeForPubSecHandler(PdfDictionary encDict) {
            int cryptoMode;
            var length = 0;
            var vValue = encDict.GetAsNumber(PdfName.V);
            if (vValue == null) {
                throw new PdfException(PdfException.IllegalVValue);
            }
            var v = vValue.IntValue();
            var embeddedFilesOnlyMode = ReadEmbeddedFilesOnlyFromEncryptDictionary(encDict);
            switch (v) {
                case 1: {
                    cryptoMode = EncryptionConstants.STANDARD_ENCRYPTION_40;
                ...
                case 4:
                case 5: {
                    var dic = encDict.GetAsDictionary(PdfName.CF);
                    if (dic == null) {
                        throw new PdfException(PdfException.CfNotFoundEncryption);
                    }
                    ...
                    var em = dic.GetAsBoolean(PdfName.EncryptMetadata);
                    if (em != null && !em.GetValue()) {
                        cryptoMode |= EncryptionConstants.DO_NOT_ENCRYPT_METADATA;
                    }
                    if (embeddedFilesOnlyMode) {
        ...
        internal static bool ReadEmbeddedFilesOnlyFromEncryptDictionary(PdfDictionary encDict) {
            var embeddedFilesFilter = encDict.GetAsName(PdfName.EFF);
            var encryptEmbeddedFiles = !PdfName.Identity.Equals(embeddedFilesFilter) && embeddedFilesFilter != null;
            var encryptStreams = !PdfName.Identity.Equals(encDict.GetAsName(PdfName.StmF));
            var encryptStrings = !PdfName.Identity.Equals(encDict.GetAsName(PdfName.StrF));
            if (encryptStreams || encryptStrings || !encryptEmbeddedFiles) {
                return false;
            }
            var cfDictionary = encDict.GetAsDictionary(PdfName.CF);
