...
namespace IText.Kernel.Pdf {
    public class PdfNumTree {
        ...
        public virtual IDictionary<int?, PdfObject> GetNumbers() {
            ...
            if (treeType.Equals(PdfName.PageLabels)) {
                ...
                if (treeType.Equals(PdfName.ParentTree)) {
                    var structTreeRoot = catalog.GetPdfObject().GetAsDictionary(PdfName.StructTreeRoot);
        ...
        public virtual PdfDictionary BuildTree() {
            ...
            var skip = NODE_SIZE;
            var kids = new PdfDictionary[(numbers.Length + NODE_SIZE - 1) / NODE_SIZE];
            for (var i = 0; i < kids.Length; ++i) {
                var offset = i * NODE_SIZE;
                var end = Math.Min(offset + NODE_SIZE, numbers.Length);
                var dic = new PdfDictionary();
