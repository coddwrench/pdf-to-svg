...
namespace IText.Kernel.Pdf.Extgstate {
    ...
    public class PdfExtGState : PdfObjectWrapper<PdfDictionary> {
        ...
        /// <see cref="PdfDoPdfFontel.Font.PdfFont)"/>
        /// method first.
        /// </remarks>
        /// <param name="font">
        /// a
        ...
        public virtual bool? IsBlackPointCompensationUsed() {
            var useBlackPointCompensation = GetPdfObject().GetAsName(PdfName.UseBlackPtComp);
        ...
        public virtual float[] GetHalftoneOrigin() {
            var hto = GetPdfObject().GetAsArray(PdfName.HTO);
            if (hto != null && hto.Size() == 2 && hto.Get(0).IsNumber() && hto.Get(1).IsNumber()) {
                return new[] { hto.GetAsNumber(0).FloatValue(), hto.GetAsNumber(1).FloatValue() };
            }
