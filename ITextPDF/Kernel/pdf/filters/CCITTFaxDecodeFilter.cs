...
namespace IText.Kernel.Pdf.Filters {
    ...
    public class CCITTFaxDecodeFilter : IFilterHandler {
        public virtual byte[] Decode(byte[] b, PdfName filterName, PdfObject decodeParams, PdfDictionary streamDictionary
            ...
            var wn = streamDictionary.GetAsNumber(PdfName.Width);
            var hn = streamDictionary.GetAsNumber(PdfName.Height);
            if (wn == null || hn == null) {
                throw new PdfException(PdfException.FilterCcittfaxdecodeIsOnlySupportedForImages);
            }
            var width = wn.IntValue();
            var height = hn.IntValue();
            var param = decodeParams is PdfDictionary ? (PdfDictionary)decodeParams : null;
            var k = 0;
            var blackIs1 = false;
            var byteAlign = false;
            if (param != null) {
                var kn = param.GetAsNumber(PdfName.K);
                if (kn != null) {
                    k = kn.IntValue();
                }
                var bo = param.GetAsBoolean(PdfName.BlackIs1);
                if (bo != null) {
            ...
            var outBuf = new byte[(width + 7) / 8 * height];
            var decoder = new TIFFFaxDecompressor();
            if (k == 0 || k > 0) {
                var tiffT4Options = k > 0 ? TIFFConstants.GROUP3OPT_2DENCODING : 0;
                ...
                if (decoder.fails > 0) {
                    var outBuf2 = new byte[(width + 7) / 8 * height];
                    var oldFails = decoder.fails;
                ...
                var deca = new TIFFFaxDecoder(1, width, height);
