...
namespace IText.Kernel.Pdf {
    internal class EncryptedEmbeddedStreamsHandler {
        ...
        internal virtual void StoreAllEmbeddedStreams() {
            for (var i = 0; i < document.GetNumberOfPdfObjects(); ++i) {
                var indirectObject = document.GetPdfObject(i);
                if (indirectObject is PdfDictionary) {
                    var embeddedStream = GetEmbeddedFileStreamFromDictionary((PdfDictionary)indirectObject);
        ...
        private static PdfStream GetEmbeddedFileStreamFromDictionary(PdfDictionary dictionary) {
            var embeddedFileDictionary = dictionary.GetAsDictionary(PdfName.EF);
