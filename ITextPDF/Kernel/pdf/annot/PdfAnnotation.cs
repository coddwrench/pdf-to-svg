...
namespace IText.Kernel.Pdf.Annot {
    ...
    public abstract class PdfAnnotation : PdfObjectWrapper<PdfDictionary> {
        ...
        /// Also see
        /// <see cref="PdfWidgetAnnotation.SetHighlightMode(PdfName)"/>.
        /// </remarks>
        ...
        /// Also see
        /// <see cref="PdfTextAnnotation.SetState(PdfString)"/>.
        /// </remarks>
        ...
        /// Also see
        /// <see cref="PdfTextAnnotation.SetStateModel(PdfString)"/>.
        /// </remarks>
        ...
        public static PdfAnnotation MakeAnnotation(PdfObject pdfObject) {
            ...
            if (pdfObject.IsDictionary()) {
                ...
                var subtype = dictionary.GetAsName(PdfName.Subtype);
        ...
        public virtual PdfPage GetPage() {
            ...
            if (page == null && (annotationIndirectReference = GetPdfObject().GetIndirectReference()) != null) {
                var doc = annotationIndirectReference.GetDocument();
                var pageDictionary = GetPageObject();
                if (pageDictionary != null) {
                    page = doc.GetPage(pageDictionary);
                }
                else {
                    for (var i = 1; i <= doc.GetNumberOfPages(); i++) {
                        var docPage = doc.GetPage(i);
                        if (!docPage.IsFlushed()) {
                            foreach (var annot in docPage.GetAnnotations()) {
        ...
        public virtual int GetFlags() {
            var f = GetPdfObject().GetAsNumber(PdfName.F);
            if (f != null) {
                return f.IntValue();
            }
        ...
        /// <see cref="PdfName.N">Normal</see>
        /// ,
        /// <see cref="PdfName.R">Rollover</see>
        /// and
        /// <see cref="PdfName.D">Down</see>.
        /// </param>
        /// <returns>
        ...
        public virtual PdfDictionary GetAppearanceObject(PdfName appearanceType) {
            var ap = GetAppearanceDictionary();
            if (ap != null) {
                var apObject = ap.Get(appearanceType);
        ...
        /// See also
        /// <see cref="GetAppearanceObject(PdfName)"/>.
        /// </remarks>
        ...
        /// instance.
        /// </returns>
        public virtual PdfAnnotation SetAppearance(PdfName appearanceType, PdfAnnotationAppearance
             appearance) {
        ...
        public virtual int GetStructParentIndex() {
            var n = GetPdfObject().GetAsNumber(PdfName.StructParent);
            if (n == null) {
                return -1;
            }
        ...
        public virtual string GetLang() {
            var lang = GetPdfObject().GetAsString(PdfName.Lang);
        ...
        public virtual float GetNonStrokingOpacity() {
            var nonStrokingOpacity = GetPdfObject().GetAsNumber(PdfName.ca);
        ...
        public virtual float GetStrokingOpacity() {
            var strokingOpacity = GetPdfObject().GetAsNumber(PdfName.CA);
        ...
        public virtual void AddAssociatedFile(PdfFileSpec fs) {
            if (null == ((PdfDictionary)fs.GetPdfObject()).Get(PdfName.AFRelationship)) {
                var logger = LogManager.GetLogger(typeof(PdfAnnotation));
