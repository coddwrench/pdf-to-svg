...
namespace IText.Kernel.Pdf.Annot {
    ...
    public abstract class PdfMarkupAnnotation : PdfAnnotation {
        protected internal PdfAnnotation inReplyTo;

        protected internal PdfPopupAnnotation popup;
        ...
        /// The Contents (see
        /// <see cref="PdfAnnotation.SetContents(PdfString)"/>
        ...
        public virtual PdfPopupAnnotation GetPopup() {
            if (popup == null) {
                var popupObject = GetPopupObject();
                if (popupObject != null) {
                    var annotation = MakeAnnotation(popupObject);
                    if (!(annotation is PdfPopupAnnotation)) {
                        var logger = LogManager.GetLogger(typeof(PdfMarkupAnnotation));
        ...
        /// <list type="bullet">
        /// <item><description>
        /// <see cref="PdfName.Type"/>
        /// - (optional) If present, shall be
        /// <see cref="PdfName.ExData"/>.
        /// </description></item>
        /// <item><description>
        ...
        /// instance
        /// </returns>
        [Obsolete(@"use SetExternalData(iText.Kernel.Pdf.PdfDictionary) instead")]
        public virtual PdfMarkupAnnotation SetExternalData(PdfName exData) {
