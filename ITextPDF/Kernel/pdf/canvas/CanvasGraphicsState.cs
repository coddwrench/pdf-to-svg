...
namespace IText.Kernel.Pdf.Canvas {
    ...
    public class CanvasGraphicsState {
        ...
        private bool automaticStrokeAdjustment;
        ...
        // alpha source
        private bool alphaIsShape;

        private bool strokeOverprint;

        private bool fillOverprint;
        ...
        internal virtual void UpdateFromExtGState(PdfExtGState extGState, PdfDocument pdfDocument) {
            var lw = extGState.GetLineWidth();
            if (lw != null) {
                lineWidth = (float)lw;
            }
            var lc = extGState.GetLineCapStyle();
            if (lc != null) {
                lineCapStyle = (int)lc;
            }
            var lj = extGState.GetLineJoinStyle();
            if (lj != null) {
                lineJoinStyle = (int)lj;
            }
            var ml = extGState.GetMiterLimit();
            if (ml != null) {
                miterLimit = (float)ml;
            }
            var d = extGState.GetDashPattern();
            if (d != null) {
                dashPattern = d;
            }
            var ri = extGState.GetRenderingIntent();
            if (ri != null) {
                renderingIntent = ri;
            }
            var op = extGState.GetStrokeOverprintFlag();
            if (op != null) {
            ...
            var opm = extGState.GetOverprintMode();
            if (opm != null) {
                overprintMode = (int)opm;
            }
            var fnt = extGState.GetFont();
            if (fnt != null) {
                var fontDictionary = fnt.GetAsDictionary(0);
                if (font == null || font.GetPdfObject() != fontDictionary) {
                    font = pdfDocument.GetFont(fontDictionary);
                }
                var fntSz = fnt.GetAsNumber(1);
                if (fntSz != null) {
                    fontSize = fntSz.FloatValue();
                }
            }
            var bg = extGState.GetBlackGenerationFunction();
            if (bg != null) {
                blackGenerationFunction = bg;
            }
            var bg2 = extGState.GetBlackGenerationFunction2();
            if (bg2 != null) {
                blackGenerationFunction2 = bg2;
            }
            var ucr = extGState.GetUndercolorRemovalFunction();
            if (ucr != null) {
                underColorRemovalFunction = ucr;
            }
            var ucr2 = extGState.GetUndercolorRemovalFunction2();
            if (ucr2 != null) {
                underColorRemovalFunction2 = ucr2;
            }
            var tr = extGState.GetTransferFunction();
            if (tr != null) {
                transferFunction = tr;
            }
            var tr2 = extGState.GetTransferFunction2();
            if (tr2 != null) {
                transferFunction2 = tr2;
            }
            var ht = extGState.GetHalftone();
            if (ht != null) {
                halftone = ht;
            }
            var local_htp = extGState.GetPdfObject().Get(PdfName.HTP);
            if (local_htp != null) {
                htp = local_htp;
            }
            var fl = extGState.GetFlatnessTolerance();
            if (fl != null) {
                flatnessTolerance = (float)fl;
            }
            var sm = extGState.GetSmothnessTolerance();
            if (sm != null) {
                smoothnessTolerance = sm;
            }
            var sa = extGState.GetAutomaticStrokeAdjustmentFlag();
            if (sa != null) {
                automaticStrokeAdjustment = (bool)sa;
            }
            var bm = extGState.GetBlendMode();
            if (bm != null) {
                blendMode = bm;
            }
            var sMask = extGState.GetSoftMask();
            if (sMask != null) {
                softMask = sMask;
            }
            var ca = extGState.GetStrokeOpacity();
            if (ca != null) {
            ...
            var ais = extGState.GetAlphaSourceFlag();
            if (ais != null) {
                alphaIsShape = (bool)ais;
            }
            var tk = extGState.GetTextKnockoutFlag();
            if (tk != null) {
