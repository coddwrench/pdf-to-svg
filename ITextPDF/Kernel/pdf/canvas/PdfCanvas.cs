...
namespace IText.Kernel.Pdf.Canvas {
    ...
    public class PdfCanvas {
        ...
        /// in the content stream managed by this Canvas.
        /// Contrast with
        /// <see cref="SetTextMatrix(AffineTransform)"/>
        /// </remarks>
        ...
        /// <returns>current canvas</returns>
        public virtual PdfCanvas ConcatMatrix(double a, double b, double c, double d, double
             e, double f) {
        ...
        public virtual PdfCanvas SetFontAndSize(PdfFont font, float size) {
            ...
            var fontName = resources.AddFont(document, font);
        ...
        /// <param name="text">the text to write</param>
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas NewlineShowText(float wordSpacing, float charSpacing, string
             text) {
        ...
        /// <see cref="ConcatMatrix(PdfArray)"/>
        /// </remarks>
        /// <param name="a">operand 1,1 in the matrix.</param>
        /// <param name="b">operand 1,2 in the matrix.</param>
        /// <param name="c">operand 2,1 in the matrix.</param>
        ...
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas SetTextMatrix(float a, float b, float c, float d, float x
            , float y) {
        ...
        /// </param>
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas ShowText(GlyphLine text, IEnumerator<GlyphLine.GlyphLinePart
            > iterator) {
            ...
            var fontSize = currentGs.GetFontSize() / 1000f;
            var charSpacing = currentGs.GetCharSpacing();
            var scaling = currentGs.GetHorizontalScaling() / 100f;
            var glyphLineParts = EnumeratorToList(iterator);
            for (var partIndex = 0; partIndex < glyphLineParts.Count; ++partIndex) {
                var glyphLinePart = glyphLineParts[partIndex];
                ...
                var sub = glyphLinePart.start;
                for (var i = glyphLinePart.start; i < glyphLinePart.end; i++) {
                    var glyph = text.Get(i);
                    if (glyph.HasOffsets()) {
                        if (i - 1 - sub >= 0) {
                        ...
                        var xPlacement = float.NaN;
                        var yPlacement = float.NaN;
                        if (glyph.HasPlacement()) {
 {
                                float xPlacementAddition = 0;
                                var currentGlyphIndex = i;
                                var currentGlyph = text.Get(i);
                            ...
                            }
 {
        ...
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas CurveTo(double x1, double y1, double x2, double y2, double
             x3, double y3) {
        ...
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas Arc(double x1, double y1, double x2, double y2, double startAng
            , double extent) {
        ...
        public virtual PdfCanvas ArcContinuous(double x1, double y1, double x2, double y2, 
        ...
        public static IList<double[]> BezierArc(double x1, double y1, double x2, double y2, double startAng, double
            ...
            var rx = (x2 - x1) / 2f;
            var ry = (y2 - y1) / 2f;
            var halfAng = (fragAngle * Math.PI / 360.0);
            var kappa = Math.Abs(4.0 / 3.0 * (1.0 - Math.Cos(halfAng)) / Math.Sin(halfAng));
            IList<double[]> pointList = new List<double[]>();
            for (var iter = 0; iter < Nfrag; ++iter) {
                var theta0 = ((startAng + iter * fragAngle) * Math.PI / 180.0);
                var theta1 = ((startAng + (iter + 1) * fragAngle) * Math.PI / 180.0);
                var cos0 = Math.Cos(theta0);
                var cos1 = Math.Cos(theta1);
                var sin0 = Math.Sin(theta0);
                var sin1 = Math.Sin(theta1);
                if (fragAngle > 0.0) {
                    pointList.Add(new[] { x_cen + rx * cos0, y_cen - ry * sin0, x_cen + rx * (cos0 - kappa * sin0), y_cen
        ...
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas RoundRectangle(double x, double y, double width, double height
            , double radius) {
            if (width < 0) {
                x += width;
                width = -width;
        ...
        public virtual PdfCanvas PaintShading(PdfShading shading) {
            var shadingName = resources.AddShading(shading);
        ...
        /// </returns>
        [Obsolete(@"in favour of endPath(), which does exactly the same thing but is better named"
            )]
        public virtual PdfCanvas NewPath() {
            return EndPath();
        }
        ...
        /// <returns>current canvas.</returns>
        /// <seealso cref="PdfCanvasConstants.LineCapStyle">for possible values.</seealso>
        ...
        /// <returns>current canvas.</returns>
        /// <seealso cref="PdfCanvasConstants.LineJoinStyle">for possible values.</seealso>
        ...
        public virtual PdfCanvas SetLineDash(float unitsOn, float phase) {
            currentGs.SetDashPattern(GetDashPatternArray(new[] { unitsOn }, phase));
        ...
        public virtual PdfCanvas SetLineDash(float unitsOn, float unitsOff, float phase) {
            currentGs.SetDashPattern(GetDashPatternArray(new[] { unitsOn, unitsOff }, phase));
        ...
        public virtual PdfCanvas SetLineDash(float[] array, float phase) {
            ...
            var @out = contentStream.GetOutputStream();
            @out.WriteByte('[');
        ...
        /// <param name="fill">set fill color (<c>true</c>) or stroke color (<c>false</c>)</param>
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas SetColor(PdfColorSpace colorSpace, float[] colorValue, bool
             fill) {
        ...
        /// <param name="fill">set fill color (<c>true</c>) or stroke color (<c>false</c>)</param>
        /// <returns>current canvas.</returns>
        public virtual PdfCanvas SetColor(PdfColorSpace colorSpace, float[] colorValue, PdfPattern
             pattern, bool fill) {
            var setColorValueOnly = false;
            var oldColor = fill ? currentGs.GetFillColor() : currentGs.GetStrokeColor();
            var newColor = CreateColor(colorSpace, colorValue, pattern);
                            ...
                            if (colorSpace is PdfSpecialCs.Pattern) {
                                ...
                                if (colorSpace.GetPdfObject().IsIndirect()) {
                                    if (!setColorValueOnly) {
                                        var name = resources.AddColorSpace(colorSpace);
        ...
        public virtual PdfCanvas SetFillColorGray(float g) {
            return SetColor(gray, new[] { g }, true);
        }
        ...
        public virtual PdfCanvas SetStrokeColorGray(float g) {
            return SetColor(gray, new[] { g }, false);
        }
        ...
        public virtual PdfCanvas SetFillColorRgb(float r, float g, float b) {
            return SetColor(rgb, new[] { r, g, b }, true);
        }
        ...
        public virtual PdfCanvas SetStrokeColorRgb(float r, float g, float b) {
            return SetColor(rgb, new[] { r, g, b }, false);
        }
        ...
        public virtual PdfCanvas SetFillColorCmyk(float c, float m, float y, float k) {
            return SetColor(cmyk, new[] { c, m, y, k }, true);
        }
        ...
        public virtual PdfCanvas SetStrokeColorCmyk(float c, float m, float y, float k) {
            return SetColor(cmyk, new[] { c, m, y, k }, false);
        }
        ...
        /// <see cref="PdfPdfLayer.Layer.PdfLayer)"/>
        ...
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, useAddImageWithTransformationMatrix(iText.IO.Image.ImageData, float, float, float, float, float, float) or AddXObjectWithTransformationMatrix(iText.Kernel.Pdf.Xobject.PdfXObject, float, float, float, float, float, float) instead"
            )]
        ...
        /// <returns>the created imageXObject or null in case of in-line image (asInline = true)</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, useAddImageWithTransformationMatrix(iText.IO.Image.ImageData, float, float, float, float, float, float, bool) instead"
            )]
        public virtual PdfXObject AddImage(ImageData image, float a, float b, float c, float d, float e, float f, 
            bool asInline) {
             ...
             d, float e, float f, bool asInline)
        {
	        if (image.GetOriginalType() == ImageType.WMF) {
                var wmf = new WmfImageHelper(image);
                var xObject = wmf.CreateFormXObject(document);
                AddXObject(xObject, a, b, c, d, e, f);
                return xObject;
            }

	        var imageXObject = new PdfImageXObject(image);
        ...
        /// <returns>the created imageXObject or null in case of in-line image (asInline = true)</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, useAddImageFittedIntoRectangle(iText.IO.Image.ImageData, iText.Kernel.Geom.Rectangle, bool) orAddXObjectFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfXObject, iText.Kernel.Geom.Rectangle) instead"
            )]
        public virtual PdfXObject AddImage(ImageData image, Rectangle rect, bool asInline) {
        ...
        /// <returns>the created imageXObject or null in case of in-line image (asInline = true)</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        /// <seealso cref="PdfXObject.CalculateProportionallyFitPdfXObject.Xobject.PdfXObject, float, float, float)
        ///     "/>
        /// <seealso cref="PdfXObject.CalculateProportionallyFitRPdfXObject.Xobject.PdfXObject, float, float, float)
        ///     "/>
        public virtual PdfXObject AddImageFittedIntoRectangle(ImageData image, Rectangle rect, bool
             asInline) {
        ...
        /// <param name="asInline">true if to add image as in-line</param>
        /// <returns>the created imageXObject or null in case of in-line image (asInline = true)</returns>
        [Obsolete(@"will be removed in 7.2, use AddImageAt(iText.IO.Image.ImageData, float, float, bool) orAddXObjectAt(iText.Kernel.Pdf.Xobject.PdfXObject, float, float) instead"
            )]
        ...
        /// <returns>the created imageXObject or null in case of in-line image (asInline = true)</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, use AddImageFittedIntoRectangle(iText.IO.Image.ImageData, iText.Kernel.Geom.Rectangle, bool) or AddXObjectFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfXObject, iText.Kernel.Geom.Rectangle) instead"
            )]
        ...
        /// <see cref="AddImage(ImageData, float, float, float, bool)"/>
        /// </param>
        /// <returns>the created imageXObject or null in case of in-line image (asInline = true)</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, use AddImageFittedIntoRectangle(iText.IO.Image.ImageData, iText.Kernel.Geom.Rectangle, bool) or AddXObjectFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfXObject, iText.Kernel.Geom.Rectangle) instead"
        ...
        public virtual PdfCanvas AddXObjectWithTransformationMatrix(PdfXObject xObject, float
        ...
        /// <returns>the current canvas</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, useAddXObjectWithTransformationMatrix(iText.Kernel.Pdf.Xobject.PdfXObject, float, float, float, float, float, float) instead"
            )]
        public virtual PdfCanvas AddXObject(PdfXObject xObject, float a, float b, float c, 
        ...
        ///     </param>
        /// <returns>the current canvas</returns>
        [Obsolete(@"will be removed in 7.2, use AddXObjectAt(iText.Kernel.Pdf.Xobject.PdfXObject, float, float) instead"
            )]
        public virtual PdfCanvas AddXObject(PdfXObject xObject, float x, float y)
        {
	        if (xObject is PdfFormXObject) {
        ...
        /// <seealso cref="PdfXObject.CalculateProportionallyFitRPdfXObject.Xobject.PdfXObject, float, float, float)
        ///     "/>
        public virtual PdfCanvas AddXObjectFittedIntoRectangle(PdfXObject xObject, Rectangle
             rect)
        {
	        if (xObject is PdfFormXObject) {
        ...
        /// </param>
        /// <returns>the current canvas</returns>
        [Obsolete(@"will be removed in 7.2, use AddXObjectFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfXObject, iText.Kernel.Geom.Rectangle) instead"
            )]
        public virtual PdfCanvas AddXObject(PdfXObject xObject, Rectangle
             rect)
        {
	        if (xObject is PdfFormXObject) {
        ...
        /// <returns>the current canvas</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, useAddXObjectFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfXObject, iText.Kernel.Geom.Rectangle) (PdfXObject, Rectangle)} andiText.Kernel.Pdf.Xobject.PdfXObject.CalculateProportionallyFitRectangleWithWidth(iText.Kernel.Pdf.Xobject.PdfXObject, float, float, float) instead"
            )]
        public virtual PdfCanvas AddXObject(PdfXObject xObject, float x, float y, float width
            )
        {
	        if (xObject is PdfFormXObject) {
	        ...
	        if (xObject is PdfImageXObject) {
		        var rect = PdfXObject.CalculateProportionallyFitRectangleWithWidth(xObject, x, y, 
			        width);
        ...
        /// <see cref="AddXObject(PdfXObject, float, float, float)"/>
        /// </param>
        /// <returns>the current canvas</returns>
        /// <seealso cref="ConcatMatrix(double, double, double, double, double, double)"/>
        [Obsolete(@"will be removed in 7.2, useAddXObjectFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfXObject, iText.Kernel.Geom.Rectangle) (PdfXObject, Rectangle)} andiText.Kernel.Pdf.Xobject.PdfXObject.CalculateProportionallyFitRectangleWithHeight(iText.Kernel.Pdf.Xobject.PdfXObject, float, float, float) instead"
            )]
        public virtual PdfCanvas AddXObject(PdfXObject xObject, float x, float y, float height
            , bool dummy)
        {
	        if (xObject is PdfFormXObject) {
	        ...
	        if (xObject is PdfImageXObject) {
		        var rect = PdfXObject.CalculateProportionallyFitRectangleWithHeight(xObject, x, y, 
			        height);
		        return AddXObject(xObject, rect);
	        }
        ...
        public virtual PdfExtGState SetExtGState(PdfDictionary extGState) {
            var egs = new PdfExtGState(extGState);
            SetExtGState(egs);
            return egs;
        }
        ...
        public virtual PdfCanvas BeginMarkedContent(PdfName tag, PdfDictionary properties) {
            mcDepth++;
            var @out = contentStream.GetOutputStream().Write(tag).WriteSpace();
            if (properties == null) {
                @out.WriteBytes(BMC);
            }
            else {
        ...
        public virtual PdfCanvas OpenTag(TagReference tagReference) {
            ...
            var tag = new CanvasTag(tagReference.GetRole());
        ...
        public virtual PdfCanvas WriteLiteral(char c) {
            contentStream.GetOutputStream().WriteInteger(c);
            return this;
        }
        ...
        protected internal virtual void AddInlineImage(PdfImageXObject imageXObject, float a, float b, float c, float
            ...
            var os = contentStream.GetOutputStream();
            os.WriteBytes(BI);
            var imageBytes = imageXObject.GetPdfObject().GetBytes(false);
            foreach (var entry in imageXObject.GetPdfObject().EntrySet()) {
                var key = entry.Key;
        ...
        /// </param>
        /// <returns>current canvas</returns>
        private PdfCanvas AddFormWithTransformationMatrix(PdfFormXObject form, float a, float
             b, float c, float d, float e, float f, bool writeIdentityMatrix) {
            SaveState();
            if (writeIdentityMatrix || !IsIdentityMatrix(a, b, c, d, e, f)) {
                ConcatMatrix(a, b, c, d, e, f);
            }
            var name = resources.AddForm(form);
        ...
        /// <param name="f">an element of the transformation matrix</param>
        /// <returns>current canvas</returns>
        [Obsolete(@"will be removed in 7.2, useAddFormWithTransformationMatrix(iText.Kernel.Pdf.Xobject.PdfFormXObject, float, float, float, float, float, float, bool) instead"
            )]
        private PdfCanvas AddForm(PdfFormXObject form, float a, float b, float c, float d, 
            float e, float f) {
            SaveState();
            ConcatMatrix(a, b, c, d, e, f);
            var name = resources.AddForm(form);
        ...
        private PdfCanvas AddFormAt(PdfFormXObject form, float x, float y) {
            var bBox = PdfFormXObject.CalculateBBoxMultipliedByMatrix(form);
            var bBoxMin = new Vector(bBox.GetLeft(), bBox.GetBottom(), 1);
            var bBoxMax = new Vector(bBox.GetRight(), bBox.GetTop(), 1);
            var rectMin = new Vector(x, y, 1);
            var rectMax = new Vector(x + bBoxMax.Get(Vector.I1) - bBoxMin.Get(Vector.I1), y + bBoxMax.Get(Vector.I2
                ) - bBoxMin.Get(Vector.I2), 1);
            var result = CalculateTransformationMatrix(rectMin, rectMax, bBoxMin
                , bBoxMax);
        ...
        /// <returns>the current canvas</returns>
        [Obsolete(@"will be removed in 7.2, use AddFormAt(iText.Kernel.Pdf.Xobject.PdfFormXObject, float, float) instead"
            )]
        ...
        /// <returns>current canvas</returns>
        [Obsolete(@"will be removed in 7.2, use AddFormFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfFormXObject, iText.Kernel.Geom.Rectangle) instead"
            )]
        private PdfCanvas AddForm(PdfFormXObject form, float x, float y, float width) {
            var bbox = form.GetPdfObject().GetAsArray(PdfName.BBox);
            if (bbox == null) {
                throw new PdfException(PdfException.PdfFormXobjectHasInvalidBbox);
            }
            var formWidth = Math.Abs(bbox.GetAsNumber(2).FloatValue() - bbox.GetAsNumber(0).FloatValue());
            var formHeight = Math.Abs(bbox.GetAsNumber(3).FloatValue() - bbox.GetAsNumber(1).FloatValue());
        ...
        /// <param name="dummy">
        /// flag to note that the method works with the height parameter as opposed to the method
        /// <see cref="AddForm(PdfFormXObject, float, float, float)"/>
        /// </param>
        /// <returns>current canvas</returns>
        [Obsolete(@"will be removed in 7.2, use AddFormFittedIntoRectangle(iText.Kernel.Pdf.Xobject.PdfFormXObject, iText.Kernel.Geom.Rectangle) instead"
            )]
        private PdfCanvas AddForm(PdfFormXObject form, float x, float y, float height, bool
             dummy) {
            var bbox = form.GetPdfObject().GetAsArray(PdfName.BBox);
            if (bbox == null) {
                throw new PdfException(PdfException.PdfFormXobjectHasInvalidBbox);
        ...
        /// <returns>the current canvas</returns>
        private PdfCanvas AddFormFittedIntoRectangle(PdfFormXObject form, Rectangle
             rect) {
            var bBox = PdfFormXObject.CalculateBBoxMultipliedByMatrix(form);
            var bBoxMin = new Vector(bBox.GetLeft(), bBox.GetBottom(), 1);
            var bBoxMax = new Vector(bBox.GetRight(), bBox.GetTop(), 1);
            var rectMin = new Vector(rect.GetLeft(), rect.GetBottom(), 1);
            var rectMax = new Vector(rect.GetRight(), rect.GetTop(), 1);
            var result = CalculateTransformationMatrix(rectMin, rectMax, bBoxMin
                , bBoxMax);
            return AddFormWithTransformationMatrix(form, result[0], result[1], result[2], result[3], result[4], result
                [5], false);
        ...
        /// <param name="f">an element of the transformation matrix</param>
        /// <returns>current canvas</returns>
        private PdfCanvas AddImageWithTransformationMatrix(PdfXObject xObject, float a, float
        ...
        /// <returns>current canvas</returns>
        private PdfCanvas AddImageFittedIntoRectangle(PdfImageXObject image, Rectangle
             rect) {
            return AddImageWithTransformationMatrix(image, rect.GetWidth(), 0, 0, rect.GetHeight(), rect.GetX(), rect.
                GetY());
        }
        ...
        private void AddToPropertiesAndBeginLayer(IPdfOCG layer) {
            var name = resources.AddProperties(layer.GetPdfObject());
        ...
        private PdfArray GetDashPatternArray(float[] dashArray, float phase) {
            var dashPatternArray = new PdfArray();
            var dArray = new PdfArray();
            if (dashArray != null) {
                foreach (var fl in dashArray) {
        ...
        private void ApplyRotation(PdfPage page) {
            var rectangle = page.GetPageSizeWithRotation();
        ...
        private PdfCanvas DrawArc(double x1, double y1, double x2, double y2, double startAng
        ...
        private static PdfStream GetPageStream(PdfPage page) {
            var stream = page.GetLastContentStream();
