...
namespace IText.Kernel.Pdf.Canvas.Parser.Data
{
    ...
    public class TextRenderInfo : AbstractRenderInfo
    {
        ...
        private readonly PdfString _string;

        private string? _textCache = null;

        private readonly Matrix _textToUserSpaceTransformMatrix;

        private readonly Matrix _textMatrix;

        private float _unscaledWidth = float.NaN;

        /// <summary>
        /// Hierarchy of nested canvas tags for the text from the most inner (nearest to text) tag to the most outer.
        ///</summary>
        private readonly IList<CanvasTag> _canvasTagHierarchy;
        ...
        /// <param name="horizontalOffset">the unscaled horizontal offset of the character that this TextRenderInfo represents</param>
            ...
            : base(parent.GraphicsState)
        {
            ...
            _textToUserSpaceTransformMatrix = offsetMatrix.Multiply(parent._textToUserSpaceTransformMatrix);
            _textMatrix = offsetMatrix.Multiply(parent._textMatrix);
            _canvasTagHierarchy = parent._canvasTagHierarchy;
        }
        ...
        public virtual bool HasMcid(int mcid, bool checkTheTopmostLevelOnly)
        {
            ...
            {
                foreach (var tag in _canvasTagHierarchy)
                {
                    if (tag.HasMcid())
                    {
                        if (tag.GetMcid() == mcid)
        ...
        /// for the amount added by Rise.
        /// </remarks>
        /// <value>a LineSegment instance</value>
        public virtual LineSegment AscentLine
        {
            get
            {
                CheckGraphicsState();
        ...
        /// </remarks>
        /// <value>a LineSegment instance</value>
        public virtual LineSegment DescentLine
        {
            get
            {
                CheckGraphicsState();
        ...
        /// </value>
        public virtual IList<TextRenderInfo> CharacterRenderInfos
        {
            get
            {
                CheckGraphicsState();
                IList<TextRenderInfo> reuslt = new List<TextRenderInfo>(_string.GetValue().Length);
                var strings = SplitString(_string);
                float totalWidth = 0;
        ...
        {
            return _canvasTagHierarchy;
        }

        /// <returns>the unscaled (i.e. in Text space) width of the text</returns>
        public virtual float GetUnscaledWidth()
        {
            if (float.IsNaN(_unscaledWidth))
            {
                _unscaledWidth = GetPdfStringWidth(_string, false);
            }
        ...
        private float ConvertWidthFromTextSpaceToUserSpace(float width)
        {
            ...
            var userSpace = textSpace.TransformBy(_textToUserSpaceTransformMatrix);
