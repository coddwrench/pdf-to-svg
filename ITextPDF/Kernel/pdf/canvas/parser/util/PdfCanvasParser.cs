...
namespace IText.Kernel.Pdf.Canvas.Parser.Util
{
	...
	public class PdfCanvasParser
	{
		...
		public virtual IList<PdfObject> Parse(IList<PdfObject> ls)
		{
			...
			{
				ls.Add(ob);
				if (tokeniser.GetTokenType() == PdfTokenizer.PdfTokenType.Other)
				{
					if ("BI".Equals(ob.ToString()))
					{
						var inlineImageAsStream = InlineImageParsingUtils.Parse(this, currentResources.GetResource(PdfName.ColorSpace
							));
						ls.Clear();
		...
		public virtual PdfDictionary ReadDictionary()
		{
			...
			while (true)
			{
				...
				var name = new PdfName(tokeniser.GetStringValue());
				var obj = ReadObject();
				dic.Put(name, obj);
			}
			return dic;
		...
		public virtual PdfArray ReadArray()
		{
			...
			{
				var obj = ReadObject();
				if (!obj.IsArray() && tokeniser.GetTokenType() == PdfTokenizer.PdfTokenType.EndArray)
				{
					break;
				}
				if (tokeniser.GetTokenType() == PdfTokenizer.PdfTokenType.EndDic && obj.GetObjectType() != PdfObject.DICTIONARY
					)
				{
		...
		public virtual PdfObject ReadObject()
		{
			...
			switch (type)
			{
				...
				case PdfTokenizer.PdfTokenType.String:
					{
						var str = new PdfString(tokeniser.GetDecodedStringContent()).SetHexWriting(tokeniser.IsHexString());
						return str;
					}
