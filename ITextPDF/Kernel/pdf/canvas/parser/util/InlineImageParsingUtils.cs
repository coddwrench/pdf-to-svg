...
namespace IText.Kernel.Pdf.Canvas.Parser.Util {
    ...
    public sealed class InlineImageParsingUtils {
        private static readonly byte[] EI = { (byte)'E', (byte)'I' };
        ...
        public static PdfStream Parse(PdfCanvasParser ps, PdfDictionary colorSpaceDic) {
            var inlineImageDict = ParseDictionary(ps);
            var samples = ParseSamples(inlineImageDict, colorSpaceDic, ps);
            var inlineImageAsStreamObject = new PdfStream(samples);
        ...
        private static PdfDictionary ParseDictionary(PdfCanvasParser ps) {
            ...
            for (var key = ps.ReadObject(); key != null && !"ID".Equals(key.ToString()); key = ps.ReadObject()) {
                var value = ps.ReadObject();
                var resolvedKey = inlineImageEntryAbbreviationMap.Get((PdfName)key);
                if (resolvedKey == null) {
                    resolvedKey = (PdfName)key;
                }
                dict.Put(resolvedKey, GetAlternateValue(resolvedKey, value));
            }
            var ch = ps.GetTokeniser().Read();
            if (!PdfTokenizer.IsWhitespace(ch)) {
                throw new InlineImageParseException(PdfException.UnexpectedCharacter1FoundAfterIDInInlineImage
                    ).SetMessageParams(ch);
            }
            return dict;
        }
        ...
        private static PdfObject GetAlternateValue(PdfName key, PdfObject value) {
            if (key == PdfName.Filter) {
                if (value is PdfName) {
                    var altValue = inlineImageFilterAbbreviationMap.Get((PdfName)value);
                    if (altValue != null) {
                        return altValue;
                    }
                }
                ...
                if (key == PdfName.ColorSpace && value is PdfName) {
                    var altValue = inlineImageColorSpaceAbbreviationMap.Get((PdfName)value);
                    if (altValue != null) {
                        return altValue;
                    }
                }
        ...
        private static int GetComponentsPerPixel(PdfName colorSpaceName, PdfDictionary colorSpaceDic) {
            ...
            if (colorSpaceDic != null) {
                ...
                if (colorSpace != null) {
                    ...
                    var tempName = colorSpaceDic.GetAsName(colorSpaceName);
                    if (tempName != null) {
                        return GetComponentsPerPixel(tempName, colorSpaceDic);
                    }
                }
            }
            throw new InlineImageParseException(PdfException.UnexpectedColorSpace1).SetMessageParams
                (colorSpaceName);
        }
        ...
        private static int ComputeBytesPerRow(PdfDictionary imageDictionary, PdfDictionary colorSpaceDic) {
            ...
            var cpp = GetComponentsPerPixel(imageDictionary.GetAsName(PdfName.ColorSpace), colorSpaceDic);
            var w = wObj.IntValue();
            var bpc = bpcObj != null ? bpcObj.IntValue() : 1;
        ...
        private static byte[] ParseUnfilteredSamples(PdfDictionary imageDictionary, PdfDictionary colorSpaceDic, PdfCanvasParser
            ...
            var bytesToRead = ComputeBytesPerRow(imageDictionary, colorSpaceDic) * h.IntValue();
            ...
            var shouldBeWhiteSpace = tokeniser.Read();
            ...
            for (var i = startIndex; i < bytesToRead; i++) {
                var ch = tokeniser.Read();
                if (ch == -1) {
        ...
        private static bool ImageColorSpaceIsKnown(PdfDictionary imageDictionary, PdfDictionary colorSpaceDic) {
            var cs = imageDictionary.GetAsName(PdfName.ColorSpace);
