...
namespace IText.Kernel.Pdf.Canvas.Parser.Listener {
    ...
    public class FilteredEventListener : IEventListener {
        ...
        /// empty instance.
        /// Use
        /// <see cref="AttachEventListener{T}(IEventListener, IEventFilter[])"/>
        /// to add an event listener along with its filters.
        /// </remarks>
        public FilteredEventListener() {
            delegates = new List<IEventListener>();
        ...
        public virtual void EventOccurred(IEventData data, EventType type) {
            for (var i = 0; i < delegates.Count; i++) {
                var delegate_ = delegates[i];
                var filtersPassed = delegate_.GetSupportedEvents() == null || delegate_.GetSupportedEvents().Contains(type
                    );
