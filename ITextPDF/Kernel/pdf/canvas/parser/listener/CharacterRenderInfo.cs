...
namespace IText.Kernel.Pdf.Canvas.Parser.Listener {
    ...
    public class CharacterRenderInfo : TextChunk {
        ...
        /// </remarks>
        internal static StringConversionInfo MapString(IList<CharacterRenderInfo
            > cris) {
            IDictionary<int, int?> indexMap = new Dictionary<int, int?>();
            var sb = new StringBuilder();
            CharacterRenderInfo lastChunk = null;
            for (var i = 0; i < cris.Count; i++) {
                var chunk = cris[i];
                if (lastChunk == null) {
            ...
            var ret = new StringConversionInfo();
