...
namespace IText.Kernel.Pdf {
    ...
    public class PdfXrefTable {
        ...
        public virtual PdfIndirectReference Add(PdfIndirectReference reference) {
            ...
            var objNr = reference.GetObjNumber();
            count = Math.Max(count, objNr);
            EnsureCount(objNr);
        ...
        internal virtual void InitFreeReferencesList(PdfDocument pdfDocument) {
            ...
            var freeReferences = new SortedSet<int>();
            ...
            var prevFreeRef = xref[0];
            while (!freeReferences.IsEmpty()) {
                var currFreeRefObjNr = -1;
            ...
            while (!freeReferences.IsEmpty()) {
                var next = freeReferences.PollFirst();
                if (xref[next] == null) {
        ...
        internal virtual PdfIndirectReference CreateNewIndirectReference(PdfDocument document) {
            var reference = new PdfIndirectReference(document, ++count);
            Add(reference);
        ...
        protected internal virtual void FreeReference(PdfIndirectReference reference) {
            ...
            if (reference.CheckState(PdfObject.MUST_BE_FLUSHED)) {
                var logger = LogManager.GetLogger(typeof(PdfXrefTable));
        ...
        protected internal virtual void WriteXrefTableAndTrailer(PdfDocument document, PdfObject fileId, PdfObject
            ...
            var sections = CreateSections(document, false);
            var noModifiedObjects = (sections.Count == 0) || (xrefStream != null && sections.Count == 2 && sections[0
                ] == count && sections[1] == 1);
            ...
            var startxref = writer.GetCurrentPos();
            long xRefStmPos = -1;
            if (xrefStream != null) {
                ...
                var offsetSize = GetOffsetSize(Math.Max(startxref, Size()));
                ...
                if (document.properties.appendMode && !document.reader.hybridXref) {
                    ...
                    var lastXref = new PdfNumber(document.reader.GetLastXref());
                ...
                var xrefTable = document.GetXref();
                for (var k = 0; k < sections.Count; k += 2) {
                    var first = sections[k];
                    var len = sections[k + 1];
                    for (var i = first; i < first + len; i++) {
                        var reference = xrefTable.Get(i);
                        if (reference.IsFree()) {
            ...
            var needsRegularXref = !writer.IsFullCompression() || (document.properties.appendMode && document.reader.
                hybridXref);
            if (needsRegularXref) {
                ...
                for (var k = 0; k < sections.Count; k += 2) {
                    ...
                    for (var i = first; i < first + len; i++) {
                        var reference = xrefTable.Get(i);
                        var off = new StringBuilder("0000000000").Append(reference.GetOffset());
                        var gen = new StringBuilder("00000").Append(reference.GetGenNumber());
        ...
        private int GetOffsetSize(long startxref) {
            Debug.Assert(startxref >= 0 && startxref < (1L << 40));
        ...
        protected internal static void WriteKeyInfo(PdfDocument document) {
            var writer = document.GetWriter();
            var fingerPrint = document.GetFingerPrint();
            var platform = " for .NET";
            var versionInfo = document.GetVersionInfo();
            var k = versionInfo.GetKey();
            if (k == null) {
                k = "iText";
            }
            writer.WriteString(MessageFormatUtil.Format("%{0}-{1}{2}\n", k, versionInfo.GetRelease(), platform));
            foreach (var productInfo in fingerPrint.GetProducts()) {
        ...
        private void AppendNewRefToFreeList(PdfIndirectReference reference) {
            ...
            var lastFreeRef = freeReferencesLinkedList.Get(0);
        ...
        private PdfIndirectReference RemoveFreeRefFromList(int freeRefObjNr) {
            ...
            if (freeRefObjNr < 0) {
                ...
                foreach (var entry in freeReferencesLinkedList) {
            ...
            var freeRef = xref[freeRefObjNr];
            if (!freeRef.IsFree()) {
                return null;
            }
            var prevFreeRef = freeReferencesLinkedList.JRemove(freeRef.GetObjNumber());
            if (prevFreeRef != null) {
        ...
        private void ExtendXref(int capacity) {
            var newXref = new PdfIndirectReference[capacity];
