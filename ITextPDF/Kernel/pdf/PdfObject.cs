...
namespace IText.Kernel.Pdf
{
	public abstract class PdfObject
	{
		...
		protected internal PdfIndirectReference indirectReference;
		...
		public void Flush(bool canBeInObjStm)
		{
			...
			try
			{
				var document = GetIndirectReference().GetDocument();
				if (document != null)
				{
					if (document.IsAppendMode() && !IsModified())
					{
						var logger = LogManager.GetLogger(typeof(PdfObject));
		...
		{
			return indirectReference != null || CheckState(MUST_BE_INDIRECT);
		}
		...
		{
			var indirectReference = GetIndirectReference();
		...
		internal virtual PdfObject ProcessCopying(PdfDocument documentTo, bool allowDuplicating)
		{
			...
			{
				//copyTo case
				var writer = documentTo.GetWriter();
				if (writer == null)
				{
			...
			{
				var refTo = ((PdfIndirectReference)obj).GetRefersTo();
		...
		{
			var direct1 = obj1 != null && obj1.IsIndirectReference() ? ((PdfIndirectReference)obj1).GetRefersTo(
				true) : obj1;
			var direct2 = obj2 != null && obj2.IsIndirectReference() ? ((PdfIndirectReference)obj2).GetRefersTo(
				true) : obj2;
