...
namespace IText.Kernel.Pdf {
    public class PdfNameTree {
        ...
        public virtual IDictionary<string, PdfObject> GetNames() {
            ...
            var dictionary = catalog.GetPdfObject().GetAsDictionary(PdfName.Names);
            if (dictionary != null) {
                ...
                if (dictionary != null) {
                    ...
                    foreach (var key in keys) {
                        if (treeType.Equals(PdfName.Dests)) {
                            var arr = GetDestArray(items.Get(key));
                            if (arr != null) {
                                items.Put(key, arr);
                            }
                            else {
            ...
            if (treeType.Equals(PdfName.Dests)) {
                var destinations = catalog.GetPdfObject().GetAsDictionary(PdfName.Dests);
                if (destinations != null) {
                    var keys = destinations.KeySet();
                    foreach (var key in keys) {
                        var array = GetDestArray(destinations.Get(key));
                        if (array == null) {
                            continue;
                        }
        ...
        public virtual void AddEntry(string key, PdfObject value) {
            ...
            if (existingVal != null)
            {
	            ...
	            var logger = LogManager.GetLogger(typeof(PdfNameTree));
        ...
        public virtual PdfDictionary BuildTree() {
            ...
            var skip = NODE_SIZE;
            var kids = new PdfDictionary[(names.Length + NODE_SIZE - 1) / NODE_SIZE];
            for (var k = 0; k < kids.Length; ++k) {
                var offset = k * NODE_SIZE;
                var end = Math.Min(offset + NODE_SIZE, names.Length);
                var dic = new PdfDictionary();
            ...
            while (true) {
                ...
                for (var i = 0; i < tt; ++i) {
                    var offset = i * NODE_SIZE;
                    var end = Math.Min(offset + NODE_SIZE, top);
                    var dic = (PdfDictionary)new PdfDictionary().MakeIndirect(catalog.GetDocument());
                    var arr = new PdfArray();
        ...
        private PdfString IterateItems(PdfDictionary dictionary, IDictionary<string, PdfObject> items, PdfString leftOver
            ...
            var names = dictionary.GetAsArray(PdfName.Names);
            if (names != null) {
                ...
                if ((names = dictionary.GetAsArray(PdfName.Kids)) != null) {
                    for (var k = 0; k < names.Size(); k++) {
                        var kid = names.GetAsDictionary(k);
        ...
        private PdfArray GetDestArray(PdfObject obj) {
            ...
            if (obj.IsDictionary()) {
	            var arr = ((PdfDictionary)obj).GetAsArray(PdfName.D);
	            return arr;
            }
            return null;
        }
