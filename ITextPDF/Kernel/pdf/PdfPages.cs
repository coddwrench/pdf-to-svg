...
namespace IText.Kernel.Pdf {
    internal class PdfPages : PdfObjectWrapper<PdfDictionary> {
        ...
        private readonly PdfPages parent;
        ...
        public PdfPages(int from, PdfDocument pdfDocument, PdfPages parent)
            ...
            GetPdfObject().Put(PdfName.Kids, kids);
            GetPdfObject().Put(PdfName.Count, count);
            if (parent != null) {
        ...
        public PdfPages(int from, int maxCount, PdfDictionary pdfObject, PdfPages parent)
            ...
            SetForbidRelease();
            this.from = from;
            count = pdfObject.GetAsNumber(PdfName.Count);
            this.parent = parent;
            if (count == null) {
                count = new PdfNumber(1);
                pdfObject.Put(PdfName.Count, count);
            }
            else {
            ...
            kids = pdfObject.GetAsArray(PdfName.Kids);
        ...
        public virtual void AddPages(PdfPages pdfPages) {
        ...
        public virtual PdfPages GetParent() {
            return parent;
        }
