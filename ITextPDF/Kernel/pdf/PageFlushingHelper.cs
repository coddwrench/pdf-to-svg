...
namespace IText.Kernel.Pdf {
    ...
    public class PageFlushingHelper {
        private static readonly DeepFlushingContext pageContext;
        ...
        public virtual void AppendModeFlush(int pageNum) {
            ...
            var pageWasModified = page.GetPdfObject().IsModified();
            page.SetModified();
            release = true;
            ...
            var thumb = page.GetPdfObject().Get(PdfName.Thumb, false);
            FlushIfModified(thumb);
            var contents = page.GetPdfObject().Get(PdfName.Contents, false);
            if (contents is PdfIndirectReference) {
                if (contents.CheckState(PdfObject.MODIFIED) && !contents.CheckState(PdfObject.FLUSHED)) {
                    var contentsDirectObj = ((PdfIndirectReference)contents).GetRefersTo();
        ...
        private bool FlushPage(int pageNum) {
            ...
            var resourcesDict = page.InitResources(false);
            var resources = page.GetResources(false);
        ...
        private void InitCurrentLayers(PdfDocument pdfDoc) {
            if (pdfDoc.GetCatalog().IsOCPropertiesMayHaveChanged()) {
                var layers = pdfDoc.GetCatalog().GetOCProperties(false).GetLayers();
        ...
        private void FlushObjectRecursively(PdfObject obj, DeepFlushingContext context) {
            ...
            var avoidReleaseForIndirectObjInstance = false;
            if (obj.IsIndirectReference()) {
                var indRef = (PdfIndirectReference)obj;
            ...
            else
            {
	            ...
	            if (release && obj.IsIndirect()) {
		            ...
		            Debug.Assert(obj.IsReleaseForbidden() || obj.GetIndirectReference() == null);
        ...
        private void ArrayFlushIfModified(PdfArray contentsArr) {
            for (var i = 0; i < contentsArr.Size(); ++i) {
                var c = contentsArr.Get(i, false);
                FlushIfModified(c);
            }
        }
        ...
        private static DeepFlushingContext InitPageFlushingContext() {
            ...
            var NO_INNER_CONTEXTS = JavaCollectionsUtil.EmptyMap
                <PdfName, DeepFlushingContext>();
            // --- action dictionary context ---
            var actionContext = new DeepFlushingContext(
            ...
            var aaContext = new DeepFlushingContext(
                        // all inner entries leading to this context
                        actionContext);
            // ---
            // --- annotation dictionary context ---
            var annotInnerContexts = new LinkedDictionary
                <PdfName, DeepFlushingContext>();
            var annotsContext = new DeepFlushingContext(
            ...
            var sepInfoContext = new DeepFlushingContext(
            ...
            var bContext = new DeepFlushingContext(
                        // bead dict flushing blacklist
                        ALL_KEYS_IN_BLACK_LIST, NO_INNER_CONTEXTS);
            // ---
            // --- pres steps dictionary context ---
            var presStepsInnerContexts = new LinkedDictionary
                <PdfName, DeepFlushingContext>();
            var presStepsContext = new DeepFlushingContext(
            ...
            var pageInnerContexts = new LinkedDictionary
                <PdfName, DeepFlushingContext>();
            var pageContext = new DeepFlushingContext(new LinkedHashSet
        ...
        private class DeepFlushingContext {
            ...
            internal IDictionary<PdfName, DeepFlushingContext> innerContexts;

            internal DeepFlushingContext unconditionalInnerContext;

            public DeepFlushingContext(ICollection<PdfName> blackList, IDictionary<PdfName, DeepFlushingContext
                > innerContexts) {
            ...
            public DeepFlushingContext(DeepFlushingContext unconditionalInnerContext) {
                blackList = JavaCollectionsUtil.EmptySet<PdfName>();
                innerContexts = null;
            ...
            public virtual DeepFlushingContext GetInnerContextFor(PdfName key) {
