...
namespace IText.Kernel.Pdf.Layer {
    ...
    public class PdfOCProperties : PdfObjectWrapper<PdfDictionary> {
        ...
        public virtual void AddOCGRadioGroup(IList<PdfLayer> group) {
            ...
            if (ar.Size() != 0) {
                var d = GetPdfObject().GetAsDictionary(PdfName.D);
                if (d == null) {
                    d = new PdfDictionary();
                    GetPdfObject().Put(PdfName.D, d);
                }
                var radioButtonGroups = d.GetAsArray(PdfName.RBGroups);
        ...
        public virtual PdfObject FillDictionary(bool removeNonDocumentOcgs) {
            ...
            for (var i = 0; i < docOrder.Count; i++) {
                var layer = docOrder[i];
        ...
        private void RemoveNotRegisteredOcgs() {
            var dDict = GetPdfObject().GetAsDictionary(PdfName.D);
            var ocProperties = GetDocument().GetCatalog().GetPdfObject().GetAsDictionary(PdfName.OCProperties
                );
            ...
            var rbGroups = dDict.GetAsArray(PdfName.RBGroups);
            if (rbGroups != null) {
                foreach (var rbGroupObj in rbGroups) {
                    var rbGroup = (PdfArray)rbGroupObj;
                    for (var i = rbGroup.Size() - 1; i > -1; i--) {
        ...
        private void AddASEvent(PdfName @event, PdfName category) {
            ...
            foreach (var layer in layers) {
                if (layer.GetTitle() == null && !layer.GetPdfObject().IsFlushed()) {
                    var usage = layer.GetPdfObject().GetAsDictionary(PdfName.Usage);
            ...
            var d = GetPdfObject().GetAsDictionary(PdfName.D);
            var arras = d.GetAsArray(PdfName.AS);
            if (arras == null) {
                arras = new PdfArray();
                d.Put(PdfName.AS, arras);
            }
            var @as = new PdfDictionary();
            @as.Put(PdfName.Event, @event);
            var categoryArray = new PdfArray();
        ...
        private void ReadLayersFromDictionary() {
            var ocgs = GetPdfObject().GetAsArray(PdfName.OCGs);
            ...
            IDictionary<PdfIndirectReference, PdfLayer> layerMap = new SortedDictionary<PdfIndirectReference, PdfLayer
                >();
            for (var ind = 0; ind < ocgs.Size(); ind++) {
                var currentLayer = new PdfLayer((PdfDictionary)ocgs.GetAsDictionary(ind).MakeIndirect(GetDocument()));
            ...
            if (d != null && !d.IsEmpty()) {
                ...
                var locked = d.GetAsArray(PdfName.Locked);
                if (locked != null) {
                    for (var i = 0; i < locked.Size(); i++) {
                        var lockedLayer = locked.Get(i, false);
                ...
                var orderArray = d.GetAsArray(PdfName.Order);
            ...
            foreach (var layer in layerMap.Values) {
                if (!layer.IsOnPanel()) {
                    layers.Add(layer);
                }
            }
        ...
        private void ReadOrderFromDictionary(PdfLayer parent, PdfArray orderArray, IDictionary<PdfIndirectReference
            , PdfLayer> layerMap) {
            for (var i = 0; i < orderArray.Size(); i++) {
                var item = orderArray.Get(i);
                if (item.GetObjectType() == PdfObject.DICTIONARY) {
                    var layer = layerMap.Get(item.GetIndirectReference());
                    if (layer != null) {
                        layers.Add(layer);
                        layer.onPanel = true;
                        if (parent != null) {
                        ...
                        if (i + 1 < orderArray.Size() && orderArray.Get(i + 1).GetObjectType() == PdfObject.ARRAY) {
                            var nextArray = orderArray.GetAsArray(i + 1);
                    ...
                    if (item.GetObjectType() == PdfObject.ARRAY) {
                        var subArray = (PdfArray)item;
                        if (subArray.IsEmpty()) {
                            continue;
                        }
                        var firstObj = subArray.Get(0);
                        if (firstObj.GetObjectType() == PdfObject.STRING) {
                            var titleLayer = PdfLayer.CreateTitleSilent(((PdfString)firstObj).ToUnicodeString(), GetDocument());
        ...
        private string CreateUniqueName() {
            ...
            var configs = GetPdfObject().GetAsArray(PdfName.Configs);
            if (null != configs) {
                for (var i = 0; i < configs.Size(); i++) {
                    var alternateDictionary = configs.GetAsDictionary(i);
