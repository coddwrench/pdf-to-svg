...
namespace IText.Kernel.Pdf.Layer {
    ...
    public class PdfLayer : PdfObjectWrapper<PdfDictionary>, IPdfOCG {
        ...
        protected internal PdfLayer parent;

        protected internal IList<PdfLayer> children;
        ...
        public static PdfLayer CreateTitle(string title, PdfDocument document) {
            var layer = CreateTitleSilent(title, document);
        ...
        public virtual IList<PdfLayer> GetChildren() {
            return children == null ? null : new List<PdfLayer>(children);
        }
        ...
        protected internal static PdfLayer CreateTitleSilent(string title, PdfDocument document
            ) {
            if (title == null) {
                throw new ArgumentException("Invalid title argument");
            }
            var layer = new PdfLayer(document);
            layer.title = title;
            return layer;
        }
        ...
        protected internal virtual PdfDictionary GetUsage() {
            var usage = GetPdfObject().GetAsDictionary(PdfName.Usage);
            if (usage == null) {
