...
namespace IText.Kernel.Pdf.Tagging {
    ...
    internal class ParentTreeHandler {
        ...
        private IDictionary<PdfIndirectReference, PageMcrsContainer> pageToPageMcrs;
        ...
        public virtual PageMcrsContainer GetPageMarkedContentReferences(PdfPage page) {
        ...
        public virtual PdfMcr FindMcrByMcid(PdfDictionary pageDict, int mcid) {
            var pageMcrs = pageToPageMcrs.Get(pageDict.GetIndirectReference());
        ...
        public virtual int GetNextMcidForPage(PdfPage page) {
            var pageMcrs = GetPageMarkedContentReferences(page);
            if (pageMcrs == null || pageMcrs.GetPageContentStreamsMcrs().Count == 0) {
                return 0;
            }

            return pageMcrs.GetPageContentStreamsMcrs().Last().Key + 1;
        }
        ...
        public virtual void CreateParentTreeEntryForPage(PdfPage page) {
            var mcrs = GetPageMarkedContentReferences(page);
            if (mcrs == null) {
                return;
            }
        ...
        public virtual void SavePageStructParentIndexIfNeeded(PdfPage page) {
            var indRef = page.GetPdfObject().GetIndirectReference();
            ...
            var hasNonObjRefMcr = pageToPageMcrs.Get(indRef).GetPageContentStreamsMcrs().Count > 0 || pageToPageMcrs.
                Get(indRef).GetPageResourceXObjects().Count > 0;
            if (hasNonObjRefMcr) {
                pageToStructParentsInd.Put(indRef, GetOrCreatePageStructParentIndex(page));
            }
        }
        ...
        private void RegisterMcr(PdfMcr mcr, bool registeringOnInit) {
            var mcrPageIndRef = mcr.GetPageIndirectReference();
            if (mcrPageIndRef == null || (!(mcr is PdfObjRef) && mcr.GetMcid() < 0)) {
                var logger = LogManager.GetLogger(typeof(ParentTreeHandler));
                logger.Error(LogMessageConstant.ENCOUNTERED_INVALID_MCR);
                return;
            }
            var pageMcrs = pageToPageMcrs.Get(mcrPageIndRef);
            if (pageMcrs == null) {
            ...
            if ((stm = GetStm(mcr)) != null) {
                ...
                var structParent = xObjectStream.GetAsInt(PdfName.StructParents);
                ...
                if (mcr is PdfObjRef) {
                    var obj = ((PdfDictionary)mcr.GetPdfObject()).GetAsDictionary(PdfName.Obj);
                    ...
                    var n = obj.GetAsNumber(PdfName.StructParent);
                    if (n != null) {
        ...
        public virtual void UnregisterMcr(PdfMcr mcrToUnregister) {
            var pageDict = mcrToUnregister.GetPageObject();
            if (pageDict == null) {
                // invalid mcr, ignore
                return;
            }
            ...
            if (pageMcrs != null) {
                ...
                if ((stm = GetStm(mcrToUnregister)) != null) {
                    var xObjectReference = stm is PdfIndirectReference ? (PdfIndirectReference)stm : stm.GetIndirectReference
                        ();
                    ...
                    if (mcrToUnregister is PdfObjRef) {
                        var obj = ((PdfDictionary)mcrToUnregister.GetPdfObject()).GetAsDictionary(PdfName.Obj);
                        if (obj != null && !obj.IsFlushed()) {
                            var n = obj.GetAsNumber(PdfName.StructParent);
                            if (n != null) {
                        ...
                        foreach (var entry in pageMcrs.GetObjRefs()) {
        ...
        private void RegisterAllMcrs() {
            pageToPageMcrs = new Dictionary<PdfIndirectReference, PageMcrsContainer>();
            // we create new number tree and not using parentTree, because we want parentTree to be empty
            var parentTreeEntries = new PdfNumTree(structTreeRoot.GetDocument().GetCatalog(), 
                PdfName.ParentTree).GetNumbers();
            ICollection<PdfDictionary> mcrParents = new LinkedHashSet<PdfDictionary>();
            var maxStructParentIndex = -1;
            foreach (var entry in parentTreeEntries) {
                ...
                if (entryValue.IsDictionary()) {
                    ...
                    if (entryValue.IsArray()) {
                        var parentsArray = (PdfArray)entryValue;
                        for (var i = 0; i < parentsArray.Size(); ++i) {
                            var parent = parentsArray.GetAsDictionary(i);
                            if (parent != null) {
            ...
            foreach (PdfObject mcrParent in mcrParents) {
                var mcrParentStructElem = new PdfStructElem((PdfDictionary)mcrParent);
                foreach (var kid in mcrParentStructElem.GetKids()) {
                    if (kid is PdfMcr) {
        ...
        private bool UpdateStructParentTreeEntries(PdfPage page, PageMcrsContainer mcrs) {
            var res = false;
            foreach (var entry in mcrs.GetObjRefs()) {
                var mcr = entry.Value;
                var parentObj = ((PdfStructElem)mcr.GetParent()).GetPdfObject();
                if (!parentObj.IsIndirect()) {
                    continue;
                }
                var structParent = entry.Key;
                parentTree.AddEntry(structParent, parentObj);
                res = true;
            }
            int pageStructParentIndex;
            foreach (var entry in mcrs.GetPageResourceXObjects
                ()) {
                var xObjectRef = entry.Key;
        ...
        private bool UpdateStructParentTreeForContentStreamEntries(IDictionary<int, PdfMcr> mcrsOfContentStream, int
            ...
            var parentsOfMcrs = new PdfArray();
            var currentMcid = 0;
            foreach (var entry in mcrsOfContentStream) {
                var mcr = entry.Value;
                var parentObj = ((PdfStructElem)mcr.GetParent()).GetPdfObject();
                if (!parentObj.IsIndirect()) {
                    continue;
        ...
        private int GetOrCreatePageStructParentIndex(PdfPage page) {
            var structParentIndex = page.GetStructParentIndex();
        ...
        internal class PageMcrsContainer {
            ...
            */
            ...
            internal virtual void PutXObjectMcr(PdfIndirectReference xObjectIndRef, PdfMcr mcr) {
                var xObjectMcrs = pageResourceXObjects.Get(xObjectIndRef);
                if (xObjectMcrs == null) {
            ...
            internal virtual ICollection<PdfMcr> GetAllMcrsAsCollection() {
                ...
                foreach (var entry in pageResourceXObjects) {
