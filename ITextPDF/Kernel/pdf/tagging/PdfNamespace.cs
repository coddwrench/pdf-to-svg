...
namespace IText.Kernel.Pdf.Tagging {
    ...
    public class PdfNamespace : PdfObjectWrapper<PdfDictionary> {
        ...
        public virtual string GetNamespaceName() {
            var ns = GetPdfObject().GetAsString(PdfName.NS);
        ...
        public virtual PdfFileSpec GetSchema() {
            var schemaObject = GetPdfObject().Get(PdfName.Schema);
        ...
        /// <see cref="SetNamespaceRoleMap(PdfDictionary)"/>
        ...
        public virtual PdfNamespace AddNamespaceRoleMapping(string thisNsRole, string defaultNsRole
            ...
            var prevVal = GetNamespaceRoleMap(true).Put(PdfStructTreeRoot.ConvertRoleToPdfName(thisNsRole), PdfStructTreeRoot
        ...
        /// instance.
        /// </returns>
        public virtual PdfNamespace AddNamespaceRoleMapping(string thisNsRole, string targetNsRole
            , PdfNamespace targetNs) {
            var targetMapping = new PdfArray();
            targetMapping.Add(PdfStructTreeRoot.ConvertRoleToPdfName(targetNsRole));
            targetMapping.Add(targetNs.GetPdfObject());
            var prevVal = GetNamespaceRoleMap(true).Put(PdfStructTreeRoot.ConvertRoleToPdfName(thisNsRole), targetMapping
                );
            LogOverwritingOfMappingIfNeeded(thisNsRole, prevVal);
            SetModified();
            return this;
        ...
        private PdfDictionary GetNamespaceRoleMap(bool createIfNotExist) {
            var roleMapNs = GetPdfObject().GetAsDictionary(PdfName.RoleMapNS);
        ...
        private void LogOverwritingOfMappingIfNeeded(string thisNsRole, PdfObject prevVal) {
            if (prevVal != null) {
                var logger = LogManager.GetLogger(typeof(PdfNamespace));
                var nsNameStr = GetNamespaceName();
                if (nsNameStr == null) {
                    nsNameStr = "this";
                }
                logger.Warn(MessageFormatUtil.Format(LogMessageConstant.MAPPING_IN_NAMESPACE_OVERWRITTEN, thisNsRole
                    , nsNameStr));
            }
        }
    }
