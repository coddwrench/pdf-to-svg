...
namespace IText.Kernel.Pdf.Tagging {
    ...
    internal class StructureTreeCopier {
        ...
        public static void Move(PdfDocument document, PdfPage from, int insertBefore) {
            ...
            var fromNum = document.GetPageNumber(from);
            ...
            var mcrs = document.GetStructTreeRoot().GetPageMarkedContentReferences(from);
            if (mcrs != null) {
                foreach (var mcr in mcrs) {
                    var top = GetTopmostParent(mcr);
                    if (top != null) {
                        if (top.IsFlushed()) {
            ...
            var tops = document.GetStructTreeRoot().GetKidsObject();
            for (var i = 0; i < tops.Size(); ++i) {
                var top = tops.GetAsDictionary(i);
            ...
            foreach (var top in orderedTopsToMove) {
        ...
        private static int SeparateStructure(PdfDocument document, int startPage, int beforePage, int startPageStructTopIndex
            ...
            for (var i = startPage; i < beforePage; ++i) {
                var pageOfFirstHalf = document.GetPage(i);
                var pageMcrs = document.GetStructTreeRoot().GetPageMarkedContentReferences(pageOfFirstHalf
                    );
                if (pageMcrs != null) {
                    foreach (var mcr in pageMcrs) {
                        firstPartElems.Add(mcr.GetPdfObject());
                        var top = AddAllParentsToSet(mcr, firstPartElems);
            ...
            var lastTopBefore = startPageStructTopIndex - 1;
            for (var i = 0; i < tops.Size(); ++i) {
                var top = tops.GetAsDictionary(i);
                if (firstPartElems.Contains(top)) {
                    lastTopBefore = i;
                    var lastCloned = new LastClonedAncestor();
                    lastCloned.ancestor = top;
                    var topClone = top.Clone(ignoreKeysForClone);
            ...
            for (var i = 0; i < clonedTops.Count; ++i) {
        ...
        private static void CopyTo(PdfDocument destDocument, int insertBeforePage, IDictionary<PdfPage, PdfPage> page2page
            ...
            var insertIndex = SeparateStructure(destDocument, insertBeforePage);
        ...
        private static void CopyTo(PdfDocument destDocument, IDictionary<PdfPage, PdfPage> page2page, PdfDocument 
            ...
            var copiedStructure = CopyStructure(destDocument, page2page, callingDocument
                , copyFromDestDocument);
            var destStructTreeRoot = destDocument.GetStructTreeRoot();
            destStructTreeRoot.MakeIndirect(destDocument);
            foreach (var copied in copiedStructure.GetTopsList()) {
            ...
            if (!copyFromDestDocument) {
                ...
                var srcRoleMap = callingDocument.GetStructTreeRoot().GetRoleMap();
                var destRoleMap = destStructTreeRoot.GetRoleMap();
                foreach (var mappingEntry in srcRoleMap.EntrySet()) {
                    if (!destRoleMap.ContainsKey(mappingEntry.Key)) {
                        ...
                        if (!mappingEntry.Value.Equals(destRoleMap.Get(mappingEntry.Key))) {
                            var srcMapping = mappingEntry.Key + " -> " + mappingEntry.Value;
                            var destMapping = mappingEntry.Key + " -> " + destRoleMap.Get(mappingEntry.Key);
                            var logger = LogManager.GetLogger(typeof(StructureTreeCopier));
                            logger.Warn(string.Format(LogMessageConstant.ROLE_MAPPING_FROM_SOURCE_IS_NOT_COPIED_ALREADY_EXIST
        ...
        private static CopyStructureResult CopyStructure(PdfDocument destDocument, IDictionary
            <PdfPage, PdfPage> page2page, PdfDocument callingDocument, bool copyFromDestDocument) {
            var fromDocument = copyFromDestDocument ? destDocument : callingDocument;
            ...
            IDictionary<PdfDictionary, PdfDictionary> page2pageDictionaries = new Dictionary<PdfDictionary, PdfDictionary
                >();
            foreach (var page in page2page) {
                page2pageDictionaries.Put(page.Key.GetPdfObject(), page.Value.GetPdfObject());
                var mcrs = fromDocument.GetStructTreeRoot().GetPageMarkedContentReferences(page.Key);
                if (mcrs != null) {
                    foreach (var mcr in mcrs) {
                        if (mcr is PdfMcrDictionary || mcr is PdfObjRef) {
                            objectsToCopy.Add(mcr.GetPdfObject());
                        }
                        var top = AddAllParentsToSet(mcr, objectsToCopy);
                        if (top != null) {
                            if (top.IsFlushed()) {
            ...
            foreach (var kid in fromDocument.GetStructTreeRoot().GetKids()) {
                if (kid == null) {
                    continue;
                }
                var kidObject = ((PdfStructElem)kid).GetPdfObject();
                if (topsToFirstDestPage.ContainsKey(kidObject)) {
                    topsInOriginalOrder.Add(kidObject);
                }
            }
            var structElemCopyingParams = new StructElemCopyingParams
            ...
            foreach (var top in topsInOriginalOrder) {
                var copied = CopyObject(top, topsToFirstDestPage.Get(top), false, structElemCopyingParams);
        ...
        private static PdfDictionary CopyObject(PdfDictionary source, PdfDictionary destPage, bool parentChangePg, 
            StructElemCopyingParams copyingParams) {
            ...
            if (copyingParams.IsCopyFromDestDocument()) {
                ...
                var pg = source.GetAsDictionary(PdfName.Pg);
                if (pg != null) {
                    if (copyingParams.IsCopyFromDestDocument()) {
                        if (pg != destPage) {
                ...
                var obj = source.GetAsDictionary(PdfName.Obj);
                if (obj != null) {
                ...
                var nsDict = source.GetAsDictionary(PdfName.NS);
                if (nsDict != null) {
                    var copiedNsDict = CopyNamespaceDict(nsDict, copyingParams);
                    copied.Put(PdfName.NS, copiedNsDict);
                }
                var pg = source.GetAsDictionary(PdfName.Pg);
                if (pg != null) {
                    var pageAnalog = copyingParams.GetPage2page().Get(pg);
                    if (pageAnalog == null) {
                        pageAnalog = destPage;
                        parentChangePg = true;
                    }
            ...
            var k = source.Get(PdfName.K);
            if (k != null) {
                if (k.IsArray()) {
                    var kArr = (PdfArray)k;
                    var newArr = new PdfArray();
                    for (var i = 0; i < kArr.Size(); i++) {
                        var copiedKid = CopyObjectKid(kArr.Get(i), copied, destPage, parentChangePg, copyingParams);
                        if (copiedKid != null) {
                    ...
                    var copiedKid = CopyObjectKid(k, copied, destPage, parentChangePg, copyingParams);
                    if (copiedKid != null) {
        ...
        private static PdfObject CopyObjectKid(PdfObject kid, PdfDictionary copiedParent, PdfDictionary destPage, 
            bool parentChangePg, StructElemCopyingParams copyingParams) {
            if (kid.IsNumber()) {
                if (!parentChangePg) {
                ...
                if (kid.IsDictionary()) {
                    ...
                    if (copyingParams.GetObjectsToCopy().Contains(kidAsDict)) {
                        var hasParent = kidAsDict.ContainsKey(PdfName.P);
                        var copiedKid = CopyObject(kidAsDict, destPage, parentChangePg, copyingParams);
                        if (hasParent) {
                            ...
                            if (copiedKid.ContainsKey(PdfName.Obj)) {
                                ...
                                var contentItemObject = copiedKid.GetAsDictionary(PdfName.Obj);
        ...
        private static PdfDictionary CopyNamespaceDict(PdfDictionary srcNsDict, StructElemCopyingParams
             copyingParams) {
            var excludeKeys = JavaCollectionsUtil.SingletonList(PdfName.RoleMapNS);
            var toDocument = copyingParams.GetToDocument();
            var copiedNsDict = srcNsDict.CopyTo(toDocument, excludeKeys, false);
            copyingParams.AddCopiedNamespace(copiedNsDict);
            var srcRoleMapNs = srcNsDict.GetAsDictionary(PdfName.RoleMapNS);
            // if this src namespace was already copied (or in the process of copying) it will contain role map already
            var copiedRoleMap = copiedNsDict.GetAsDictionary(PdfName.RoleMapNS);
            if (srcRoleMapNs != null && copiedRoleMap == null) {
                copiedRoleMap = new PdfDictionary();
                copiedNsDict.Put(PdfName.RoleMapNS, copiedRoleMap);
                foreach (var entry in srcRoleMapNs.EntrySet()) {
                    PdfObject copiedMapping;
                    if (entry.Value.IsArray()) {
                        var srcMappingArray = (PdfArray)entry.Value;
                        if (srcMappingArray.Size() > 1 && srcMappingArray.Get(1).IsDictionary()) {
                            var copiedMappingArray = new PdfArray();
                            copiedMappingArray.Add(srcMappingArray.Get(0).CopyTo(toDocument));
                            var copiedNamespace = CopyNamespaceDict(srcMappingArray.GetAsDictionary(1), copyingParams);
                            copiedMappingArray.Add(copiedNamespace);
                            copiedMapping = copiedMappingArray;
                        }
                        else {
                            var logger = LogManager.GetLogger(typeof(StructureTreeCopier));
                            logger.Warn(string.Format(LogMessageConstant.ROLE_MAPPING_FROM_SOURCE_IS_NOT_COPIED_INVALID, entry
	                            .Key));
                            continue;
                        }
                    }
                    ...
                    var copiedRoleFrom = (PdfName)entry.Key.CopyTo(toDocument);
        ...
        private static void SeparateKids(PdfDictionary structElem, ICollection<PdfObject> firstPartElems, LastClonedAncestor
             lastCloned, PdfDocument document) {
            var k = structElem.Get(PdfName.K);
        ...
        private static void CloneParents(PdfDictionary structElem, LastClonedAncestor lastCloned
            , PdfDocument document) {
            if (lastCloned.ancestor != structElem) {
                var structElemClone = (PdfDictionary)structElem.Clone(ignoreKeysForClone).MakeIndirect(document);
                var currClone = structElemClone;
                var currElem = structElem;
                while (currElem.Get(PdfName.P) != lastCloned.ancestor) {
                    var parent = currElem.GetAsDictionary(PdfName.P);
                    var parentClone = (PdfDictionary)parent.Clone(ignoreKeysForClone).MakeIndirect(document);
        ...
        private static PdfDictionary AddAllParentsToSet(PdfMcr mcr, ICollection<PdfObject> set) {
            var allParents = RetrieveParents(mcr, true);
        ...
        private static IList<PdfDictionary> RetrieveParents(PdfMcr mcr, bool all) {
            ...
            var firstParent = mcr.GetParent();
            PdfDictionary previous = null;
            var current = firstParent is PdfStructElem ? ((PdfStructElem)firstParent).GetPdfObject() : null;
        ...
        private class StructElemCopyingParams {
            ...
            public StructElemCopyingParams(ICollection<PdfObject> objectsToCopy, PdfDocument toDocument, IDictionary<PdfDictionary
                ...
                copiedNamespaces = new LinkedHashSet<PdfObject>();
            }
