...
namespace IText.Kernel.Pdf.Tagging {
    public class PdfStructTreeRoot : PdfObjectWrapper<PdfDictionary>, IStructureNode {
        ...
        public static PdfName ConvertRoleToPdfName(string role) {
            var name = PdfName.staticNames.Get(role);
            if (name != null) {
                return name;
            }
        ...
        public virtual IList<IStructureNode> GetKids() {
            var k = GetPdfObject().Get(PdfName.K);
        ...
        public virtual PdfArray GetKidsObject() {
            PdfArray k = null;
            var kObj = GetPdfObject().Get(PdfName.K);
        ...
        public virtual void AddRoleMapping(string fromRole, string toRole) {
            ...
            var prevVal = roleMap.Put(ConvertRoleToPdfName(fromRole), ConvertRoleToPdfName(toRole));
            if (prevVal != null && prevVal is PdfName) {
                var logger = LogManager.GetLogger(typeof(PdfStructTreeRoot));
                logger.Warn(string.Format(LogMessageConstant.MAPPING_IN_STRUCT_ROOT_OVERWRITTEN, fromRole, prevVal
                    , toRole));
            }
        ...
        public virtual PdfDictionary GetRoleMap() {
            var roleMap = GetPdfObject().GetAsDictionary(PdfName.RoleMap);
            if (roleMap == null) {
        ...
        public virtual IList<PdfNamespace> GetNamespaces() {
            var namespacesArray = GetPdfObject().GetAsArray(PdfName.Namespaces);
            ...
            for (var i = 0; i < namespacesArray.Size(); ++i) {
        ...
        public virtual IList<PdfFileSpec> GetPronunciationLexiconsList() {
            var pronunciationLexicons = GetPdfObject().GetAsArray(PdfName.PronunciationLexicon);
            ...
            for (var i = 0; i < pronunciationLexicons.Size(); ++i) {
        ...
        public virtual ICollection<PdfMcr> GetPageMarkedContentReferences(PdfPage page) {
            var pageMcrs = GetParentTreeHandler().GetPageMarkedContentReferences(page);
        ...
        public override void Flush() {
            for (var i = 0; i < GetDocument().GetNumberOfPages(); ++i) {
        ...
        public virtual void Move(PdfPage fromPage, int insertBeforePage) {
            for (var i = 1; i <= GetDocument().GetNumberOfPages(); ++i) {
