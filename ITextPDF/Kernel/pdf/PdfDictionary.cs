...
namespace IText.Kernel.Pdf
{
	...
	public class PdfDictionary : PdfObject
	{
		...
		{
			return asDirects ? Values() : map.Values;
		}
		...
		public override string ToString()
		{
			if (!IsFlushed())
			{
				...
				{
					var indirectReference = entry.Value.GetIndirectReference();
					@string = @string + entry.Key + " " + (indirectReference == null ? entry.Value.ToString() : indirectReference
						.ToString()) + " ";
				}
				@string += ">>";
				return @string;
		...
		public virtual PdfDictionary Clone(IList<PdfName> excludeKeys)
		{
			...
			var dictionary = (PdfDictionary)Clone();
			map.AddAll(excluded);
			return dictionary;
		}
		...
		public virtual PdfDictionary CopyTo(PdfDocument document, IList<PdfName> excludeKeys, bool
			 allowDuplicating)
		{
			IDictionary<PdfName, PdfObject> excluded = new SortedDictionary<PdfName, PdfObject>();
			foreach (var key in excludeKeys)
			...
			var dictionary = (PdfDictionary)CopyTo(document, allowDuplicating
				);
			map.AddAll(excluded);
			return dictionary;
		}
		...
		{
			foreach (var key in other.KeySet())
				if (!ContainsKey(key))
		...
		protected internal override void CopyContent(PdfObject from, PdfDocument document)
		{
			...
			var dictionary = (PdfDictionary)from;
			foreach (var entry in dictionary.map)
