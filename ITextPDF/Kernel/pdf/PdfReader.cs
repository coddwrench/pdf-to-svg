...
namespace IText.Kernel.Pdf
{
	...
	public class PdfReader : IDisposable
	{
		...
		/// to be used.
		/// </summary>
		public static readonly StrictnessLevel DEFAULT_STRICTNESS_LEVEL = StrictnessLevel.LENIENT;
		...
		protected internal bool rebuiltXref;

		protected internal bool hybridXref;

		protected internal bool fixedXref;

		protected internal bool xrefStm;
		...
		/// <see cref="StrictnessLevel"/>
		/// </returns>
		public virtual StrictnessLevel GetStrictnessLevel()
		{
		...
		/// instance
		/// </returns>
		public virtual PdfReader SetStrictnessLevel(StrictnessLevel strictnessLevel)
		{
		...
		{
			var b = ReadStreamBytesRaw(stream);
			if (decode && b != null)
			{
		...
		{
			var type = stream.GetAsName(PdfName.Type);
			...
			try
			{
				...
				var embeddedStream = pdfDocument.DoesStreamBelongToEmbeddedFile(stream);
				if (decrypt != null && (!decrypt.IsEmbeddedFilesOnly() || embeddedStream))
				{
					var filter = stream.Get(PdfName.Filter, true);
					var skip = false;
					if (filter != null)
					{
		...
		{
			var bytes = ReadStreamBytes(stream, decode);
			...
			> filterHandlers)
		{
			...
			var filter = streamDictionary.Get(PdfName.Filter);
			...
			var memoryLimitsAwarenessRequired = null != memoryLimitsAwareHandler && memoryLimitsAwareHandler.IsMemoryLimitsAwarenessRequiredOnDecompression
				(filters);
			...
			var dp = new PdfArray();
			var dpo = streamDictionary.Get(PdfName.DecodeParms);
			...
			{
				var filterName = (PdfName)filters.Get(j);
				var filterHandler = filterHandlers.Get(filterName);
		...
		public virtual byte[] GetOriginalFileId()
		{
			...
			var id = trailer.GetAsArray(PdfName.ID);
		...
		{
			var version = tokens.CheckPdfHeader();
			try
			{
				headerPdfVersion = PdfVersion.FromString(version);
			}
			...
			{
				var logger = LogManager.GetLogger(typeof(PdfReader));
		...
		{
			var objectStreamNumber = objectStream.GetIndirectReference().GetObjNumber();
			var first = objectStream.GetAsNumber(PdfName.First).IntValue();
			var n = objectStream.GetAsNumber(PdfName.N).IntValue();
			var bytes = ReadStreamBytes(objectStream, true);
			...
			try
			{
				...
				for (var k = 0; k < n; ++k)
				{
					...
					if (tokens.GetTokenType() != PdfTokenizer.PdfTokenType.Number)
					{
						ok = false;
						break;
					}
				...
				for (var k = 0; k < n; ++k)
				{
					...
					tokens.NextToken();
					PdfObject obj;
					var reference = pdfDocument.GetXref().Get(objNumber[k]);
		...
		{
			var num = tokens.GetObjNr();
			if (num < 0)
			{
				return CreatePdfNullInstance(readAsDirect);
			}
			var table = pdfDocument.GetXref();
			var reference = table.Get(num);
			if (reference != null)
			{
				if (reference.IsFree())
				{
					var logger = LogManager.GetLogger(typeof(PdfReader));
					logger.Warn(MessageFormatUtil.Format(LogMessageConstant.INVALID_INDIRECT_REFERENCE, tokens.GetObjNr
						(), tokens.GetGenNr()));
					return CreatePdfNullInstance(readAsDirect);
				}
		...
		protected internal virtual PdfObject ReadObject(bool readAsDirect, bool objStm)
		{
			...
			var type = tokens.GetTokenType();
			switch (type)
			{
				case PdfTokenizer.PdfTokenType.StartDic:
					{
						var dict = ReadDictionary(objStm);
						...
						while (hasNext && tokens.GetTokenType() == PdfTokenizer.PdfTokenType.Comment);
						...
						tokens.Seek(pos);
						return dict;
						goto case PdfTokenizer.PdfTokenType.StartArray;
					}
				...
				case PdfTokenizer.PdfTokenType.String:
					{
						var pdfString = new PdfString(tokens.GetByteContent(), tokens.IsHexString());
				...
				case PdfTokenizer.PdfTokenType.Ref:
					{
						return ReadReference(readAsDirect);
					}

				case PdfTokenizer.PdfTokenType.EndOfFile:
					{
		...
		protected internal virtual PdfName ReadPdfName(bool readAsDirect)
		{
			if (readAsDirect)
			{
				var cachedName = PdfName.staticNames.Get(tokens.GetStringValue());
				if (cachedName != null)
				{
					return cachedName;
				}
		...
		protected internal virtual PdfDictionary ReadDictionary(bool objStm)
		{
			...
			{
				tokens.NextValidToken();
				if (tokens.GetTokenType() == PdfTokenizer.PdfTokenType.EndDic)
				{
					break;
				}
				if (tokens.GetTokenType() != PdfTokenizer.PdfTokenType.Name)
				{
					tokens.ThrowError(PdfException.DictionaryKey1IsNotAName, tokens.GetStringValue());
				}
				var name = ReadPdfName(true);
				var obj = ReadObject(true, objStm);
				if (obj == null)
				{
					if (tokens.GetTokenType() == PdfTokenizer.PdfTokenType.EndDic)
					{
					...
					if (tokens.GetTokenType() == PdfTokenizer.PdfTokenType.EndArray)
					{
		...
		protected internal virtual void ReadXref()
		{
			...
			var startxref = tokens.GetLongValue();
			lastXref = startxref;
			...
			var xrefSize = trailer.GetAsInt(PdfName.Size);
			if (xrefSize == null)
			{
		...
		protected internal virtual PdfDictionary ReadXrefSection()
		{
			...
			while (true)
			{
				...
				var start = tokens.GetIntValue();
				tokens.NextValidToken();
				if (tokens.GetTokenType() != PdfTokenizer.PdfTokenType.Number)
				{
					tokens.ThrowError(PdfException.NumberOfEntriesInThisXrefSubsectionNotFound);
				}
				var end = tokens.GetIntValue() + start;
				for (var num = start; num < end; num++)
				{
					tokens.NextValidToken();
					var pos = tokens.GetLongValue();
					tokens.NextValidToken();
					var gen = tokens.GetIntValue();
					tokens.NextValidToken();
					...
					var reference = xref.Get(num);
					var refReadingState = reference != null && reference.CheckState(PdfObject.READING) && reference.GetGenNumber
						() == gen;
					// for references that are added by xref table itself (like 0 entry)
					var refFirstEncountered = reference == null || !refReadingState && reference.GetDocument() == null;
			...
			var trailer = (PdfDictionary)ReadObject(false);
		...
		protected internal virtual bool ReadXrefStream(long ptr)
		{
			while (ptr != -1)
			{
				...
				if (!tokens.NextToken() || tokens.GetTokenType() != PdfTokenizer.PdfTokenType.Number)
				{
					return false;
				}
				...
				var size = ((PdfNumber)xrefStream.Get(PdfName.Size)).IntValue();
				PdfArray index;
				var obj = xrefStream.Get(PdfName.Index);
				if (obj == null)
				{
					index = new PdfArray();
				...
				var w = xrefStream.GetAsArray(PdfName.W);
				long prev = -1;
				...
				var b = ReadStreamBytes(xrefStream, true);
				var bptr = 0;
				var wc = new int[3];
				for (var k = 0; k < 3; ++k)
				{
		...
		protected internal virtual void FixXref()
		{
			...
			var xref = pdfDocument.GetXref();
			tokens.Seek(0);
			var buffer = new ByteBuffer(24);
			var lineTokeniser = new PdfTokenizer(new RandomAccessFileOrArray(new ReusableRandomAccessSource
				(buffer)));
			for (; ; )
			{
				var pos = tokens.GetPosition();
				...
				{
					var obj = PdfTokenizer.CheckObjectStart(lineTokeniser);
					if (obj == null)
					{
						continue;
					}
		...
		protected internal virtual void RebuildXref()
		{
			...
			for (; ; )
			{
				...
				if (buffer.Get(0) == 't')
				{
					...
					try
					{
						var dic = (PdfDictionary)ReadObject(false);
		...
		private void ReadDecryptObj()
		{
			...
			var enc = trailer.GetAsDictionary(PdfName.Encrypt);
			if (enc == null)
			{
				return;
			}
			encrypted = true;
			var filter = enc.GetAsName(PdfName.Filter);
		...
		{
			var tok = new PdfTokenizer(new RandomAccessFileOrArray(byteSource));
		...
		private PdfObject ReadObject(PdfIndirectReference reference, bool fixXref)
		{
			...
			try
			{
				...
				{
					var objectStream = (PdfStream)pdfDocument.GetXref().Get(reference.GetObjStreamNumber()).GetRefersTo(
						false);
				...
				if (reference.GetOffset() > 0)
				{
					...
					try
					{
						...
						tokens.NextValidToken();
						if (tokens.GetTokenType() != PdfTokenizer.PdfTokenType.Obj || tokens.GetObjNr() != reference.GetObjNumber() ||
		...
		private void CheckPdfStreamLength(PdfStream pdfStream)
		{
			...
			var start = pdfStream.GetOffset();
			var calc = false;
			var streamLength = 0;
			var pdfNumber = pdfStream.GetAsNumber(PdfName.Length);
			if (pdfNumber != null)
			{
			...
			if (calc)
			{
				...
				while (true)
				{
					...
					if (line.StartsWith(endobj))
					{
						...
						var index = s.IndexOf(endstream1, StringComparison.Ordinal);
						if (index >= 0)
						{
							pos = pos - 16 + index;
						}
		...
		public sealed class StrictnessLevel
		{
			...
			/// and ambiguities.
			/// </summary>
			public static readonly StrictnessLevel CONSERVATIVE = new StrictnessLevel(5000);

			/// <summary>
			/// The reading strictness level at which iText tries to recover from parsing
			/// errors if possible.
			/// </summary>
			public static readonly StrictnessLevel LENIENT = new StrictnessLevel(3000);
			...
			/// </returns>
			public bool IsStricter(StrictnessLevel compareWith)
			{
				return compareWith == null || levelValue > compareWith.levelValue;
			}
		}
