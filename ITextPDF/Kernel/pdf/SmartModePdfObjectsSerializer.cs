...
namespace IText.Kernel.Pdf
{
	internal class SmartModePdfObjectsSerializer
	{
		...
		public virtual SerializedObjectContent SerializeObject(PdfObject obj)
		{
			...
			var indRef = obj.GetIndirectReference();
			Debug.Assert(indRef != null);
			var serializedCache = indRef.GetDocument().serializedObjectsCache;
			var content = serializedCache.Get(indRef);
			if (content == null)
			{
			...
			serializedCache)
		{
			...
			if (obj.IsIndirectReference())
			{
				...
				var cached = serializedCache.Get(reference);
				if (cached != null)
				{
					bb.Append(cached);
					return;
