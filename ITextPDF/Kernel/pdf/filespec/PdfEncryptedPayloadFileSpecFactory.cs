...
namespace IText.Kernel.Pdf.Filespec
{
	public class PdfEncryptedPayloadFileSpecFactory
	{
		...
		public static PdfFileSpec Wrap(PdfDictionary dictionary)
		{
			...
			{
				LogManager.GetLogger(typeof(PdfEncryptedPayloadFileSpecFactory)).Error(LogMessageConstant.ENCRYPTED_PAYLOAD_FILE_SPEC_SHALL_HAVE_AFRELATIONSHIP_FILED_EQUAL_TO_ENCRYPTED_PAYLOAD
					);
			}
			var ef = dictionary.GetAsDictionary(PdfName.EF);
			...
			var fileSpec = PdfFileSpec.WrapFileSpecObject(dictionary);
		...
		{
			var result = "This embedded file is encrypted using " + ep.GetSubtype().GetValue();
