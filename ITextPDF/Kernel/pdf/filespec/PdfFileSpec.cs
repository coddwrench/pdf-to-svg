...
namespace IText.Kernel.Pdf.Filespec {
    public class PdfFileSpec : PdfObjectWrapper<PdfObject> {
        ...
        public static PdfFileSpec CreateExternalFileSpec(PdfDocument doc, string filePath
            ...
            return (PdfFileSpec)new PdfFileSpec(dict).MakeIndirect
                (doc);
        }
        ...
        /// containing the file specification of the file
        /// </returns>
        public static PdfFileSpec CreateEmbeddedFileSpec(PdfDocument doc, byte[] fileStore
            , string description, string fileDisplay, PdfName mimeType, PdfDictionary fileParameter, PdfName afRelationshipValue
            ) {
            var stream = (PdfStream)new PdfStream(fileStore).MakeIndirect(doc);
            var @params = new PdfDictionary();
        ...
        /// containing the file specification of the file
        /// </returns>
        public static PdfFileSpec CreateEmbeddedFileSpec(PdfDocument doc, string filePath
            , string description, string fileDisplay, PdfName mimeType, PdfDictionary fileParameter, PdfName afRelationshipValue
            ) {
            var stream = new PdfStream(doc, UrlUtil.OpenStream(UrlUtil.ToURL(filePath)));
            var @params = new PdfDictionary();
            if (fileParameter != null) {
                @params.MergeDifferent(fileParameter);
            }
        ...
        /// containing the file specification of the file
        /// </returns>
        public static PdfFileSpec CreateEmbeddedFileSpec(PdfDocument doc, Stream @is, string
             description, string fileDisplay, PdfName mimeType, PdfDictionary fileParameter, PdfName afRelationshipValue
            ) {
            var stream = new PdfStream(doc, @is);
            var @params = new PdfDictionary();
            if (fileParameter != null) {
                @params.MergeDifferent(fileParameter);
            }
        ...
        /// containing the file specification of the file
        /// </returns>
        private static PdfFileSpec CreateEmbeddedFileSpec(PdfDocument doc, PdfStream stream
            , string description, string fileDisplay, PdfName mimeType, PdfName afRelationshipValue) {
            var dict = new PdfDictionary();
        ...
        public virtual PdfImageXObject GetThumbnailImage() {
            var thumbnailStream = ((PdfDictionary)GetPdfObject()).GetAsStream(PdfName.Thumb);
