...
namespace IText.Kernel.Pdf
{
	...
	public class PdfStream : PdfDictionary
	{
		...
		public PdfStream(byte[] bytes, int compressionLevel)
		{
			...
			{
				outputStream = new PdfOutputStream(new ByteArrayOutputStream(bytes.Length));
		...
		internal PdfStream(long offset, PdfDictionary keys)
		{
			...
			this.offset = offset;
			PutAll(keys);
			var length = GetAsNumber(PdfName.Length);
			if (length == null)
			{
				this.length = 0;
			}
		...
		public virtual byte[] GetBytes(bool decoded)
		{
			...
			if (inputStream != null)
			{
				LogManager.GetLogger(typeof(PdfStream)).Warn("PdfStream was created by InputStream." + "getBytes() always returns null in this case"
					);
				return null;
			}
			byte[] bytes = null;
			...
			{
				Debug.Assert(outputStream.GetOutputStream() is ByteArrayOutputStream, "Invalid OutputStream: ByteArrayByteArrayOutputStream expected"
					);
				try
				{
			...
			else
			{
				if (GetIndirectReference() != null)
				{
					...
					var reader = GetIndirectReference().GetReader();
					if (reader != null)
					{
						try
						{
		...
		public virtual void SetData(byte[] bytes, bool append)
		{
			...
			var outputStreamIsUninitialized = outputStream == null;
		...
		protected internal override void CopyContent(PdfObject from, PdfDocument document)
		{
			...
			var stream = (PdfStream)from;
			Debug.Assert(inputStream == null, "Try to copy the PdfStream that has been just created."
				);
