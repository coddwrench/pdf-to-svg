...
namespace IText.Kernel.Pdf.Navigation {
    public class PdfStructureDestination : PdfDestination {
        ...
        public static PdfStructureDestination CreateXYZ(PdfStructElem elem, float left
        ...
        public static PdfStructureDestination CreateFitR(PdfStructElem elem, float left
        ...
        private static PdfStructureDestination Create(PdfStructElem elem, PdfName type
            , float left, float bottom, float right, float top, float zoom) {
            return new PdfStructureDestination().Add(elem).Add(type).Add(left).Add(bottom)
                .Add(right).Add(top).Add(zoom);
        }

        public override PdfObject GetDestinationPage(IDictionary<string, PdfObject> names) {
            var firstObj = ((PdfArray)GetPdfObject()).Get(0);
            if (firstObj.IsDictionary()) {
                var structElem = new PdfStructElem((PdfDictionary)firstObj);
                while (true) {
                    var kids = structElem.GetKids();
                    var firstKid = kids.Count > 0 ? kids[0] : null;
