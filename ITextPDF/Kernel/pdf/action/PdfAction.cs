...
namespace IText.Kernel.Pdf.Action {
    ...
    public class PdfAction : PdfObjectWrapper<PdfDictionary> {
        ...
        /// </param>
        /// <returns>created action</returns>
        public static PdfAction CreateGoToE(PdfDestination destination, bool newWindow, PdfTarget
             targetDictionary) {
        ...
        /// </param>
        /// <returns>created action</returns>
        public static PdfAction CreateGoToE(PdfFileSpec fileSpec, PdfDestination destination
            , bool newWindow, PdfTarget targetDictionary) {
            var action = new PdfAction().Put(PdfName.S, PdfName.
            ...
            if (destination != null) {
                ...
                LogManager.GetLogger(typeof(PdfAction)).Warn(LogMessageConstant.EMBEDDED_GO_TO_DESTINATION_NOT_SPECIFIED
                    );
            }
        ...
        /// <returns>created action</returns>
        public static PdfAction CreateThread(PdfFileSpec fileSpec, PdfObject destinationThread
            , PdfObject bead) {
            var action = new PdfAction().Put(PdfName.S, PdfName.
        ...
        public static PdfAction CreateURI(string uri, bool isMap) {
            return new PdfAction().Put(PdfName.S, PdfName.URI).Put(PdfName.URI, new PdfString(
        ...
        /// <returns>created action</returns>
        public static PdfAction CreateSound(PdfStream sound, float volume, bool synchronous
            , bool repeat, bool mix) {
            if (volume < -1 || volume > 1) {
                throw new ArgumentException("volume");
            }
            return new PdfAction().Put(PdfName.S, PdfName.Sound).Put(PdfName.Sound, sound).Put
        ...
        /// <see cref="PdfName.Play"/>
        /// ,
        /// <see cref="PdfName.Stop"/>
        /// ,
        /// <see cref="PdfName.Pause"/>
        /// ,
        /// <see cref="PdfName.Resume"/>
        /// </param>
        /// <returns>created annotation</returns>
        public static PdfAction CreateMovie(PdfAnnotation annotation, string title, PdfName
             operation) {
            var action = new PdfAction().Put(PdfName.S, PdfName.
        ...
        public static PdfAction CreateHide(PdfAnnotation[] annotations, bool hidden) {
            return new PdfAction().Put(PdfName.S, PdfName.Hide).Put(PdfName.T, GetPdfArrayFromAnnotationsList
        ...
        /// <returns>created action</returns>
        public static PdfAction CreateHide(string[] text, bool hidden) {
            return new PdfAction().Put(PdfName.S, PdfName.Hide).Put(PdfName.T, GetArrayFromStringList
        ...
        /// <see cref="PdfName.NextPage"/>
        /// ,
        /// <see cref="PdfName.PrevPage"/>
        /// ,
        /// <see cref="PdfName.FirstPage"/>
        /// ,
        /// <see cref="PdfName.LastPage"/>
        /// </param>
        /// <returns>created action</returns>
        ...
        public static PdfAction CreateSetOcgState(IList<PdfActionOcgState> states, bool preserveRb
            ...
            var stateArr = new PdfArray();
            foreach (var state in states) {
                stateArr.AddAll(state.GetObjectList());
            }
            return new PdfAction().Put(PdfName.S, PdfName.SetOCGState).Put(PdfName.State, stateArr
        ...
        /// <returns>created action</returns>
        public static PdfAction CreateRendition(string file, PdfFileSpec fileSpec, string 
            mimeType, PdfAnnotation screenAnnotation) {
            return new PdfAction().Put(PdfName.S, PdfName.Rendition).Put(PdfName.OP, new PdfNumber
        ...
        public static PdfAction CreateJavaScript(string javaScript) {
            return new PdfAction().Put(PdfName.S, PdfName.JavaScript).Put(PdfName.JS, new PdfString
                (javaScript));
        }
        ...
        public static PdfAction CreateSubmitForm(string file, object[] names, int flags) {
            ...
            var urlFileSpec = new PdfDictionary();
        ...
        /// </param>
        public static void SetAdditionalAction(PdfObjectWrapper<PdfDictionary> wrapper, PdfName key, PdfAction
             action) {
            PdfDictionary dic;
            var obj = wrapper.GetPdfObject().Get(PdfName.AA);
            var aaExists = obj != null && obj.IsDictionary();
            if (aaExists) {
        ...
        private static void ValidateRemoteDestination(PdfDestination destination) {
            ...
            if (destination is PdfExplicitDestination) {
                var firstObj = ((PdfArray)destination.GetPdfObject()).Get(0);
                ...
                if (destination is PdfStructureDestination) {
                    ...
                    var firstObj = ((PdfArray)destination.GetPdfObject()).Get(0);
                    if (firstObj.IsDictionary()) {
                        var structElemObj = (PdfDictionary)firstObj;
                        var id = structElemObj.GetAsString(PdfName.ID);
                        if (id == null) {
                        ...
                        LogManager.GetLogger(typeof(PdfAction)).Warn(LogMessageConstant.STRUCTURE_ELEMENT_REPLACED_BY_ITS_ID_IN_STRUCTURE_DESTINATION
                        );
        ...
        /// <param name="destination">
        /// the
        /// <see cref="PdfDestination">destination</see>
        /// to be validated
        /// </param>
        public static void ValidateNotRemoteDestination(PdfDestination destination) {
            if (destination is PdfExplicitRemoteGoToDestination) {
                LogManager.GetLogger(typeof(PdfAction)).Warn(LogMessageConstant.INVALID_DESTINATION_TYPE
                    );
            }
            else {
                if (destination is PdfExplicitDestination) {
