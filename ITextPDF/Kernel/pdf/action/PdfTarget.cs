...
namespace IText.Kernel.Pdf.Action {
    ...
    public class PdfTarget : PdfObjectWrapper<PdfDictionary> {
        ...
        private static PdfTarget Create(PdfName r) {
            var pdfTarget = new PdfTarget(new PdfDictionary());
        ...
        public static PdfTarget CreateChildTarget(string namedDestination, string annotationIdentifier
            ...
            return Create(PdfName.C).Put(PdfName.P, new PdfString(namedDestination))
        ...
        public static PdfTarget CreateChildTarget(int pageNumber, int annotationIndex) {
            return Create(PdfName.C).Put(PdfName.P, new PdfNumber(pageNumber - 1)).Put
        ...
        /// <returns>this object wrapper</returns>
        public virtual PdfTarget SetAnnotation(PdfFileAttachmentAnnotation pdfAnnotation, 
            PdfDocument pdfDocument) {
            var page = pdfAnnotation.GetPage();
            if (null == page) {
        ...
        public virtual PdfFileAttachmentAnnotation GetAnnotation(PdfDocument pdfDocument) {
            var pValue = GetPdfObject().Get(PdfName.P);
            PdfPage page = null;
            if (pValue is PdfNumber) {
                ...
                if (pValue is PdfString) {
                    var destsTree = pdfDocument.GetCatalog().GetNameTree(PdfName.Dests);
                    var dests = destsTree.GetNames();
                    var pdfArray = (PdfArray)dests.Get(((PdfString)pValue).GetValue());
                    if (null != pdfArray) {
            ...
            var aValue = GetPdfObject().Get(PdfName.A);
            ...
            if (null == resultAnnotation) {
                var logger = LogManager.GetLogger(typeof(PdfTarget));
        ...
        public virtual PdfTarget GetTarget() {
            var targetDictObject = GetPdfObject().GetAsDictionary(PdfName.T);
