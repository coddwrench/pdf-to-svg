...
namespace IText.Kernel.Log {
    ...
    /// </remarks>
    [Obsolete(@"will be removed in next major release, please use iText.Kernel.Counter.EventCounterHandler instead."
        )]
    public class CounterManager {
        ...
        public virtual IList<ICounter> GetCounters(Type cls) {
            var result = new List<ICounter>();
            foreach (var factory in factories) {
                var counter = factory.GetCounter(cls);
                if (counter != null) {
                    result.Add(counter);
                }
            }
