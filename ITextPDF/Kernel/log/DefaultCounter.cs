...
namespace IText.Kernel.Log {
    ...
    /// </summary>
    [Obsolete(@"will be removed in next major release, please use iText.Kernel.Counter.DefaultEventCounter instead."
        )]
        ...
        private void PlusOne() {
            if (++count > repeat_level) {
                if (Version.IsAGPLVersion() || Version.IsExpired()) {
                    var message = JavaUtil.GetStringForBytes(message_1, EncodingUtil.ISO_8859_1
                        );
                    if (Version.IsExpired()) {
                        message = JavaUtil.GetStringForBytes(message_2, EncodingUtil.ISO_8859_1);
                    }
                    level++;
                    if (level == 1) {
