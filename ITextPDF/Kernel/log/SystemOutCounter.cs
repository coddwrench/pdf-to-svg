...
namespace IText.Kernel.Log {
    ...
    /// </summary>
    [Obsolete(@"will be removed in the next major release, please use iText.Kernel.Counter.SystemOutEventCounter instead."
        )]
    public class SystemOutCounter : ICounter {
        ...
        public virtual void OnDocumentRead(long size) {
            Console.Out.WriteLine(MessageFormatUtil.Format("[{0}] {1} bytes read", name, size));
        }

        public virtual void OnDocumentWritten(long size) {
            Console.Out.WriteLine(MessageFormatUtil.Format("[{0}] {1} bytes written", name, size));
        }
    }
}
