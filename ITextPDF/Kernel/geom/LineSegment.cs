...
namespace IText.Kernel.Geom {
    ...
    public class LineSegment {
        ...
        public virtual Rectangle GetBoundingRectangle() {
            var x1 = GetStartPoint().Get(Vector.I1);
            var y1 = GetStartPoint().Get(Vector.I2);
            var x2 = GetEndPoint().Get(Vector.I1);
            var y2 = GetEndPoint().Get(Vector.I2);
        ...
        public virtual LineSegment TransformBy(Matrix m) {
            var newStart = startPoint.Cross(m);
            var newEnd = endPoint.Cross(m);
        ...
        public virtual bool ContainsPoint(Vector point) {
            ...
            var diff1 = point.Subtract(startPoint);
            if (diff1.Get(0) < 0 || diff1.Get(1) < 0 || diff1.Get(2) < 0) {
                return false;
            }
            var diff2 = endPoint.Subtract(point);
