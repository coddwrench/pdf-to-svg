    ...
    public sealed class ShapeTransformUtil {
        ...
        public static Path TransformPath(Path path, Matrix ctm) {
            ...
            foreach (var subpath in path.GetSubpaths()) {
                var transformedSubpath = TransformSubpath(subpath, ctm);
        ...
        private static Subpath TransformSubpath(Subpath subpath, Matrix ctm) {
            ...
            foreach (var segment in subpath.GetSegments()) {
                var transformedSegment = TransformSegment(segment, ctm);
        ...
        private static IShape TransformSegment(IShape segment, Matrix ctm) {
            ...
            var newBasePoints = TransformPoints(ctm, basePoints.ToArray(new Point[basePoints.Count]));
            IShape newSegment;
        ...
        private static Point[] TransformPoints(Matrix ctm, params Point[] points) {
            try {
                var t = new AffineTransform(ctm.Get(Matrix.I11), ctm.Get(Matrix.I12), ctm.Get(Matrix.I21), ctm
