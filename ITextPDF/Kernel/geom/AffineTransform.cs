...
namespace IText.Kernel.Geom {
    public class AffineTransform {
        ...
        public virtual int GetTransformType() {
            ...
            var dx = m00 * m00 + m10 * m10;
            var dy = m01 * m01 + m11 * m11;
            if (dx != dy) {
        ...
        internal virtual AffineTransform Multiply(AffineTransform t1, AffineTransform
             t2) {
            return new AffineTransform(t1.m00 * t2.m00 + t1.m10 * t2.m01, t1.m00 * t2.m10 + t1.m10 *
        ...
        public virtual AffineTransform CreateInverse() {
            ...
            return new AffineTransform(m11 / det, -m10 / det, -m01 / det, m00 / det, (m01 * m12 - m11
        ...
        public virtual void Transform(Point[] src, int srcOff, Point[] dst, int dstOff, int length) {
            while (--length >= 0) {
                var srcPoint = src[srcOff++];
                var x = srcPoint.GetX();
                var y = srcPoint.GetY();
                var dstPoint = dst[dstOff];
                if (dstPoint == null) {
                    dstPoint = new Point();
                }
        ...
        public virtual void InverseTransform(double[] src, int srcOff, double[] dst, int dstOff, int length) {
            ...
            while (--length >= 0) {
                var x = src[srcOff++] - m02;
                var y = src[srcOff++] - m12;
        ...
        public virtual void InverseTransform(float[] src, int srcOff, float[] dst, int dstOff, int length) {
            ...
            while (--length >= 0) {
                var x = (float)(src[srcOff++] - m02);
                var y = (float)(src[srcOff++] - m12);
        ...
        public virtual AffineTransform Clone() {
            return (AffineTransform) MemberwiseClone();
        }
