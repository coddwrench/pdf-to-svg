    ...
    public abstract class AbstractLinearGradientBuilder {
        ...
        public virtual AbstractLinearGradientBuilder SetSpreadMethod(GradientSpreadMethod gradientSpreadMethod) {
            if (spreadMethod != null) {
                ...
                spreadMethod = GradientSpreadMethod.NONE;
            }
            return this;
        }
        ...
        public virtual IList<GradientColorStop> GetColorStops() {
            return new List<GradientColorStop>(stops);
        }
        ...
        public virtual Color BuildColor(Rectangle targetBoundingBox, AffineTransform contextTransform, PdfDocument
            ...
            var baseCoordinatesVector = GetGradientVector(targetBoundingBox, contextTransform);
            if (baseCoordinatesVector == null || stops.IsEmpty()) {
                // Can not create gradient color with 0 stops or null coordinates vector
                return null;
            }
            // evaluate actual coordinates and transformation
            var shadingTransform = new AffineTransform();
            if (contextTransform != null) {
                shadingTransform.Concatenate(contextTransform);
            }
            var gradientTransformation = GetCurrentSpaceToGradientVectorSpaceTransformation(targetBoundingBox
                , contextTransform);
            if (gradientTransformation != null) {
                try {
                    ...
                    LogManager.GetLogger(GetType()).Error(LogMessageConstant.UNABLE_TO_INVERT_GRADIENT_TRANSFORMATION
                        );
                }
            }
            var axial = CreateAxialShading(baseCoordinatesVector, stops, spreadMethod, targetBoundingBox
                );
            if (axial == null) {
                return null;
            }
            var shading = new PdfPattern.Shading(axial);
        ...
        protected internal static double[] EvaluateCoveringDomain(Point[] coords, Rectangle toCover) {
            ...
            var scale = 1d / (coords[0].Distance(coords[1]));
            var sin = -(coords[1].GetY() - coords[0].GetY()) * scale;
            var cos = (coords[1].GetX() - coords[0].GetX()) * scale;
            ...
            var rectanglePoints = toCover.ToPointsArray();
            var minX = transform.Transform(rectanglePoints[0], null).GetX();
            var maxX = minX;
            for (var i = 1; i < rectanglePoints.Length; ++i) {
                var currentX = transform.Transform(rectanglePoints[i], null).GetX();
        ...
        protected internal static Point[] CreateCoordinatesForNewDomain(double[] newDomain, Point[] baseVector) {
            var xDiff = baseVector[1].GetX() - baseVector[0].GetX();
            var yDiff = baseVector[1].GetY() - baseVector[0].GetY();
        ...
        private static PdfShading.Axial CreateAxialShading(Point[] baseCoordinatesVector, IList<GradientColorStop>
            ...
            var baseVectorLength = baseCoordinatesVector[1].Distance(baseCoordinatesVector[0]);
            var stopsToConstruct = NormalizeStops(stops, baseVectorLength);
            double[] coordinatesDomain = { 0, 1 };
            ...
            if (baseVectorLength < ZERO_EPSILON || stopsToConstruct.Count == 1) {
                ...
                actualCoordinates = new[] { new Point(targetBoundingBox.GetLeft(), targetBoundingBox.GetBottom()), new 
                    Point(targetBoundingBox.GetRight(), targetBoundingBox.GetBottom()) };
                var lastColorStop = stopsToConstruct[stopsToConstruct.Count - 1];
                ...
                if (spreadMethod == GradientSpreadMethod.REPEAT || spreadMethod == GradientSpreadMethod.REFLECT) {
                    ...
                    if (spreadMethod == GradientSpreadMethod.PAD) {
                        ...
                        // none case
                        var firstStopOffset = stopsToConstruct[0].GetOffset();
                        var lastStopOffset = stopsToConstruct[stopsToConstruct.Count - 1].GetOffset();
                ...
                Debug.Assert(coordinatesDomain[0] <= coordinatesDomain[1]);
        ...
        private static IList<GradientColorStop> NormalizeStops(IList<GradientColorStop> toNormalize, double baseVectorLength
            ...
            var result = CopyStopsAndNormalizeAbsoluteOffsets(toNormalize, baseVectorLength);
        ...
        private static void NormalizeHintsOffsets(IList<GradientColorStop> result) {
            ...
            for (var i = 0; i < result.Count - 1; ++i) {
                var stopColor = result[i];
                if (stopColor.GetHintOffsetType() == GradientColorStop.HintOffsetType.RELATIVE_ON_GRADIENT) {
                    var currentStopOffset = stopColor.GetOffset();
                    var nextStopOffset = result[i + 1].GetOffset();
                    if (currentStopOffset != nextStopOffset) {
                        var hintOffset = (stopColor.GetHintOffset() - currentStopOffset) / (nextStopOffset - currentStopOffset);
        ...
        private static void NormalizeAutoStops(IList<GradientColorStop> toNormalize) {
            Debug.Assert(toNormalize[0].GetOffsetType() == GradientColorStop.OffsetType.RELATIVE);
            var firstAutoStopIndex = 1;
            var firstStopColor = toNormalize[0];
            var prevOffset = firstStopColor.GetHintOffsetType() == GradientColorStop.HintOffsetType.RELATIVE_ON_GRADIENT
                 ? firstStopColor.GetHintOffset() : firstStopColor.GetOffset();
            for (var i = 1; i < toNormalize.Count; ++i) {
                var currentStop = toNormalize[i];
                if (currentStop.GetOffsetType() == GradientColorStop.OffsetType.AUTO) {
                    if (currentStop.GetHintOffsetType() == GradientColorStop.HintOffsetType.RELATIVE_ON_GRADIENT) {
                        var hintOffset = currentStop.GetHintOffset();
                    ...
                    if (firstAutoStopIndex < i) {
                        ...
                        var offset = currentStop.GetOffset();
            ...
            if (firstAutoStopIndex < toNormalize.Count) {
                var lastStopOffset = Math.Max(1, prevOffset);
        ...
        private static void NormalizeAutoStops(IList<GradientColorStop> toNormalizeList, int fromIndex, int toIndex
            ...
            Debug.Assert(toIndex >= fromIndex);
            var intervalsCount = Math.Min(toIndex, toNormalizeList.Count - 1) - fromIndex + 1;
            var offsetShift = (nextOffset - prevOffset) / intervalsCount;
            var currentOffset = prevOffset;
            for (var i = fromIndex; i < toIndex; ++i) {
                currentOffset += offsetShift;
                var currentAutoStop = toNormalizeList[i];
                Debug.Assert(currentAutoStop.GetOffsetType() == GradientColorStop.OffsetType.AUTO);
        ...
        private static IList<GradientColorStop> CopyStopsAndNormalizeAbsoluteOffsets(IList<GradientColorStop> toNormalize
            ...
            var lastUsedOffset = double.NegativeInfinity;
            IList<GradientColorStop> copy = new List<GradientColorStop>(toNormalize.Count);
            foreach (var stop in toNormalize) {
                var offset = stop.GetOffset();
                var offsetType = stop.GetOffsetType();
                ...
                var result = new GradientColorStop(stop, offset, offsetType);
                var hintOffset = stop.GetHintOffset();
                var hintOffsetType = stop.GetHintOffsetType();
        ...
        private static void AdjustStopsForPadIfNeeded(IList<GradientColorStop> stopsToConstruct, double[] coordinatesDomain
            ...
            var firstStop = stopsToConstruct[0];
            ...
            var lastStop = stopsToConstruct[stopsToConstruct.Count - 1];
        ...
        private static IList<GradientColorStop> AdjustNormalizedStopsToCoverDomain(IList<GradientColorStop> normalizedStops
            ...
            var lastColorStop = normalizedStops[normalizedStops.Count - 1];
            var originalIntervalEnd = lastColorStop.GetOffset();
            var originalIntervalStart = normalizedStops[0].GetOffset();
            var originalIntervalLength = originalIntervalEnd - originalIntervalStart;
            ...
            var startIntervalsShift = Math.Floor((targetDomain[0] - originalIntervalStart) / originalIntervalLength
                );
            var iterationOffset = originalIntervalStart + (originalIntervalLength * startIntervalsShift);
            var isIterationInverse = spreadMethod == GradientSpreadMethod.REFLECT && Math.Abs(startIntervalsShift) % 
                2 != 0;
            var currentIterationIndex = isIterationInverse ? normalizedStops.Count - 1 : 0;
            var lastComputedOffset = iterationOffset;
            while (lastComputedOffset <= targetDomain[1]) {
                var currentStop = normalizedStops[currentIterationIndex];
                ...
                var computedStop = new GradientColorStop(currentStop, lastComputedOffset, GradientColorStop.OffsetType
                    .RELATIVE);
                ...
                if (isIterationInverse) {
                    var nextColor = normalizedStops[currentIterationIndex];
        ...
        private static PdfFunction ConstructFunction(IList<GradientColorStop> toConstruct) {
            var functionsAmount = toConstruct.Count - 1;
            var bounds = new double[functionsAmount - 1];
            IList<PdfFunction> type2Functions = new List<PdfFunction>(functionsAmount);
            GradientColorStop currentStop;
            var nextStop = toConstruct[0];
            var domainStart = nextStop.GetOffset();
            for (var i = 1; i < functionsAmount; ++i) {
            ...
            type2Functions.Add(ConstructSingleGradientSegmentFunction(currentStop, nextStop));
            var domainEnd = nextStop.GetOffset();
            var encode = new double[functionsAmount * 2];
            for (var i = 0; i < encode.Length; i += 2) {
                encode[i] = 0d;
                encode[i + 1] = 1d;
            }
            return new PdfFunction.Type3(new PdfArray(new[] { domainStart, domainEnd }), null, type2Functions, 
        ...
        private static PdfFunction ConstructSingleGradientSegmentFunction(GradientColorStop from, GradientColorStop
             to) {
            var exponent = 1d;
            var fromColor = from.GetRgbArray();
            var toColor = to.GetRgbArray();
            if (from.GetHintOffsetType() == GradientColorStop.HintOffsetType.RELATIVE_BETWEEN_COLORS) {
                var hintOffset = from.GetHintOffset();
            ...
            return new PdfFunction.Type2(new PdfArray(new[] { 0f, 1f }), null, new PdfArray(fromColor), new PdfArray
                (toColor), new PdfNumber(exponent));
        }

        private static PdfArray CreateCoordsPdfArray(Point[] coordsPoints) {
            Debug.Assert(coordsPoints != null && coordsPoints.Length == 2);
            return new PdfArray(new[] { coordsPoints[0].GetX(), coordsPoints[0].GetY(), coordsPoints[1].GetX(), 
                coordsPoints[1].GetY() });
        }
    }
}
