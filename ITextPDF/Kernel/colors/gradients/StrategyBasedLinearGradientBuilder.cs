...
namespace IText.Kernel.Colors.Gradients {
    ...
    public class StrategyBasedLinearGradientBuilder : AbstractLinearGradientBuilder {
        ...
        private GradientStrategy gradientStrategy = GradientStrategy
            .TO_BOTTOM;

        private bool isCentralRotationAngleStrategy;
        ...
        public virtual StrategyBasedLinearGradientBuilder SetGradientDirectionAsCentralRotationAngle
            (double radians) {
            rotateVectorAngle = radians;
            isCentralRotationAngleStrategy = true;
            return this;
        }
        ...
        /// <returns>the current builder instance</returns>
        public virtual StrategyBasedLinearGradientBuilder SetGradientDirectionAsStrategy
            (GradientStrategy gradientStrategy) {
            this.gradientStrategy = gradientStrategy != null ? gradientStrategy : GradientStrategy
                .TO_BOTTOM;
            isCentralRotationAngleStrategy = false;
            return this;
        }
        ...
        public virtual GradientStrategy GetGradientStrategy() {
        ...
        protected internal override Point[] GetGradientVector(Rectangle targetBoundingBox, AffineTransform contextTransform
            ...
            return isCentralRotationAngleStrategy ? BuildCentralRotationCoordinates(targetBoundingBox, rotateVectorAngle
                ) : BuildCoordinatesWithGradientStrategy(targetBoundingBox, gradientStrategy);
        }

        private static Point[] BuildCoordinatesWithGradientStrategy(Rectangle targetBoundingBox, GradientStrategy
             gradientStrategy) {
        ...
        private static Point[] BuildCentralRotationCoordinates(Rectangle targetBoundingBox, double angle) {
            double xCenter = targetBoundingBox.GetX() + targetBoundingBox.GetWidth() / 2;
            var rotateInstance = AffineTransform.GetRotateInstance(angle, xCenter, targetBoundingBox.GetY(
        ...
        private static Point[] BuildToCornerCoordinates(Rectangle targetBoundingBox, Point gradientCenterLineRightCorner
            ...
            var transform = BuildToCornerTransform(new Point(targetBoundingBox.GetX() + targetBoundingBox.
        ...
        private static AffineTransform BuildToCornerTransform(Point center, Point gradientCenterLineRightCorner) {
            var scale = 1d / (center.Distance(gradientCenterLineRightCorner));
            var sin = (gradientCenterLineRightCorner.GetY() - center.GetY()) * scale;
            var cos = (gradientCenterLineRightCorner.GetX() - center.GetX()) * scale;
            ...
            var m02 = center.GetX() * (1d - cos) + center.GetY() * sin;
            var m12 = center.GetY() * (1d - cos) - center.GetX() * sin;
        ...
        private static Point[] BuildCoordinates(Rectangle targetBoundingBox, AffineTransform transformation) {
            double xCenter = targetBoundingBox.GetX() + targetBoundingBox.GetWidth() / 2;
            var start = transformation.Transform(new Point(xCenter, targetBoundingBox.GetBottom()), null);
            var end = transformation.Transform(new Point(xCenter, targetBoundingBox.GetTop()), null);
            Point[] baseVector = { start, end };
            var targetDomain = EvaluateCoveringDomain(baseVector, targetBoundingBox);
