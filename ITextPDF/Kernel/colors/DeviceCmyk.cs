...
namespace IText.Kernel.Colors {
    ...
    public class DeviceCmyk : Color {
        ...
        public static readonly DeviceCmyk CYAN = new DeviceCmyk(100, 0, 0, 
            0);

        /// <summary>Predefined magenta DeviceCmyk color</summary>
        public static readonly DeviceCmyk MAGENTA = new DeviceCmyk(0, 100, 
            0, 0);

        /// <summary>Predefined yellow DeviceCmyk color</summary>
        public static readonly DeviceCmyk YELLOW = new DeviceCmyk(0, 0, 100
            , 0);

        /// <summary>Predefined black DeviceCmyk color</summary>
        public static readonly DeviceCmyk BLACK = new DeviceCmyk(0, 0, 0, 
            100);
        ...
        public DeviceCmyk(float c, float m, float y, float k)
            : base(new PdfDeviceCs.Cmyk(), new[] { c > 1 ? 1 : (c > 0 ? c : 0), m > 1 ? 1 : (m > 0 ? m : 0), y >
                                                                                                             1 ? 1 : (y > 0 ? y : 0), k > 1 ? 1 : (k > 0 ? k : 0) }) {
            if (c > 1 || c < 0 || m > 1 || m < 0 || y > 1 || y < 0 || k > 1 || k < 0) {
                var LOGGER = LogManager.GetLogger(typeof(DeviceCmyk));
        ...
        public static DeviceCmyk MakeLighter(DeviceCmyk cmykColor) {
            var rgbEquivalent = ConvertCmykToRgb(cmykColor);
            var lighterRgb = DeviceRgb.MakeLighter((rgbEquivalent));
        ...
        public static DeviceCmyk MakeDarker(DeviceCmyk cmykColor) {
            var rgbEquivalent = ConvertCmykToRgb(cmykColor);
            var darkerRgb = DeviceRgb.MakeDarker(rgbEquivalent);
