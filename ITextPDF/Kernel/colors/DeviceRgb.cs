...
namespace IText.Kernel.Colors {
    ...
    public class DeviceRgb : Color {
        ...
        public DeviceRgb(float r, float g, float b)
            : base(new PdfDeviceCs.Rgb(), new[] { r > 1 ? 1 : (r > 0 ? r : 0), g > 1 ? 1 : (g > 0 ? g : 0), b > 
                                                                                                            1 ? 1 : (b > 0 ? b : 0) }) {
            if (r > 1 || r < 0 || g > 1 || g < 0 || b > 1 || b < 0) {
                var LOGGER = LogManager.GetLogger(typeof(DeviceRgb));
        ...
        public DeviceRgb(System.Drawing.Color color)
            ...
            if (color.A != 255) {
                var LOGGER = LogManager.GetLogger(typeof(DeviceRgb));
                LOGGER.Warn(MessageFormatUtil.Format(LogMessageConstant.COLOR_ALPHA_CHANNEL_IS_IGNORED, color.A));
            }
        }
        ...
        public static DeviceRgb MakeLighter(DeviceRgb rgbColor) {
            var r = rgbColor.GetColorValue()[0];
            var g = rgbColor.GetColorValue()[1];
            var b = rgbColor.GetColorValue()[2];
