...
namespace IText.Kernel.Colors {
    ...
    public class Color {
                ...
                if (colorSpace is PdfCieBasedCs) {
                    ...
                    if (colorSpace is PdfSpecialCs) {
                        if (colorSpace is PdfSpecialCs.Separation) {
                            var separation = (PdfSpecialCs.Separation)colorSpace;
                            ...
                            if (colorSpace is PdfSpecialCs.DeviceN) {
                                ...
                                var deviceN = (PdfSpecialCs.DeviceN)colorSpace;
        ...
        public static DeviceRgb ConvertCmykToRgb(DeviceCmyk cmykColor) {
            var cyanComp = 1 - cmykColor.GetColorValue()[0];
            var magentaComp = 1 - cmykColor.GetColorValue()[1];
            var yellowComp = 1 - cmykColor.GetColorValue()[2];
            var blackComp = 1 - cmykColor.GetColorValue()[3];
            var r = cyanComp * blackComp;
            var g = magentaComp * blackComp;
            var b = yellowComp * blackComp;
        ...
        public static DeviceCmyk ConvertRgbToCmyk(DeviceRgb rgbColor) {
            var redComp = rgbColor.GetColorValue()[0];
            var greenComp = rgbColor.GetColorValue()[1];
            var blueComp = rgbColor.GetColorValue()[2];
            var k = 1 - Math.Max(Math.Max(redComp, greenComp), blueComp);
            var c = (1 - redComp - k) / (1 - k);
            var m = (1 - greenComp - k) / (1 - k);
            var y = (1 - blueComp - k) / (1 - k);
        ...
        /// <summary>
        /// Returns the
        /// <see cref="PdfColorSpace">color space</see>
        ...
        public override int GetHashCode() {
            var result = colorSpace != null ? colorSpace.GetPdfObject().GetHashCode() : 0;
