...
namespace IText.Kernel.Counter {
    ...
    /// <see cref="OnEvent(IEvent, IMetaInfo, Type)
    ///     "/>
    /// method.
    /// <para />
    ...
    public class EventCounterHandler {
        ...
        private static readonly EventCounterHandler instance = new EventCounterHandler
            ();
        ...
        public virtual void OnEvent(IEvent @event, IMetaInfo metaInfo, Type caller) {
            ...
            foreach (var factory in factories.Keys) {
                var counter = factory.GetCounter(caller);
                if (counter != null) {
