    ...
    public class PdfSplitter {
        ...
        public virtual IList<PdfDocument> SplitBySize(long size) {
            ...
            while (currentPage <= numOfPages) {
                var nextRange = GetNextRange(currentPage, numOfPages, size);
                splitRanges.Add(nextRange);
                var allPages = nextRange.GetQualifyingPageNums(numOfPages);
        ...
        /// </param>
        public virtual void SplitByPageNumbers(IList<int> pageNumbers, IDocumentReadyListener documentReady
            ) {
            var currentPageNumber = 1;
            for (var ind = 0; ind <= pageNumbers.Count; ind++) {
                var nextPageNumber = ind == pageNumbers.Count ? pdfDocument.GetNumberOfPages() + 1 : pageNumbers[ind];
                if (ind == 0 && nextPageNumber == 1) {
                    continue;
                }
                var currentPageRange = new PageRange().AddPageSequence(currentPageNumber, nextPageNumber - 1);
                var currentDocument = CreatePdfDocument(currentPageRange);
        ...
        private sealed class _IDocumentReadyListener_171 : IDocumentReadyListener {
        ...
        /// You can close this document in this listener, for instance.
        /// </param>
        public virtual void SplitByPageCount(int pageCount, IDocumentReadyListener documentReady) {
            for (var startPage = 1; startPage <= pdfDocument.GetNumberOfPages(); startPage += pageCount) {
                var endPage = Math.Min(startPage + pageCount - 1, pdfDocument.GetNumberOfPages());
                var currentPageRange = new PageRange().AddPageSequence(startPage, endPage);
                var currentDocument = CreatePdfDocument(currentPageRange);
        ...
        private sealed class _IDocumentReadyListener_208 : IDocumentReadyListener {
        ...
        public virtual IList<PdfDocument> ExtractPageRanges(IList<PageRange> pageRanges) {
            IList<PdfDocument> splitDocuments = new List<PdfDocument>();
            foreach (var currentPageRange in pageRanges) {
                var currentPdfDocument = CreatePdfDocument(currentPageRange);
        ...
        private PdfDocument CreatePdfDocument(PageRange currentPageRange) {
            var newDocument = new PdfDocument(GetNextPdfWriter(currentPageRange), new DocumentProperties().SetEventCountingMetaInfo
                (metaInfo));
        ...
        public virtual IList<PdfDocument> SplitByOutlines(IList<string> outlineTitles) {
            ...
            foreach (var title in outlineTitles) {
                var document = SplitByOutline(title);
                if (document != null) {
        ...
        private PdfDocument SplitByOutline(string outlineTitle) {
            var startPage = -1;
            var endPage = -1;
            var toDocument = CreatePdfDocument(null);
            var size = pdfDocument.GetNumberOfPages();
            for (var i = 1; i <= size; i++) {
                var pdfPage = pdfDocument.GetPage(i);
                var outlineList = pdfPage.GetOutlines(false);
                if (outlineList != null) {
                    foreach (var pdfOutline in outlineList) {
                        if (pdfOutline.GetTitle().Equals(outlineTitle)) {
                            startPage = pdfDocument.GetPageNumber(pdfPage);
                            var nextOutLine = GetAbsoluteTreeNextOutline(pdfOutline);
                            if (nextOutLine != null) {
        ...
        private PdfOutline GetAbsoluteTreeNextOutline(PdfOutline outline) {
            var nextPdfObject = outline.GetContent().Get(PdfName.Next);
            PdfOutline nextPdfOutline = null;
            if (outline.GetParent() != null && nextPdfObject != null) {
                foreach (var pdfOutline in outline.GetParent().GetAllChildren()) {
        ...
        private PageRange GetNextRange(int startPage, int endPage, long size) {
            var counter = new PdfResourceCounter(pdfDocument.GetTrailer());
            var resources = counter.GetResources();
            // initialize with trailer length
            var lengthWithoutXref = counter.GetLength(null);
            var currentPage = startPage;
            var oversized = false;
            do {
                var page = pdfDocument.GetPage(currentPage++);
