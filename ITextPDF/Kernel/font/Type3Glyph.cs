...
namespace IText.Kernel.Font {
    ...
    public sealed class Type3Glyph : PdfCanvas {
        ...
        private void FillBBFromBytes(byte[] bytes) {
            var str = JavaUtil.GetStringForBytes(bytes, EncodingUtil.ISO_8859_1);
            var d0Pos = str.IndexOf(D_0_STR, StringComparison.Ordinal);
            var d1Pos = str.IndexOf(D_1_STR, StringComparison.Ordinal);
            if (d0Pos != -1) {
                isColor = true;
                var bbArray = StringUtil.Split(str.JSubstring(0, d0Pos - 1), " ");
                if (bbArray.Length == 2) {
                    wx = float.Parse(bbArray[0], CultureInfo.InvariantCulture);
                }
            }
            else {
                if (d1Pos != -1) {
                    isColor = false;
                    var bbArray = StringUtil.Split(str.JSubstring(0, d1Pos - 1), " ");
                    if (bbArray.Length == 6) {
                        wx = float.Parse(bbArray[0], CultureInfo.InvariantCulture);
                        llx = float.Parse(bbArray[2], CultureInfo.InvariantCulture);
                        lly = float.Parse(bbArray[3], CultureInfo.InvariantCulture);
                        urx = float.Parse(bbArray[4], CultureInfo.InvariantCulture);
                        ury = float.Parse(bbArray[5], CultureInfo.InvariantCulture);
                    }
                }
            }
        }
