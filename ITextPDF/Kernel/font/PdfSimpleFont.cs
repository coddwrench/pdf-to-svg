...
namespace IText.Kernel.Font {
    public abstract class PdfSimpleFont<T> : PdfFont
        ...
        protected internal bool forceWidthsOutput;

        /// <summary>The array used with single byte encodings.</summary>
        protected internal byte[] shortTag = new byte[SIMPLE_FONT_MAX_CHAR_CODE_VALUE + 1];
        ...
        public override GlyphLine CreateGlyphLine(string content) {
            ...
            if (fontEncoding.IsFontSpecific()) {
                for (var i = 0; i < content.Length; i++) {
                    var glyph = fontProgram.GetGlyphByCode(content[i]);
                    if (glyph != null) {
                        glyphs.Add(glyph);
                    }
                }
        ...
        public override int AppendGlyphs(string text, int from, int to, IList<Glyph> glyphs) {
            ...
            if (fontEncoding.IsFontSpecific()) {
                for (var i = from; i <= to; i++) {
                    var glyph = fontProgram.GetGlyphByCode(text[i] & 0xFF);
                    if (glyph != null) {
                        glyphs.Add(glyph);
                        processed++;
                    }
                ...
                for (var i = from; i <= to; i++) {
                    ...
                    if (glyph != null && (ContainsGlyph(glyph.GetUnicode()) || IsAppendableGlyph(glyph))) {
                        ...
                        if (glyph == null && TextUtil.IsWhitespaceOrNonPrintable(text[i])) {
                            processed++;
                        }
                        else {
                            break;
        ...
        public override byte[] ConvertToBytes(string text) {
            var bytes = fontEncoding.ConvertToBytes(text);
        ...
        {
	        if (glyphLine != null) {
                var bytes = new byte[glyphLine.Size()];
                var ptr = 0;
                if (fontEncoding.IsFontSpecific()) {
                    for (var i = 0; i < glyphLine.Size(); i++) {
        ...
        public override bool AppendDecodedCodesToGlyphsList(IList<Glyph> list, PdfString characterCodes) {
            ...
            var enc = GetFontEncoding();
            var contentBytes = characterCodes.GetValueBytes();
            foreach (var b in contentBytes) {
                var code = b & 0xff;
                Glyph glyph = null;
                var toUnicodeCMap = GetToUnicode();
        ...
        protected internal virtual void FlushFontData(string fontName, PdfName subtype) {
            ...
            if (fontEncoding.HasDifferences()) {
                ...
                for (var k = lastChar; k >= firstChar; --k) {
                    if (!FontEncoding.NOTDEF.Equals(fontEncoding.GetDifference(k))) {
                        lastChar = k;
                        break;
                    }
            ...
            if (IsForceWidthsOutput() || !IsBuiltInFont() || fontEncoding.HasDifferences()) {
                ...
                var wd = BuildWidthsArray(firstChar, lastChar);
                GetPdfObject().Put(PdfName.Widths, wd);
            }
            var fontDescriptor = !IsBuiltInFont() ? GetFontDescriptor(fontName) : null;
        ...
        protected internal override PdfDictionary GetFontDescriptor(string fontName) {
            Debug.Assert(fontName != null && fontName.Length > 0);
        ...
        protected internal virtual PdfArray BuildWidthsArray(int firstChar, int lastChar) {
            ...
            for (var k = firstChar; k <= lastChar; ++k) {
                if (shortTag[k] == 0) {
                    ...
                    var uni = fontEncoding.GetUnicode(k);
                    var glyph = uni > -1 ? GetGlyph(uni) : fontProgram.GetGlyphByCode(k);
