		...
		internal static TrueTypeFont CreateFontProgram(PdfDictionary fontDictionary, CMapToUnicode toUnicode)
		{
			...
			if (toUnicode != null)
			{
				...
				foreach (var cid in toUnicode.GetCodes())
				{
					...
					fontProgram.СodeToGlyph.Put(cid, glyph);
					fontProgram.avgWidth += width;
				}

				if (fontProgram.СodeToGlyph.Count != 0)
				{
					fontProgram.avgWidth /= fontProgram.СodeToGlyph.Count;
				}
			}

			if (fontProgram.СodeToGlyph.Get(0) == null)
			{
				fontProgram.СodeToGlyph.Put(0, new Glyph(0,
