...
namespace IText.Kernel.Font
{
	...
	public class Type3Font : FontProgram
	{
		...
		{
			var glyph = type3GlyphsWithoutUnicode.Get(code);
			if (glyph == null && ÑodeToGlyph.Get(code) != null)
			{
				glyph = type3Glyphs.Get(ÑodeToGlyph.Get(code).GetUnicode());
			}
			return glyph;
		}
		...
		{
			var removed = ÑodeToGlyph.JRemove(glyphCode);
			if (removed == null)
			{
				return;
			}
		...
		{
			var widthSum = 0;
			var glyphsNumber = ÑodeToGlyph.Count;
			foreach (var glyph in ÑodeToGlyph.Values)
			{
				if (glyph.GetWidth() == 0)
				{
					glyphsNumber--;
