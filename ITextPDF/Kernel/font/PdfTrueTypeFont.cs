...
namespace IText.Kernel.Font {
    ...
    public class PdfTrueTypeFont : PdfSimpleFont<TrueTypeFont> {
        internal PdfTrueTypeFont(TrueTypeFont ttf, string encoding, bool embedded)
        {
            ...
            var fontNames = ttf.GetFontNames();
        ...
        /// </param>
        [Obsolete(@"use iText.IO.Font.TrueTypeFont.UpdateUsedGlyphs(Java.Util.SortedSet{E}, bool, System.Collections.Generic.IList{E})"
            )]
            ...
            if (embedded) {
                ...
                if (fontProgram is IDocFontProgram) {
                    ...
                    if (((TrueTypeFont)GetFontProgram()).IsCff()) {
                        ...
                        try {
                            var fontStreamBytes = ((TrueTypeFont)GetFontProgram()).GetFontStreamBytes();
                            fontStream = GetPdfFontStream(fontStreamBytes, new[] { fontStreamBytes.Length });
                            fontStream.Put(PdfName.Subtype, new PdfName("Type1C"));
                        }
                        catch (PdfException e) {
                            var logger = LogManager.GetLogger(typeof(PdfTrueTypeFont));
                        ...
                        for (var k = 0; k < shortTag.Length; k++) {
                            if (shortTag[k] != 0) {
                                ...
                                var glyph = uni > -1 ? fontProgram.GetGlyph(uni) : fontProgram.GetGlyphByCode(k);
                                if (glyph != null) {
