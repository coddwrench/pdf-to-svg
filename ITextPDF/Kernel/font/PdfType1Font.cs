...
namespace IText.Kernel.Font {
    public class PdfType1Font : PdfSimpleFont<Type1Font> {
        ...
        protected internal override void AddFontStream(PdfDictionary fontDescriptor) {
            if (embedded) {
                if (fontProgram is IDocFontProgram) {
                    var docType1Font = (IDocFontProgram)fontProgram;
                    ...
                    var fontStreamBytes = ((Type1Font)GetFontProgram()).GetFontStreamBytes();
                    if (fontStreamBytes != null) {
                        var fontStream = new PdfStream(fontStreamBytes);
                        var fontStreamLengths = ((Type1Font)GetFontProgram()).GetFontStreamLengths();
