	...
	internal class DocFontEncoding : FontEncoding
	{
		...
		public static FontEncoding CreateDocFontEncoding(PdfObject encoding, CMapToUnicode toUnicode)
		{
			if (encoding != null)
			{
				if (encoding.IsName())
				{
					return CreateFontEncoding(((PdfName)encoding).GetValue());
				}

				if (encoding.IsDictionary())
				{
					var fontEncoding = new DocFontEncoding();
			...
			return CreateFontSpecificEncoding();
		}
		...
		private static void FillDifferences(DocFontEncoding fontEncoding, PdfArray diffs, CMapToUnicode
			 toUnicode)
		{
			var byte2uni = toUnicode != null ? toUnicode.CreateDirectMapping() : new IntHashtable();
			if (diffs != null)
			{
				var currentNumber = 0;
				for (var k = 0; k < diffs.Size(); ++k)
				{
					...
					else
					{
						...
						{
							var LOGGER = LogManager.GetLogger(typeof(DocFontEncoding));
							LOGGER.Warn(MessageFormatUtil.Format(LogMessageConstant.DOCFONT_HAS_ILLEGAL_DIFFERENCES, ((PdfName
								)obj).GetValue()));
						}
						else
						{
                            ...
                            */
							...
							var unicode = AdobeGlyphList.NameToUnicode(glyphName);
							if (unicode != -1)
							{
		...
		{
			var byte2uni = toUnicode.CreateDirectMapping();
			foreach (int? code in byte2uni.GetKeys())
			{
				var unicode = byte2uni.Get((int)code);
