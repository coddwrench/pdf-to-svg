...
namespace Svg
{
	internal class SvgPaintServerFactory : TypeConverter
	{
		...
		public static SvgPaintServer Create(string value, SvgDocument document)
		{
			...
			else
			{
				...
				while (!string.IsNullOrEmpty(value))
				{
					...
					else if (document.IdManager.GetElementById(value) != null &&
							 document.IdManager.GetElementById(value).GetType().BaseType == typeof(SvgGradientServer))
					{
					...
					else if (value.StartsWith("#")) // Otherwise try and parse as colour
					{
						switch (CountHexDigits(value, 1))
						{
							case 3:
								servers.Add(
									new SvgColourServer((Color)_colourConverter.ConvertFrom(value.Substring(0, 4))));
		...
		public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
		{
			...
			{
				//check for none
				if (value == SvgPaintServer.None || value == SvgPaintServer.None) return "none";
				if (value == SvgPaintServer.Inherit) return "inherit";
				if (value == SvgPaintServer.NotSet) return "";
